...
impl ResolverConfigLisp {
    ...
    /// claimed the `defresolver-config` keyword in this process. Peer
    /// of [`caixa_core::Caixa::register`] and the other per-crate entry
...
"#;
    ...
    fn register_populates_registry() {
        ResolverConfigLisp::register()
            .expect("first register call in this test process must succeed");
