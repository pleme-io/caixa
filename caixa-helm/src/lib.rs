...
fn build_readme(caixa: &Caixa, chart_name: &str) -> String {
    ...
    // Route the per-`README.md` `## Origin` line's `{repo}` interpolation
    // through the substrate-canonical [`caixa_core::Caixa::canonical_git_url`]
    // resolved-git-URL composer (124f864) rather than the prior
    // `caixa.repositorio().unwrap_or(caixa.nome())` two-arm inline fallback.
    // The prior shape's `:repositorio`-null arm folded to `caixa.nome()`
    // verbatim, so the rendered README's `## Origin` line emitted the
    // meaningless `Generated by `caixa-helm` from `<nome>/caixa.lisp``
    // scalar on every `:repositorio`-null caixa — a bare `<nome>` prefix
    // that told a downstream reader nothing about where the caixa's
    // source actually lives, and disagreed on that same axis with the
    // sibling [`caixa_flux::ClusterBundleOpts::for_caixa`] `git_url`
    // seed which had already routed through the resolved-URL composer
    // and emitted the canonical `https://github.com/{DEFAULT_PLEME_GIT_ORG}/<nome>`
    // pleme-org fallback on the same author-omitted-`:repositorio` case.
    // The 124f864 commit body explicitly anticipated this converge (the
    // "same accessor consumed by every renderer" convergence surface it
    // called out as the live-behavior-correcting fold distinct from that
    // commit's pure byte-preserving convergence). Post-lift the two
    // renderers agree byte-for-byte on the resolved-URL substrate
    // primitive on both arms, and the README's `## Origin` line names a
    // concrete pleme-org URL on the fallback path.
    format!(
        "# {chart_name}\n\
         \n\
         {descricao}\n\
...
mod tests {
    ...
    use caixa_core::{
        ...
        kube_u64, mapping_string_keys, parse_yaml_at_path, parse_yaml_at_path_as,
    };
...
"#,
    ...
    fn chart_yaml_metadata_propagates() {
        let dir = render_chart_for_servico(&sample_caixa(), &sample_cu_yaml()).unwrap();
        let chart: ChartYaml = parse_yaml_at_path_as(&dir.files, HELM_CHART_YAML_FILENAME);
    ...
    fn limits_slot_propagates_into_values_block() {
        ...
        assert_eq!(kube_u64(limits, M2_LIMITS_KEY_FUEL), Some(1_000_000));
    ...
    #[test]
    fn readme_repositorio_null_fallback_routes_through_canonical_git_url_accessor() {
        // Emit-path pin: on a `:repositorio`-null caixa the
        // [`build_readme`] `## Origin` line's `{repo}` interpolation must
        // derive from the substrate-canonical
        // [`caixa_core::Caixa::canonical_git_url`] resolved-git-URL
        // composer's None-arm fallback (`https://github.com/{DEFAULT_PLEME_GIT_ORG}/<nome>`)
        // rather than the prior `caixa.repositorio().unwrap_or(caixa.nome())`
        // two-arm inline whose fallback folded to the bare `<nome>` scalar
        // and emitted the meaningless `Generated by `caixa-helm` from
        // `<nome>/caixa.lisp`` line the 124f864 commit body explicitly
        // called out as the live-behavior-correcting converge surface
        // waiting on this lift. Fail-before-pass-after: the pre-lift
        // shape carried the bare `hello-rio/caixa.lisp` substring and
        // this pin asserts the post-lift canonical pleme-org URL substring
        // in its place, so a regression that re-inlines the raw two-arm
        // fallback at the emit site surfaces at caixa-helm build time on
        // this test's failure rather than at a downstream README-consuming
        // UI's "where does this caixa live?" broken-URL trail.
        let caixa = Caixa {
            repositorio: None,
            ..sample_caixa()
        };
        ...
        assert!(
            ...
            "README.md `:repositorio`-null fallback must derive its \
             ...
             `caixa_core::Caixa::canonical_git_url` resolved-git-URL \
             composer byte-for-byte ({expected:?}) — a regression that \
             re-inlines the prior `caixa.repositorio().unwrap_or(caixa.nome())` \
             two-arm fallback at the emit site silently regresses the \
             README origin-line repo axis to the meaningless bare `<nome>` \
             prefix and splits the caixa-helm README axis from the sibling \
             [`caixa_flux::ClusterBundleOpts::for_caixa`] `git_url` seed \
             which already resolves through the canonical composer. \
             Full contents:\n{contents}",
            contents = readme_file.contents,
        );
    }
    ...
    #[test]
    fn readme_repositorio_null_fallback_no_longer_emits_bare_nome_prefix() {
        // Fail-before-pass-after regression guard: pins the negative arm
        // the sibling
        // [`readme_repositorio_null_fallback_routes_through_canonical_git_url_accessor`]
        // pin's positive arm implies — the pre-lift bare `<nome>/caixa.lisp`
        // substring (the shape the prior `caixa.repositorio().unwrap_or(caixa.nome())`
        // inline two-arm fallback emitted on every `:repositorio`-null
        // caixa) must NOT appear in the rendered README's `## Origin` line.
        // A regression that re-inlines the two-arm fallback at
        // [`build_readme`]'s emit site would satisfy the positive
        // canonical-composer substring check on the `Some(<full-URL>)`
        // arm and drop the fallback arm back to `<nome>/caixa.lisp`
        // silently — so an explicit negative-substring pin closes the
        // remaining converge surface the sibling positive pin does not
        // catch on its own.
        let caixa = Caixa {
            repositorio: None,
            ..sample_caixa()
        ...
        assert!(
            !readme_file.contents.contains(&regressed),
            "README.md `:repositorio`-null fallback must NOT emit the \
             pre-lift bare `<nome>/caixa.lisp` prefix ({regressed:?}) — \
             the `caixa.repositorio().unwrap_or(caixa.nome())` two-arm \
             inline fallback was retired in favor of \
             `caixa.canonical_git_url()` so the fallback arm now names \
             the substrate-canonical pleme-org URL; a regression that \
             re-inlines the two-arm fallback at the emit site would \
             silently split this caixa-helm README axis from the sibling \
             [`caixa_flux::ClusterBundleOpts::for_caixa`] `git_url` seed. \
             Full contents:\n{contents}",
            contents = readme_file.contents,
        );
    ...
    #[test]
    fn readme_repositorio_declared_arm_routes_through_canonical_git_url_accessor() {
        // Emit-path pin (peer of the `:repositorio`-null fallback sibling):
        // on a `:repositorio`-declared caixa the [`build_readme`] `## Origin`
        // line's `{repo}` interpolation must also derive from the substrate-
        // canonical [`caixa_core::Caixa::canonical_git_url`] resolved-git-URL
        // composer — the Some-arm return byte-string is the author-declared
        // `:repositorio` verbatim, so the substring the rendered README
        // carries matches the composer output on both arms and the pair
        // of pins jointly pins the entire two-arm dispatch through one
        // canonical composer. A regression that split the emit site into
        // a Some-arm-only bypass through the raw
        // [`caixa_core::Caixa::repositorio`] accessor while leaving the
        // fallback arm routed through the canonical composer would
        // silently drift the two-arm shape and be caught here (the raw
        // accessor path bypasses any future extension of the resolved-URL
        // composer, e.g. a `github:` → `https://github.com/` canonicalization
        // pass or the per-cluster repo-mirror overlay the M4 CR materializer
        // will apply).
        let caixa = sample_caixa();
        assert!(
            caixa.repositorio().is_some(),
            "sample_caixa() must carry a `Some(...)` `:repositorio` for \
             the Some-arm to exercise the canonical composer's non-fallback \
             path — a future sample_caixa() edit that drops the field must \
             re-establish it here or the pin regresses to a no-op",
        );
        let dir = render_chart_for_servico(&caixa, &sample_cu_yaml()).unwrap();
        let readme_file =
            find_file_by_path(&dir.files, HELM_CHART_README_FILENAME).expect("README.md present");
        ...
        assert!(
            readme_file.contents.contains(&expected),
            "README.md `:repositorio`-declared arm must derive its `repo` \
             interpolation from the typed `caixa_core::Caixa::canonical_git_url` \
             resolved-git-URL composer byte-for-byte ({expected:?}) so the \
             two-arm dispatch routes through one canonical composer end to \
             end. Full contents:\n{contents}",
            contents = readme_file.contents,
        );
    }
    ...
    #[test]
    fn parse_yaml_at_path_as_matches_prior_inline_find_then_from_str_typed_shape() {
        // Per-crate byte-equivalence pin on the lifted
        // [`caixa_core::parse_yaml_at_path_as`] caller-typed composed
        // navigator: for the `Chart.yaml` YAML leaf the
        // `render_chart_for_servico` `lareira-<nome>` chart-directory
        // emit writes that the test-side routed callers parse to a
        // [`ChartYaml`]-typed body, the lifted helper must return a
        // `ChartYaml` byte-equal to the prior two-step
        //
        ...
        //
        // the 9 test-side per-`Chart.yaml`-typed-parse readback sites
        // previously carried. Mirrors the sibling
        // [`parse_yaml_at_path_matches_prior_inline_find_then_from_str_shape`]
        // untyped-Value-arity pin in this crate at the caller-typed-
        // arity peer, and the substrate-level
        // `parse_yaml_at_path_as_matches_prior_inline_typed_two_step_shape`
        // pin at the primitive definition — the three-arm closure
        // that closes the same discipline the sibling
        // [`parse_yaml_at_path`] sweep established. Only
        // [`HELM_CHART_YAML_FILENAME`] is swept: the routed
        // caller-typed-parse sites in this crate reach exclusively
        // through the `ChartYaml`-typed body at the `Chart.yaml`
        // leaf; [`HELM_VALUES_YAML_FILENAME`] carries no typed-mirror
        // struct in this crate (the routed callers navigate its
        // parsed [`serde_yaml::Value`] through `kube_*` / `mapping_*`
        // primitives), and [`HELM_CHART_README_FILENAME`] is not
        // parsed as YAML at all.
        let dir = render_chart_for_servico(&sample_caixa(), &sample_cu_yaml()).unwrap();
        let filename = HELM_CHART_YAML_FILENAME;
        let via_helper: ChartYaml = parse_yaml_at_path_as(&dir.files, filename);
        let via_inline_file = find_file_by_path(&dir.files, filename).unwrap();
        let via_inline: ChartYaml = serde_yaml::from_str(&via_inline_file.contents).unwrap();
        assert_eq!(
            via_helper, via_inline,
            "parse_yaml_at_path_as::<ChartYaml>(&dir.files, {filename:?}) \
             must byte-equal the prior two-step \
             `find_file_by_path(&dir.files, {filename:?}).unwrap() + \
             serde_yaml::from_str::<ChartYaml>(&_.contents).unwrap()` \
