...
/// [`ArchVerdict`] closed-set caixa-arch verdict-outcome typed enum —
/// routes byte-for-byte through the paired
/// [`TryFrom<&str> for ArchVerdict`] impl (which in turn routes
/// through the substrate-primitive [`ArchVerdict::from_wire`]
/// `Option<Self>` accessor), so `"...".parse::<ArchVerdict>()` /
/// `<ArchVerdict as FromStr>::from_str(…)` reaches the same two-arm
/// canonical-lowercase `"proven"` / `"rejected"` accept-set the
/// sibling method-named [`ArchVerdict::from_wire`] resolver and the
/// paired [`TryFrom<&str>`] impl already resolve against.
///
...
/// fieldless typed enum via [`crate::invariants::InvariantKind`]
/// (fb16072) — onto the *second outside-caixa-core* closed-set
...
/// Coherent by construction on this enum specifically: [`ArchVerdict`]
/// carries exactly one canonical lowercase wire axis (`proven` /
/// `rejected`) with no paired secondary parse surface, so — unlike
/// the peer [`caixa_core::supervisor::RestartStrategy`] /
...
/// lifting [`FromStr`] onto [`ArchVerdict`] cannot collide with a
...
/// onto the closed-set enum unlocks the `.parse::<ArchVerdict>()`
/// short-form on every consumer (a future `feira arch --verdict
/// <proven|rejected>` clap-style arg-parse composes
/// `arg.parse::<ArchVerdict>()`; a `serde` string-tagged deserializer
/// with a `#[serde(with = "serde_with::DisplayFromStr")]` shim routes
/// through the same `FromStr` bound; a future M4
/// `mesh.pleme.io/v1alpha1/ArchAudit` CR admission-webhook body
/// reloader that walks a `Vec<String>` of prior
/// `ArchVerdict::as_str` outputs reaches the typed enum through
/// `line.parse::<ArchVerdict>()` without a per-consumer
/// `TryFrom<&str>` restatement).
///
...
/// to the same two-arm accept-set by construction. A future arm
/// addition (a `PartiallyProven` tier the `iac-forge` policy-engine
/// grows for compliance-only violation sets, an `Unknown` tier for
/// the M4 admission-webhook's timeout-during-check outcome — both
/// trajectory items the sibling [`ArchVerdict::ALL`] doc block
/// already names) reaches every parse path through one edit on the
/// substrate-primitive [`ArchVerdict::from_wire`] accessor, not a
/// coordinated rewrite across the two reverse-projection trait
/// impls.
///
/// Pinned load-bearing by
/// [`tests::arch_verdict_from_str_routes_through_try_from_str_impl`]
/// (byte-parity pin across the two-arm accept-set + delegated-
/// `.parse()`-projection witness) and
/// [`tests::arch_verdict_from_str_rejects_unknown_byte_strings`]
/// (rejection witness against silent accept-set widening).
impl std::str::FromStr for ArchVerdict {
    type Err = ();
...
/// projection on the caixa-arch verdict-outcome two-arm closed-set
/// fieldless typed enum [`ArchVerdict`] — the single-threaded reference-
/// counted peer of the paired owned-input
/// [`From<ArchVerdict> for std::sync::Arc<str>`] impl above on the
...
/// idiomatic surface a `let key: std::rc::Rc<str> = verdict.into();`-
/// shaped call site reaches through this impl and no other. A single-
/// threaded caixa-arch audit-report renderer whose per-`ArchReport`
/// verdict-outcome label is cloned across intra-report tree nodes via
/// [`std::rc::Rc::clone`] (its cheaper non-atomic refcount bump the
/// [`std::sync::Arc<str>`] axis's atomically-reference-counted return-
/// shape cannot provide within a single-threaded audit pass) reaches
/// the substrate-primitive [`ArchVerdict::as_str`] accessor through
/// this impl and no other.
///
/// Extends the outside-`caixa-core` tier of the trait-idiomatic
/// [`std::rc::Rc<str>`] forward-projection axis onto its second peer,
/// following the first-mover [`crate::invariants::InvariantKind`] pair
/// (13a4ad1 `{Self, &Self}` corner) that opened the tier — after the
/// axis opened on the structurally most fundamental caixa-core-internal
...
/// (the paired `impl From<ArchVerdict> for std::sync::Arc<str>` /
/// `impl From<&ArchVerdict> for std::sync::Arc<str>` pair above).
///
/// Pinned load-bearing by
/// [`tests::arch_verdict_from_into_rc_str_routes_through_as_str_accessor`]
/// (byte-parity pin against [`ArchVerdict::as_str`] across the two-arm
/// [`ArchVerdict::ALL`] emit-set on the owned-input surface, plus a
/// blanket-derived [`Into`] shape witness and cross-axis byte-parity
...
/// axes).
impl From<ArchVerdict> for std::rc::Rc<str> {
    fn from(verdict: ArchVerdict) -> std::rc::Rc<str> {
        std::rc::Rc::<str>::from(verdict.as_str())
    }
}

/// Trait-idiomatic *borrowed-input, [`std::rc::Rc<str>`] output* forward
/// projection on the caixa-arch verdict-outcome two-arm closed-set
/// fieldless typed enum [`ArchVerdict`] — the borrowed-input companion
/// to the paired owned-input [`From<ArchVerdict> for std::rc::Rc<str>`]
/// impl immediately above, closing the `{Self, &Self}` input-shape
/// corner of the outside-`caixa-core` tier of the [`std::rc::Rc<str>`]
/// axis on the second peer. Routes byte-for-byte through the
/// substrate-primitive [`ArchVerdict::as_str`] `pub const fn` accessor
/// via [`std::rc::Rc::<str>::from`] on the returned `&'static str`, so a
/// `ArchVerdict::ALL.iter().map(std::rc::Rc::<str>::from)`-shaped pipe
/// (whose iterator over `&'static [ArchVerdict]` yields `&ArchVerdict`
/// by construction) reaches the same two `"proven"` / `"rejected"`
/// canonical-lowercase byte-strings the paired owned-input axis and the
/// sibling `{Self, &Self} × {&'static str, String, Cow<'static, str>,
/// Box<str>, std::sync::Arc<str>}` forward-projection corner already
/// return.
///
...
/// [`ArchVerdict::ALL`]`.iter().map(std::rc::Rc::<str>::from)` reaches
/// through this impl and no other — without it, the same pipe would
/// force a spurious [`Copy`] deref
/// (`std::rc::Rc::<str>::from((*verdict).as_str())`) or a `.copied()`
...
/// trajectory the paired [`caixa_core::CaixaKind`] (4fdfa37),
/// [`caixa_core::CaixaDialeto`] (c884517),
/// [`caixa_core::aplicacao::PlacementStrategy`] (30a1dee), and
/// [`crate::invariants::InvariantKind`] (13a4ad1) pairs walked before
/// it on the caixa-core-internal, M3-mesh-primitive-defining, and
/// first-mover outside-`caixa-core` tiers of the same axis. Leaves the
/// remaining outside-`caixa-core` closed-set fieldless typed enum peers
/// ([`caixa_lint::Severity`], [`caixa_lint::FixSafety`],
...
///
/// Pinned load-bearing by
/// [`tests::arch_verdict_from_borrowed_into_rc_str_routes_through_as_str_accessor`]
/// (byte-parity pin against [`ArchVerdict::as_str`] across the two-arm
/// [`ArchVerdict::ALL`] emit-set on the borrowed-input surface, plus a
/// blanket-derived [`Into`] shape witness, a cross-axis partition pin
/// against the paired owned-input
/// [`From<ArchVerdict> for std::rc::Rc<str>`] and the sibling
/// borrowed-input `{&'static str, String, Cow<'static, str>, Box<str>,
/// std::sync::Arc<str>}` return-shape axes, and a
/// `.iter().map(std::rc::Rc::<str>::from)` pipe witness over
/// [`ArchVerdict::ALL`] that resolves through the borrowed-input axis
...
/// Trait-idiomatic *byte-view* projection on the [`ArchVerdict`] closed-
/// set caixa-arch verdict-outcome typed enum — routes byte-for-byte
/// through the substrate-primitive [`ArchVerdict::as_str`] `pub const fn`
/// accessor's `.as_bytes()` byte-tail so every consumer that binds an
/// [`ArchVerdict`] through the standard-library `<T: AsRef<[u8]>>` trait
/// bound (a future [`std::io::Write::write_all`]-bound `feira arch`
/// per-report diagnostic sink; a byte-keyed
/// [`std::collections::HashMap`] `<K: AsRef<[u8]>, V>` per-verdict
...
/// `sha2::Sha256::update` byte-input surface on any future per-verdict
/// content-address digest folded into the [`ArchReport`] audit closure)
/// reaches the substrate primitive through one trait dispatch rather
/// than open-coding the two-hop `verdict.as_str().as_bytes()`
...
/// `verdict.as_str().as_bytes()` is reachable through the pre-existing
...
/// `PartiallyProven` tier the `iac-forge` policy-engine grows for
/// compliance-only violation sets, an `Unknown` tier for the M4
/// admission-webhook's timeout-during-check outcome — both trajectory
/// items the sibling [`ArchVerdict::ALL`] doc block already names)
/// grows the byte-view axis through one edit on the substrate-primitive
...
/// impl (daa8705), [`caixa_core::aplicacao::RateLimitUnit`] impl
/// (4867e0f), and the first-mover outside-caixa-core
/// [`crate::invariants::InvariantKind`] impl (b5baf7d) walked before it.
/// The paired [`ArchVerdict::as_str`] accessor returns the canonical
/// lowercase wire byte-string (`"proven"` / `"rejected"`) every
/// `feira arch` per-report render site, every `feira tofu` HCL-emission
/// gate, and every future M4 admission-webhook / policy-engine
/// audit-loader keys off. Leaves the remaining outside-`caixa-core`
/// closed-set fieldless typed-enum peers ([`caixa_lint::Severity`],
/// [`caixa_lint::FixSafety`],
/// [`caixa_lint::diagnostic::PathShapeViolation`],
/// [`caixa_theme::Semantic`], [`caixa_provedor::FerriteRuntime`]) as
...
///
/// Pinned load-bearing by
/// [`tests::arch_verdict_as_ref_bytes_routes_through_as_str_accessor`]
/// (fail-before-pass-after byte-parity pin against
/// [`ArchVerdict::as_str`] `.as_bytes()` across the two-arm
/// [`ArchVerdict::ALL`] emit-set, cross-axis witness against the paired
/// str-view [`AsRef<str>`] / [`std::fmt::Display`] /
/// [`ArchVerdict::as_str`] axes' `.as_bytes()` byte-tails, cross-axis
...
/// axis and the byte-view *reverse* companion of the pre-existing
/// byte-view *forward* triple ([`AsRef<[u8]>`],
/// [`From<ArchVerdict> for Vec<u8>`], [`From<&ArchVerdict> for Vec<u8>`])
/// lifted on this same enum. Routes borrowed byte-slice input through
/// [`std::str::from_utf8`] + the substrate-primitive
/// [`ArchVerdict::from_wire`] `Option<Self>` accessor so every consumer
/// that binds a byte-slice through the standard-library
/// `impl TryFrom<&[u8]> for ArchVerdict` axis (equivalently
/// `<T: for<'a> TryFrom<&'a [u8]>>`) — a future
/// [`std::io::Read::read_to_end`]-shape per-`ArchReport` ingest byte-
/// source whose framing already carries the canonical two-arm verdict
/// byte-string (`b"proven"` / `b"rejected"`) as a bare byte-slice, a
/// future protobuf/CBOR/msgpack field-decoder whose bytes arm surfaces
/// the verdict tag before UTF-8 validation, a future M4
/// `mesh.pleme.io/v1alpha1/ArchAudit` admission-webhook intercepting the
/// raw per-report verdict request bytes before the serde derive
/// dispatches, a future `bytes::Bytes::as_ref()`-shape byte-tail composer
/// folding a wire payload through the byte-view reverse-projection axis
/// before any UTF-8 round-trip — reaches the substrate primitive through
/// one trait dispatch rather than an open-coded per-call-site
/// `std::str::from_utf8(bytes).ok().and_then(ArchVerdict::from_wire)`
/// composition or a
/// `<ArchVerdict as TryFrom<&str>>::try_from(std::str::from_utf8(bytes)?)`
...
/// projection* axis onto the *second outside-`caixa-core`* closed-set
/// fieldless typed-enum peer — matching the paired [`AsRef<[u8]>`]
/// borrowed byte-view forward axis's second-outside-`caixa-core` position
/// and the paired [`From<ArchVerdict> for Vec<u8>`] byte-owned reverse-
/// projection axis's second-outside-`caixa-core` position on this same
/// enum. Tracks the trajectory the caixa-core-internal
/// [`caixa_core::CaixaKind`] `TryFrom<&[u8]>` first-mover impl (18d1940),
/// the second-mover [`caixa_core::dialeto::CaixaDialeto`] impl (d102cb8),
/// the third-mover [`caixa_core::dep::DepList`] impl (b8f25d5), the
...
/// (de03220), [`caixa_core::aplicacao::RateLimitUnit`] (294c77d), and
/// [`caixa_core::aplicacao::WitShape`] (de9875c) peers, and the first-
/// mover outside-`caixa-core` [`crate::invariants::InvariantKind`] impl
/// (40b7417) walked before it. Leaves the remaining outside-`caixa-core`
/// closed-set fieldless typed-enum peers ([`caixa_lint::Severity`],
/// [`caixa_lint::FixSafety`],
/// [`caixa_lint::diagnostic::PathShapeViolation`],
/// [`caixa_theme::Semantic`], [`caixa_provedor::FerriteRuntime`]) as the
/// campaign's next mechanical one-lift extensions against the substrate-
/// primitive `from_wire` accessor they already carry.
///
/// Rust's standard library carries no blanket
...
///
/// `type Error = ()` matches the sibling [`ArchVerdict::from_wire`]'s
...
/// future `feira arch --verdict <proven|rejected>` clap-style arg-parse
/// composes its own per-verb "unknown verdict: <arg> — accepted:
/// {proven, rejected}" message; a future M4
/// `mesh.pleme.io/v1alpha1/ArchAudit` admission-webhook rejection body
...
/// that falls outside the two-arm verdict accept-set
/// ([`ArchVerdict::from_wire`] returns `None`) — both collapse onto
...
/// [`ArchVerdict::from_wire`] explicitly.
///
/// Pinned load-bearing by
/// [`tests::arch_verdict_try_from_bytes_routes_through_from_wire_accessor`]
/// (byte-parity pin against [`ArchVerdict::from_wire`] across the two-arm
/// [`ArchVerdict::ALL`] accept-set on the borrowed byte-slice surface,
/// plus a cross-axis witness that the byte-view reverse projection agrees
/// with the paired [`TryFrom<&str>`] str-view reverse axis on every
/// accepted arm, and a forward/reverse byte-view cross-axis witness that
/// feeding the paired [`AsRef<[u8]>`] byte-tail and the paired
/// [`From<ArchVerdict> for Vec<u8>`] owned byte-tail back through the new
/// impl round-trips to the originating arm) and
/// [`tests::arch_verdict_try_from_bytes_rejects_unknown_and_non_utf8_bytes`]
...
/// rejection path — the latter mirrors the corpus the paired
/// [`TryFrom<&str>`] rejection witness already pins, including the peer
/// [`crate::invariants::InvariantKind`] three-arm severity axis's non-
/// shared canonical tags `b"safety"` / `b"compliance"` / `b"hint"` so a
/// byte-level cross-axis leak trips at caixa-arch test time rather than
/// at a downstream consumer's silent misclassification).
impl TryFrom<&[u8]> for ArchVerdict {
    type Error = ();
...
mod tests {
    ...
    #[test]
    fn arch_verdict_from_str_routes_through_try_from_str_impl() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl std::str::FromStr for ArchVerdict` — asserts the
        // standard-library `.parse()` parse-axis entry point and the
        // paired [`super::ArchVerdict::try_from`] `TryFrom<&str>` impl
        ...
        // [`caixa_core::CaixaDialeto`] (ff01b2f), and
        // [`crate::invariants::InvariantKind`] (fb16072) — onto the
        // *second outside-caixa-core* closed-set fieldless typed enum
        // on the caixa surface: the caixa-arch verdict-outcome axis
        // every [`crate::run::check_manifest`] verdict-summary
        // aggregation reads out. The peer method-named `from_wire`
        // accessor and the paired `TryFrom<&str>` trait impl fix the
        // two-arm canonical-lowercase accept-set; this pin locks the
        ...
        // past the two lifted
        // [`super::CAIXA_ARCH_VERDICT_WIRE_*`] consts) trips at
        // caixa-arch test time. Also covers the stdlib `.parse()`
        // short-form witness: `.parse::<ArchVerdict>()` is the
        ...
        for &variant in ArchVerdict::ALL {
            let wire = variant.as_str();
            assert_eq!(
                <ArchVerdict as FromStr>::from_str(wire),
                Ok(variant),
                "FromStr impl on ArchVerdict must round-trip \
                 ArchVerdict::{variant:?}.as_str() = {wire:?} back \
                 to Ok(ArchVerdict::{variant:?}) — divergence from \
            ...
            assert_eq!(
                wire.parse::<ArchVerdict>(),
                Ok(variant),
                "stdlib .parse::<ArchVerdict>() short-form on \
                 ArchVerdict::{variant:?}.as_str() = {wire:?} must \
                 route through the lifted FromStr impl and reach \
                 Ok(ArchVerdict::{variant:?})",
            );
            assert_eq!(
                <ArchVerdict as FromStr>::from_str(wire).ok(),
                ArchVerdict::from_wire(wire),
                "FromStr::from_str ok()-projection on {wire:?} must \
                 byte-equal ArchVerdict::from_wire on the same input \
                 — the two reverse-projection axes must resolve to \
                 the same two-arm canonical-lowercase accept-set",
            );
            assert_eq!(
                <ArchVerdict as FromStr>::from_str(wire),
                <ArchVerdict as TryFrom<&str>>::try_from(wire),
    ...
    #[test]
    fn arch_verdict_from_str_rejects_unknown_byte_strings() {
        ...
        // `"Proven"` / `"Rejected"` on the wire axis, a silent
        // acceptance of the peer [`crate::invariants::InvariantKind`]
        // three-arm severity axis's non-shared `"safety"` /
        // `"compliance"` / `"hint"` arms — accepting those would
        // silently split this axis's parser from the emitter's arm-
        // set, and misclassify a severity-shaped byte-string as a
        // verdict) trips at caixa-arch test time. The candidate set
        // spans the empty string, whitespace-only padding, uppercase
        // / mixed-case rebrand candidates, Levenshtein-neighbor
        // typos, sibling closed-set-enum canonical tags on the peer
        // [`caixa_core::CaixaKind`] six-arm axis and on the peer
        // [`caixa_lint::Severity`] four-arm severity axis, and
        // trailing/leading-whitespace-padded canonical tags. Pairs
        // the `<T as FromStr>::from_str` and stdlib `.parse()`
        // short-form witnesses so a future divergence between the
        // two parse-axis entry points (a hand-rolled `.parse`
        // override that bypasses the delegated `FromStr` impl) trips
        // here.
        use std::str::FromStr;
        for bad in [
            "",
            ...
            assert_eq!(
                <ArchVerdict as FromStr>::from_str(bad),
                Err(()),
                "FromStr for ArchVerdict({bad:?}) must return \
                 Err(()) — the trait impl's accept-set is exactly \
                 the two ArchVerdict::as_str outputs; a widening \
    ...
    fn arch_verdict_from_into_rc_str_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<ArchVerdict> for std::rc::Rc<str>` — asserts the
        ...
        // second peer, following the first-mover
        // [`crate::invariants::InvariantKind`] pair (13a4ad1) that
        // opened the tier — after the axis opened on the structurally
        // most fundamental caixa-core-internal
        // [`caixa_core::CaixaKind`] pair (4fdfa37), widened onto the
        // second caixa-core-internal peer
        // [`caixa_core::CaixaDialeto`] (c884517), and extended onto
        // the first M3-mesh-primitive-defining slot enum
        ...
        // `let key: std::rc::Rc<str> = verdict.into();`-shaped call
        // site reaches through this impl and no other — a paired
        // `std::rc::Rc::<str>::from(verdict.as_str())` open-code has
        // no compile-time link back to the substrate primitive, and a
        // two-step `std::rc::Rc::<str>::from(String::from(verdict))`
        // composition through the owned-`String` axis allocates twice
        // (once into the intermediate `String`, once into the
        ...
        // primitive [`super::ArchVerdict::as_str`] accessor trips at
        // caixa-arch test time.
        for &variant in super::ArchVerdict::ALL {
            let via_trait: std::rc::Rc<str> =
                <std::rc::Rc<str> as From<super::ArchVerdict>>::from(variant);
            let via_method: &'static str = variant.as_str();
            assert_eq!(
                via_trait.as_ref(),
                via_method,
                "From<ArchVerdict> for std::rc::Rc<str> impl must \
                 round-trip ArchVerdict::{variant:?} to the same \
                 canonical-lowercase byte-string ArchVerdict::as_str \
                 returns — divergence signals a silent detour off the \
                 substrate-primitive accessor"
            ...
            assert_eq!(
                via_into.as_ref(),
                via_method,
                "Into<std::rc::Rc<str>>::into on ArchVerdict::\
                 {variant:?} must byte-equal ArchVerdict::as_str on \
                 the same input — the blanket-derived Into shape must \
                 resolve to the same as_str dispatch as the explicit \
                 From impl"
            ...
            assert_eq!(
                via_trait.as_ref(),
                owned_static,
                "From<ArchVerdict> for std::rc::Rc<str> and \
                 From<ArchVerdict> for &'static str must resolve \
                 identically on ArchVerdict::{variant:?} — divergence \
            ...
            assert_eq!(
                ...
                owned_string.as_str(),
                "From<ArchVerdict> for std::rc::Rc<str> and \
                 From<ArchVerdict> for String must resolve identically \
            ...
            assert_eq!(
                ...
                owned_cow.as_ref(),
                "From<ArchVerdict> for std::rc::Rc<str> and \
                 From<ArchVerdict> for Cow<'static, str> must resolve \
                 identically on ArchVerdict::{variant:?} — divergence \
                 signals the owned-input std::rc::Rc<str> and \
            ...
            assert_eq!(
                ...
                "From<ArchVerdict> for std::rc::Rc<str> and \
                 ...
                 on the caixa-arch verdict-outcome axis)"
            );
        }
    }
    ...
    #[allow(
        ...
        reason = "cross-axis partition pin folds five borrowed-input \
                  ...
                  one exhaustive round-trip over ArchVerdict::ALL — the \
    ...
    fn arch_verdict_from_borrowed_into_rc_str_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<&ArchVerdict> for std::rc::Rc<str>` — asserts
        // the borrowed-input standard-library trait impl and the
        // substrate-primitive [`super::ArchVerdict::as_str`]
        // `pub const fn` accessor resolve to the same two-arm
        // canonical-lowercase emit-set across every arm the exhaustive
        ...
        // `let key: std::rc::Rc<str> = (&verdict).into();`-shaped call
        // site or a
        // `ArchVerdict::ALL.iter().map(std::rc::Rc::<str>::from)`-
        // shaped pipe reaches through this impl and no other — the
        // paired owned-input `From<ArchVerdict> for std::rc::Rc<str>`
        // impl forces every borrowed-input call site through an
        // explicit `Copy` deref
        // (`std::rc::Rc::<str>::from((*verdict).as_str())`) or a
        // `std::rc::Rc::<str>::from(verdict.as_str())` open-code whose
        ...
        // [`caixa_core::CaixaDialeto`] (c884517),
        // [`caixa_core::aplicacao::PlacementStrategy`] (30a1dee), and
        // [`crate::invariants::InvariantKind`] (13a4ad1) pairs walked
        // before it on the caixa-core-internal, M3-mesh-primitive-
        // defining, and first-mover outside-`caixa-core` tiers of the
        // same axis.
        //
        // Also byte-parity witness against the paired owned-input
        // [`From<ArchVerdict> for std::rc::Rc<str>`] and the sibling
        // borrowed-input [`From<&ArchVerdict> for &'static str`],
        // [`From<&ArchVerdict> for String`],
        // [`From<&ArchVerdict> for Cow<'static, str>`],
        // [`From<&ArchVerdict> for Box<str>`], and
        // [`From<&ArchVerdict> for std::sync::Arc<str>`] return-shape
        ...
        // over [`super::ArchVerdict::ALL`] — whose iterator yields
        // `&ArchVerdict` by construction, so the borrowed-input
        ...
        for &variant in super::ArchVerdict::ALL {
            ...
            assert_eq!(
                ...
                "From<&ArchVerdict> for std::rc::Rc<str> and \
                 ...
                 paths have drifted onto different emit-sets (the two \
                 disjoint reference-counted trait tables have \
                 desynchronized on the caixa-arch verdict-outcome axis \
                 under the borrowed-input corner)"
            );
        }
        let via_iter: Vec<std::rc::Rc<str>> = super::ArchVerdict::ALL
            .iter()
            .map(std::rc::Rc::<str>::from)
            .collect();
        let via_method: Vec<std::rc::Rc<str>> = super::ArchVerdict::ALL
            .iter()
            .map(|v| std::rc::Rc::<str>::from(v.as_str()))
            .collect();
        assert_eq!(
            via_iter, via_method,
            "`.iter().map(std::rc::Rc::<str>::from)` over \
             ArchVerdict::ALL — a call site whose iteration axis holds \
             `&ArchVerdict` by construction — must byte-equal \
             `.iter().map(|v| std::rc::Rc::<str>::from(v.as_str()))` \
             on every arm — the borrowed-input std::rc::Rc<str> \
             `From<&ArchVerdict> for std::rc::Rc<str>` axis is what \
             ...
             `From<ArchVerdict> for std::rc::Rc<str>` axis by first \
             calling `.copied()` on the iterator)"
        );
    }
    ...
    #[allow(
        ...
        reason = "cross-axis partition pin folds the substrate-primitive \
                  ...
                  into one exhaustive round-trip over ArchVerdict::ALL — \
    ...
    fn arch_verdict_as_ref_bytes_routes_through_as_str_accessor() {
        // `<T: AsRef<[u8]>>`-bound-consumer witness helper: a generic
        // byte-input function accepts a [`super::ArchVerdict`] directly
        // through the trait bound, without the caller open-coding the
        // two-hop `verdict.as_str().as_bytes()` composition. Lifted to
        ...
        // signature so a future per-ArchReport BLAKE3 content-address
        // closure that composes `hasher.update(verdict)` on the audit-
        // log closure builder reaches the substrate-primitive `as_str`
        // accessor through the [`super::ArchVerdict`] `AsRef<[u8]>` axis
        // and no other.
        ...
        for &variant in ArchVerdict::ALL {
            let via_trait: &[u8] = <ArchVerdict as AsRef<[u8]>>::as_ref(&variant);
            let via_method_bytes: &[u8] = variant.as_str().as_bytes();
            assert_eq!(
                via_trait, via_method_bytes,
                "AsRef<[u8]> for ArchVerdict impl must byte-equal \
                 ArchVerdict::as_str().as_bytes() on \
                 ArchVerdict::{variant:?} — divergence signals a silent \
                 detour off the substrate-primitive accessor"
            );
            // Cross-axis witness against the paired str-view axes'
            // `.as_bytes()` byte-tails.
            let via_as_ref_str: &str = <ArchVerdict as AsRef<str>>::as_ref(&variant);
            assert_eq!(
                via_trait,
                via_as_ref_str.as_bytes(),
                "AsRef<[u8]> for ArchVerdict and AsRef<str> for \
                 ArchVerdict `.as_bytes()` must byte-equal on \
                 ArchVerdict::{variant:?}"
            );
            ...
            assert_eq!(
                ...
                "AsRef<[u8]> for ArchVerdict and \
                 std::fmt::Display::fmt::<ArchVerdict> `.as_bytes()` \
                 must byte-equal on ArchVerdict::{variant:?}"
            );
            let via_as_str: &'static str = variant.as_str();
            assert_eq!(
                via_trait,
                via_as_str.as_bytes(),
                "AsRef<[u8]> for ArchVerdict and ArchVerdict::as_str \
                 `.as_bytes()` must byte-equal on ArchVerdict::{variant:?}"
            );
            ...
            assert_eq!(
                via_trait,
                via_static.as_bytes(),
                "AsRef<[u8]> for ArchVerdict and From<ArchVerdict> for \
    ...
    #[test]
    fn arch_verdict_try_from_bytes_routes_through_from_wire_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl TryFrom<&[u8]> for ArchVerdict` — asserts the trait-
        // idiomatic byte-view reverse-projection standard-library impl
        // and the substrate-primitive [`super::ArchVerdict::from_wire`]
        // `Option<Self>` accessor resolve to the same two-arm
        // canonical-lowercase verdict accept-set across every arm the
        // exhaustive [`super::ArchVerdict::ALL`] slice enumerates.
        // Extends the substrate-wide trait-idiomatic byte-view reverse-
        // projection axis onto the *second outside-`caixa-core`* closed-
        // set fieldless typed-enum peer — matching the paired
        // [`super::ArchVerdict`] byte-view forward triple
        // ([`AsRef<[u8]>`], [`From<ArchVerdict> for Vec<u8>`],
        // [`From<&ArchVerdict> for Vec<u8>`]) and the paired str-view
        // reverse-projection axis ([`TryFrom<&str> for ArchVerdict`])
        // already walked on this enum, and matching the trajectory the
        // first-mover outside-`caixa-core`
        // [`crate::invariants::InvariantKind`] `TryFrom<&[u8]>` impl
        // (40b7417) walked before it.
        //
        // Peer of the sibling
        ...
        // axis at the substrate-primitive [`super::ArchVerdict::from_wire`]
        // accessor so every future `<T: for<'a> TryFrom<&'a [u8]>>`-bound
        // consumer reaches the same two-arm verdict accept-set through
        // one trait dispatch.
        for &variant in ArchVerdict::ALL {
            let wire_bytes: &[u8] = variant.as_str().as_bytes();
            assert_eq!(
                <ArchVerdict as TryFrom<&[u8]>>::try_from(wire_bytes),
                Ok(variant),
                "TryFrom<&[u8]> impl on ArchVerdict must round-trip \
                 ArchVerdict::{variant:?}.as_str().as_bytes() back to \
                 Ok(ArchVerdict::{variant:?}) — divergence from \
                 ArchVerdict::from_wire signals a silent detour off the \
                 substrate-primitive accessor"
            );
            assert_eq!(
                <ArchVerdict as TryFrom<&[u8]>>::try_from(wire_bytes).ok(),
                ArchVerdict::from_wire(variant.as_str()),
                "TryFrom<&[u8]> ok()-projection on \
                 ArchVerdict::{variant:?}.as_str().as_bytes() must byte-\
                 equal ArchVerdict::from_wire on the paired &str input"
            );
            ...
            // reverse paths share one verdict accept-set through the
            // substrate-primitive `from_wire` accessor.
            let via_str: Result<ArchVerdict, ()> =
                <ArchVerdict as TryFrom<&str>>::try_from(variant.as_str());
            let via_bytes: Result<ArchVerdict, ()> =
                <ArchVerdict as TryFrom<&[u8]>>::try_from(wire_bytes);
            assert_eq!(
                via_bytes, via_str,
                "TryFrom<&[u8]> and TryFrom<&str> reverse-projection \
                 axes on ArchVerdict must agree on \
                 ArchVerdict::{variant:?} — divergence signals the \
            ...
            let via_asref: &[u8] = <ArchVerdict as AsRef<[u8]>>::as_ref(&variant);
            assert_eq!(
                <ArchVerdict as TryFrom<&[u8]>>::try_from(via_asref),
                Ok(variant),
            ...
            // [`From<ArchVerdict> for Vec<u8>`] owned byte-tail back
            ...
            let owned_bytes: Vec<u8> = <Vec<u8> as From<ArchVerdict>>::from(variant);
            assert_eq!(
                <ArchVerdict as TryFrom<&[u8]>>::try_from(owned_bytes.as_slice()),
                Ok(variant),
                "TryFrom<&[u8]> ∘ From<ArchVerdict> for Vec<u8> must \
                 round-trip ArchVerdict::{variant:?} — divergence \
            ...
            // discipline on the paired [`From<&ArchVerdict> for Vec<u8>`]
            ...
            // reverse impl.
            let borrowed_owned_bytes: Vec<u8> = <Vec<u8> as From<&ArchVerdict>>::from(&variant);
            assert_eq!(
                <ArchVerdict as TryFrom<&[u8]>>::try_from(borrowed_owned_bytes.as_slice()),
                Ok(variant),
                "TryFrom<&[u8]> ∘ From<&ArchVerdict> for Vec<u8> must \
                 round-trip ArchVerdict::{variant:?} — divergence \
    ...
    #[test]
    fn arch_verdict_try_from_bytes_rejects_unknown_and_non_utf8_bytes() {
        // Rejection witness on the `impl TryFrom<&[u8]> for ArchVerdict`
        ...
        // returns `Err` before [`super::ArchVerdict::from_wire`] runs)
        // and the valid-UTF-8-but-unknown-wire rejection path
        // ([`super::ArchVerdict::from_wire`] returns `None` on a byte-
        // string outside the two-arm verdict accept-set). Both must
        // reject, so a future accidental widening of the trait impl's
        // accept-set (a case-fold path, a silent inclusion of the pre-
        // lift PascalCase Debug-derived shapes `"Proven"` / `"Rejected"`
        // on the wire axis, a silent overlap with the peer
        // [`crate::invariants::InvariantKind`] three-arm severity axis's
        // non-shared canonical tags `b"safety"` / `b"compliance"` /
        // `b"hint"`, a stray fallback that maps invalid UTF-8 onto a
        // default arm rather than the trait-idiomatic `Err(())`) trips
        // at caixa-arch test time.
        //
        ...
        for &input in non_utf8_rejected {
            assert_eq!(
                <ArchVerdict as TryFrom<&[u8]>>::try_from(input),
                Err(()),
                "TryFrom<&[u8]> impl on ArchVerdict must reject the \
        ...
        // sibling `arch_verdict_try_from_str_rejects_unknown_byte_strings`
        // str-view rejection witness already pins on the paired
        // [`TryFrom<&str>`] axis: the empty byte-string, whitespace-only
        // padding, PascalCase / uppercase rebrand candidates,
        // Levenshtein-neighbor typos, the peer
        // [`crate::invariants::InvariantKind`] three-arm severity axis's
        // non-shared canonical tags (`b"safety"` / `b"compliance"` /
        // `b"hint"`), sibling closed-set-enum canonical tags on other
        // axes (`b"biblioteca"`, `b"servico"`, `b"one-for-one"`,
        ...
        for &input in unknown_wire_rejected {
            assert_eq!(
                ...
                "TryFrom<&[u8]> impl on ArchVerdict must reject the \
                 ...
                 set widening off the paired ArchVerdict::from_wire \
