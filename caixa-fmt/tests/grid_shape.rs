...
fn fmt(src: &str) -> String {
    ...
    for l in a.lines() { assert!(l.chars().count() <= 80, "over 80:\n{a}"); }
    assert!(!a.lines().any(|l| l.ends_with(' ')), "trailing whitespace:\n{a}");
    a
}
...
fn shapes() {
    for (label, src) in [
        ("data table", r#"[("us-east-1" 3 "healthy") ("us-west-2" 12 "degraded") ("eu-west-1" 1 "healthy")]"#),
