...
mod tests {
        ...
        let paths: Vec<&str> = to_ports
            ...
            .filter_map(|tp| {
                tp.get(KUBE_KEY_RULES)
                    .and_then(|r| kube_seq_first(r, CILIUM_KEY_HTTP))
                    .and_then(|rule| kube_str(rule, CILIUM_KEY_PATH))
            })
            .collect();
        assert!(
    ...
    fn httproute_retry_attempts_serialized_as_yaml_number() {
        ...
        let rules = httproute_rules(&docs);
        for rule in &rules {
            let attempts = kube_field_field(rule, GATEWAY_API_KEY_RETRY, GATEWAY_API_KEY_ATTEMPTS)
    ...
    fn cnp_ingress_rules(docs: &[serde_yaml::Value]) -> Vec<serde_yaml::Value> {
        docs.iter()
            ...
            .filter_map(|d| kube_spec_seq_first(d, CILIUM_KEY_INGRESS).cloned())
            .collect()
    }
    ...
    fn cnp_authentication_mode_serialized_as_yaml_string() {
        ...
        let rules = cnp_ingress_rules(&policies);
        for rule in &rules {
            let mode = kube_field_field(rule, CILIUM_KEY_AUTHENTICATION, CILIUM_KEY_MODE)
