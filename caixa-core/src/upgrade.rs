...
impl UpgradeError {
    ...
    /// `Self::PurgeWithoutPriorLoad { from: from.to_string(), kind,
    /// module: module.to_string() }` three-field struct-literal onto one
    ...
    /// [`UpgradeError::duplicate_from`] on the paired one-slot `{ from:
    /// String }` envelope on the same `UpgradeError` envelope, and of
    ...
    /// Closes the last unlifted `{ from: String, kind: &'static str,
    /// module: String }` three-slot open-coded struct-literal wire-up
...
mod tests {
    ...
    // the standalone [`UpgradeError::purge_without_prior_load`] inherent
    // ctor (see the paired doc-block above the ctor definition) — the
    // fold of the last open-coded three-slot `{ from: String, kind:
    // &'static str, module: String }` struct-literal wire-up on
    // [`UpgradeError`] closes the sole in-crate wire-up site inside
    // [`UpgradeFromEntry::validate_purge_ordering`]'s per-instruction
    // load-family sticky-latch dispatch onto one substrate primitive.
    // A byte-mismatched ctor body would trip the equivalence pin first,
    // ahead of any downstream diagnostic-shape drift.
    //
    // Peer of the sibling standalone-ctor equivalence + routing pins on
    // the sibling one-off variants across `UpgradeError`
    // (`duplicate_from_ctor_matches_struct_literal_wrap` /
    // `duplicate_from_ctor_routes_prior_versao_through_verbatim` /
    // `duplicate_from_ctor_projects_prior_versao_scalar_accessor` on
    // the paired one-slot `{ from: String }` envelope) and across
    // caixa-core (`contrato_endpoint_not_absolute_ctor_matches_struct_
    // literal_wrap` on the paired three-slot `{ de, para, endpoint:
    // String }` `AplicacaoError` envelope).

    #[test]
    fn purge_without_prior_load_ctor_matches_struct_literal_wrap() {
        // Equivalence pin: the ctor produces byte-equal
        // `UpgradeError::PurgeWithoutPriorLoad` to the pre-lift
        // open-coded three-field struct-literal on the same `(&str,
        // &'static str, &str)` fixture. Guards any future field-
        // addition / reordering / string-conversion tweak on the
        // variant. Same equivalence-pin shape as the sibling
        // `duplicate_from_ctor_matches_struct_literal_wrap` (7e52aec)
        // on the peer one-slot `{ from: String }` envelope.
        let from = "0.1.0";
    ...
    fn purge_without_prior_load_ctor_routes_from_kind_and_module_through_verbatim() {
        // Cross-axis routing pin: sweep the three constructor input
        // axes (`from: &str`, `kind: &'static str`, `module: &str`)
        // through distinct-per-axis fixtures across every cleanup-family
        // [`UpgradeInstruction::lisp_form`] variant + a boundary mix of
        // SemVer-2 `from` shapes (pre-release, build-metadata) + DNS-1123
        // module shapes (leaf, hyphenated, deeply-hyphenated) so any
        // wrapper-side lowercase / trim / truncate / silent axis-swap
        // (`from` ↔ `module`, `kind` misrouted onto `from`) on the
        // three-field construction surfaces at assert time rather than
        // at a downstream diagnostic consumer that reads the fields
        // back and gets a different value than the one it stored. Both
        // `&str`-literal and `&String` (via Deref coercion) carriers
        // are exercised for `from` / `module` because the sole wire-up
        // hands `self.prior_versao()` (a `&str` accessor) and
        // `instr.declared_module().expect(…)` (also a `&str`) — the
        // ctor must accept both shapes without a pre-conversion.
    ...
    fn validate_purge_ordering_arm_routes_through_purge_without_prior_load_ctor() {
        ...
        // `:purge`) before any `:load-module` so
        // [`UpgradeFromEntry::validate_purge_ordering`]'s load-family
        // sticky-latch dispatch surfaces
        // `UpgradeError::PurgeWithoutPriorLoad`, then pin that the
        // observed `Err` byte-equals the substrate-primitive
        // [`UpgradeError::purge_without_prior_load`] ctor's output on
        // the same fixture. A future silent de-lift of the wire-up back
        // to the open-coded struct-literal (or a silent axis-swap on
        // the three-field construction at the wire-up site) trips at
        ...
        // `validate_upgrade_from_duplicate_diagnostic_arm_routes_through_duplicate_from_ctor`
        // on the peer cross-entry duplicate-`:from` gate; both key off
        // exactly one typed dispatch on the substrate primitive.
