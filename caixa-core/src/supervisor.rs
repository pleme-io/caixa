...
/// projection on the M2 OTP-shape sibling-restart-strategy closed-set
/// typed enum — the fourth (and closing) corner of the
/// `{Self, &Self} × {&'static str, String}` 2×2 trait-idiomatic
/// projection family. Routes byte-for-byte through the
/// substrate-primitive [`RestartStrategy::as_str`] `pub const fn`
/// accessor (via [`str::to_owned`]) so every consumer that holds a
/// borrowed [`&RestartStrategy`] and needs an owned [`String`] — a
/// future `serde_json::Value::String(String::from(&strategy))`
/// structured-payload composer over a borrowed field, a future
/// `Iterator::map` over `&[RestartStrategy]` that projects to owned
/// keys through `.iter().map(String::from)`, a future
/// `HashMap::<String, RestartStrategy>::from_iter` that keys off a
/// borrowed-iteration axis where dereferencing the strategy would force
/// an unnecessary `Copy` at every step, the future wasm-operator's
/// per-supervisor `strategies.iter().map(String::from).collect()`
/// diagnostic emit whose iteration axis is borrowed by construction —
/// reaches the same four-arm lifted
/// [`crate::render::SUPERVISOR_ESTRATEGIA_ONE_FOR_ONE`] /
/// [`crate::render::SUPERVISOR_ESTRATEGIA_ONE_FOR_ALL`] /
/// [`crate::render::SUPERVISOR_ESTRATEGIA_REST_FOR_ONE`] /
/// [`crate::render::SUPERVISOR_ESTRATEGIA_SIMPLE_ONE_FOR_ONE`] const the
/// paired [`std::fmt::Display`], [`AsRef<str>`],
/// [`RestartStrategy::as_str`], and the three other trait-idiomatic
/// forward-projection impls
/// ([`From<RestartStrategy> for &'static str`],
/// [`From<&RestartStrategy> for &'static str`],
/// [`From<RestartStrategy> for String`]) already return.
///
/// Opens the trait-idiomatic *borrowed-input, owned-`String` output*
/// forward-projection axis on closed-set fieldless typed enums —
/// first-mover on the 2×2 completion corner, mirror of the
/// [`crate::supervisor::RestartStrategy`] first-mover position that
/// opened the paired owned-input owned-`String` axis (7baa18a), the
/// owned-input owned-`&'static str` axis (523157d), and the paired
/// [`crate::dep::DepList`] first-mover position that opened the
/// borrowed-input `&'static str` axis (64aa742). Rust's standard
/// library does not carry a blanket `impl<T: AsRef<str>> From<&T> for
/// String` (nor an `impl<T: fmt::Display> From<&T> for String`), so
/// every closed-set typed enum that carries the paired `AsRef<str>` /
/// `Display` / `From<Self> for &'static str` / `From<&Self> for
/// &'static str` / `From<Self> for String` quintuple but not the
/// borrowed-input owned-[`String`] axis forces every borrowed-input
/// owned-string call site through a `strategy.as_str().to_owned()` /
/// `String::from(*strategy)` (with a spurious `Copy`) /
/// `strategy.to_string()` (through `Display`) detour whose type bounds
...
/// and the diagnostic byte-string share the same vocabulary by
/// construction (unlike the sibling [`crate::CaixaKind`] enum whose two
/// axes diverge), so the borrowed-input owned-[`String`] projection
/// lands byte-identically on both the wire vocabulary the paired
/// [`serde::Serialize`] derive emits and the diagnostic vocabulary the
/// [`RestartStrategy::as_str`] helper returns.
///
/// The remaining fourteen closed-set typed enums on the caixa
...
/// `PathShapeViolation`, `InvariantKind`, `ArchVerdict`, `Severity`,
/// `FixSafety`, `Semantic`, `FerriteRuntime`) are the future targets of
/// this 2×2-completion campaign — each carries the same paired
/// quintuple that this borrowed-input owned-[`String`] axis extends onto.
///
/// Pinned load-bearing by
/// [`tests::restart_strategy_from_into_borrowed_owned_string_routes_through_as_str_accessor`]
/// (byte-parity pin against [`RestartStrategy::as_str`] across the
/// four-arm emit-set through the borrowed-input surface) and
/// [`tests::restart_strategy_from_into_borrowed_owned_string_agrees_with_paired_axes_on_every_arm`]
/// (cross-axis partition pin against the paired owned-input owned-
/// [`String`] [`From<RestartStrategy> for String`] impl, the paired
/// borrowed-input owned-[`&'static str`] [`From<&RestartStrategy> for
/// &'static str`] impl, and the sibling [`ToString::to_string`] surface
/// routed through [`std::fmt::Display`], plus a direct round-trip
/// witness through [`TryFrom<&str>`] on the owned-[`String`]'s
...
/// [`RestartStrategy`] closed-set typed enum — extends the substrate-
/// wide [`std::borrow::Cow<'static, str>`] forward-projection family
/// opened on [`crate::CaixaKind`] (99c1735) onto the first M2 OTP-
/// shape closed-set fieldless typed enum peer on the caixa surface
/// (`:supervisor :estrategia`). Routes byte-for-byte through the
/// substrate-primitive [`RestartStrategy::as_str`] `pub const fn`
/// accessor (via [`std::borrow::Cow::Borrowed`]) so every consumer
/// that binds a [`RestartStrategy`] through the trait-idiomatic
/// [`std::borrow::Cow<'static, str>`] axis — a future
/// `axum::response::IntoResponse` composer whose per-strategy
/// diagnostic-body typing rules out the sibling [`AsRef<str>`]
/// borrowed return, a future M4 admission-webhook rejection body
/// that composes the accepted-strategy enumeration through the same
/// `RestartStrategy::ALL.iter().map(Cow::from)` shape [`CaixaKind`]
/// already routes through, a generic `<T: for<'a>
/// Into<std::borrow::Cow<'static, str>>>`-bound structured-log
/// emitter on a per-supervisor diagnostic column — reaches the same
/// four-arm lifted [`crate::render::SUPERVISOR_ESTRATEGIA_ONE_FOR_ONE`]
/// / [`crate::render::SUPERVISOR_ESTRATEGIA_ONE_FOR_ALL`] /
/// [`crate::render::SUPERVISOR_ESTRATEGIA_REST_FOR_ONE`] /
/// [`crate::render::SUPERVISOR_ESTRATEGIA_SIMPLE_ONE_FOR_ONE`] const
/// the paired [`std::fmt::Display`], [`AsRef<str>`],
/// [`RestartStrategy::as_str`], and the four
/// `{Self, &Self} × {&'static str, String}` 2×2 trait-idiomatic
...
/// [`From<RestartStrategy> for &'static str`],
/// [`From<RestartStrategy> for String`], [`AsRef<str>`], and
...
/// this lift.
///
/// First peer to extend the substrate-wide trait-idiomatic
/// [`std::borrow::Cow<'static, str>`] forward-projection axis off the
/// top-level [`crate::CaixaKind`] enum (99c1735 owned-input,
/// d45c409 borrowed-input) onto the wider substrate — the remaining
/// twelve peers (`RestartPolicy`, `PlacementStrategy`, `RateLimitUnit`,
/// `DepList`, `CaixaDialeto`, and the outside-`caixa-core` peers
/// `WitShape`, `PathShapeViolation`, `InvariantKind`, `ArchVerdict`,
/// `Severity`, `FixSafety`, `Semantic`, `FerriteRuntime`) are the
/// future targets of this campaign.
///
/// Pinned load-bearing by
/// [`tests::restart_strategy_from_into_static_cow_str_routes_through_as_str_accessor`]
/// (byte-parity + zero-alloc [`std::borrow::Cow::Borrowed`]-arm pin
/// against [`RestartStrategy::as_str`] across the four-arm
/// [`RestartStrategy::ALL`]) and
/// [`tests::restart_strategy_from_into_static_cow_str_agrees_with_paired_axes_on_every_arm`]
/// (cross-axis partition pin against the paired [`From<RestartStrategy>
/// for &'static str`], [`From<RestartStrategy> for String`], and
/// [`ToString`]-through-[`std::fmt::Display`] axes, plus a
/// `.iter().copied().map(Cow::from)` pipe witness over
/// [`RestartStrategy::ALL`] that materializes the four-arm accept-set
...
/// projection on the M2 OTP-shape sibling-restart [`RestartStrategy`]
/// closed-set fieldless typed enum — closes the `{Self, &Self}`
/// input-shape corner of the substrate-wide `Box<str>`
/// forward-projection axis opened one commit prior (69ef45c) on the
/// paired owned-input [`From<RestartStrategy> for Box<str>`] impl.
...
/// consumer that holds a `&RestartStrategy` and needs a
/// [`Box<str>`] — a
/// `RestartStrategy::ALL.iter().map(Box::<str>::from).collect::<Vec<_>>()`
/// per-arm accept-set materializer (whose iterator over
/// `&'static [RestartStrategy]` yields `&RestartStrategy`, not
/// `RestartStrategy`, so the paired owned-input
/// [`From<RestartStrategy> for Box<str>`] axis alone forces every
/// call site through an explicit `.copied()` / dereference /
/// [`Copy`]-bound restatement rather than the direct trait-idiomatic
/// projection), a per-supervisor metric-key materializer holding
/// `&RestartStrategy` through a `caixa-operator` reconciliation
/// scheduler's borrow lifetime, a future admission-webhook rejection
/// body whose per-arm `Box<str>` field composes from a borrowed
/// `&RestartStrategy` handle without a spurious [`Copy`] deref —
/// reaches the same four-arm lifted
/// [`crate::render::SUPERVISOR_ESTRATEGIA_ONE_FOR_ONE`] /
/// [`crate::render::SUPERVISOR_ESTRATEGIA_ONE_FOR_ALL`] /
/// [`crate::render::SUPERVISOR_ESTRATEGIA_REST_FOR_ONE`] /
/// [`crate::render::SUPERVISOR_ESTRATEGIA_SIMPLE_ONE_FOR_ONE`] const
/// the paired owned-input [`From<RestartStrategy> for Box<str>`] and
/// the sibling
/// `{Self, &Self} × {&'static str, String, Cow<'static, str>}`
/// forward-projection corner already return.
///
/// Second peer on the substrate-wide trait-idiomatic
/// [`Box<str>`] forward-projection family opened one commit prior
/// (69ef45c) on the paired owned-input
/// [`From<RestartStrategy> for Box<str>`] impl — closes the
/// `{Self, &Self}` input-shape corner of the [`Box<str>`] axis on
/// the first M2 OTP-shape closed-set fieldless typed enum peer on
/// the caixa surface (`:supervisor :estrategia`), exactly as
/// ee577fd closed the paired [`Cow<'static, str>`] axis one commit
/// after its owning half (7dd28b3) landed. Rust's standard library
...
/// carries the paired owned-input `Box<str>` axis but not the
/// borrowed-input axis forces every borrowed-input
/// `Box<str>`-parameterized call site through a spurious [`Copy`]
/// deref (`Box::<str>::from((*strategy).as_str())`) or a
/// `Box::<str>::from(strategy.as_str())` open-code whose type bounds
/// have no compile-time link back to the substrate primitive.
///
/// Pinned load-bearing by
/// [`tests::restart_strategy_from_borrowed_into_box_str_routes_through_as_str_accessor`]
/// (byte-parity pin against [`RestartStrategy::as_str`] across the
/// four-arm [`RestartStrategy::ALL`] emit-set on the borrowed-input
/// surface, plus a blanket-derived [`Into`] shape witness and a
/// cross-axis pin against the paired owned-input
/// [`From<RestartStrategy> for Box<str>`] and the sibling
...
/// forward projection on the M2 OTP-shape sibling-restart
/// [`RestartStrategy`] closed-set fieldless typed enum — closes the
/// `{Self, &Self}` input-shape corner of the [`std::sync::Arc<str>`]
/// forward-projection axis on the first M2 OTP-shape closed-set
/// fieldless typed enum peer on the caixa surface
/// (`:supervisor :estrategia`), companion to the paired owned-input
/// [`From<RestartStrategy> for std::sync::Arc<str>`] impl one commit
/// prior (bca2ec8). Routes byte-for-byte through the
/// substrate-primitive [`RestartStrategy::as_str`] `pub const fn`
/// accessor (via [`std::sync::Arc::<str>::from`] on the returned
/// `&'static str`), so every consumer that binds a
/// [`&RestartStrategy`] through the standard-library `.into()` /
...
/// per-request borrowed-`&RestartStrategy` handle rendering a per-arm
/// `Sync` + `Send`-safe structured-log field across an `.await`
/// boundary through a `<T: Into<std::sync::Arc<str>>>`-bound
/// diagnostic-column dispatch, a future wasm-operator's per-
/// supervisor reconciliation pipeline whose
/// `.iter().map(std::sync::Arc::<str>::from)` collector reaches into
/// the shared-ownership per-strategy key without a spurious [`Copy`]
/// deref (which would only be reachable through the owned-input
/// [`From<RestartStrategy> for std::sync::Arc<str>`] axis by first
/// calling `.copied()` on the iterator), a future
/// `<T: Into<std::sync::Arc<str>>>`-bound `tracing`-span attributes
/// collector recording a borrowed-`&RestartStrategy` per-arm field
...
/// [`crate::render::SUPERVISOR_ESTRATEGIA_REST_FOR_ONE`] /
/// [`crate::render::SUPERVISOR_ESTRATEGIA_SIMPLE_ONE_FOR_ONE`] const
/// the paired owned-input
/// [`From<RestartStrategy> for std::sync::Arc<str>`] impl and the
/// sibling `{&'static str, String, Cow<'static, str>, Box<str>}`
/// forward-projection corner already return.
///
/// Second peer on the substrate-wide trait-idiomatic
/// [`std::sync::Arc<str>`] forward-projection family opened one
/// commit prior (bca2ec8) on the paired owned-input
/// [`From<RestartStrategy> for std::sync::Arc<str>`] impl — closes
/// the `{Self, &Self}` input-shape corner of the
/// [`std::sync::Arc<str>`] axis on the first M2 OTP-shape closed-set
/// fieldless typed enum peer on the caixa surface, exactly as
/// 59ae5dc closed the paired [`Box<str>`] axis one commit after its
/// owning half (69ef45c) landed. Rust's standard library carries
...
/// `Copy`-based `impl<T: Copy, U: From<T>> From<&T> for U`), so
/// every closed-set fieldless typed enum peer on the substrate that
/// carries the paired owned-input [`std::sync::Arc<str>`] axis but
/// not the borrowed-input axis forces every borrowed-input
/// [`std::sync::Arc<str>`]-parameterized call site through a
/// spurious [`Copy`] deref
/// (`std::sync::Arc::<str>::from((*strategy).as_str())`) or a
/// `std::sync::Arc::<str>::from(strategy.as_str())` open-code whose
/// type bounds have no compile-time link back to the substrate
/// primitive.
///
/// Pinned load-bearing by
/// [`tests::restart_strategy_from_borrowed_into_arc_str_routes_through_as_str_accessor`]
/// (byte-parity pin against [`RestartStrategy::as_str`] across the
/// four-arm [`RestartStrategy::ALL`] emit-set on the borrowed-input
/// surface, plus a blanket-derived [`Into`] shape witness and a
/// cross-axis pin against the paired owned-input
/// [`From<RestartStrategy> for std::sync::Arc<str>`] and the sibling
/// borrowed-input `{&'static str, String, Cow<'static, str>,
/// Box<str>}` return-shape axes).
impl From<&RestartStrategy> for std::sync::Arc<str> {
    fn from(strategy: &RestartStrategy) -> std::sync::Arc<str> {
        std::sync::Arc::<str>::from(strategy.as_str())
    }
...
/// reverse projection on the first M2 OTP-shape closed-set fieldless
/// typed enum peer on the caixa surface ([`RestartStrategy`]) — the
/// byte-mirror of the [`From<RestartStrategy> for String`] str-owned
/// reverse-projection axis and the owned-`Vec<u8>` reverse-projection
/// sibling of the paired [`AsRef<[u8]>`] borrowed byte-view axis
/// (cd4c4e0) lifted on this same enum. Routes byte-for-byte through
/// the substrate-primitive [`RestartStrategy::as_str`] `pub const fn`
/// accessor via [`str::as_bytes`] + [`slice::to_vec`] so every
/// consumer that binds a [`RestartStrategy`] through the standard-
/// library `impl From<RestartStrategy> for Vec<u8>` axis
/// (equivalently `<T: Into<Vec<u8>>>`) — a future
/// [`std::io::Write::write_all`]-shape per-supervisor audit-log byte-
/// sink whose input parameter is an owned [`Vec<u8>`] payload, a
/// future `bytes::Bytes::from(Vec::<u8>::from(strategy))` composer
/// folding the per-arm sibling-restart-topology byte-tag into the
/// [`bytes::Bytes`] framing surface, a future
/// `hasher.update(&Vec::<u8>::from(strategy))`-shape BLAKE3 content-
/// address closure that needs the owned byte-tail buffered before
/// folding into the [`crate::Lacre`] closure body, a future per-
/// strategy protobuf/CBOR/msgpack payload composer whose framer takes
...
/// open-coded per-call-site `strategy.as_str().as_bytes().to_vec()`
...
/// projection axis onto the first M2-OTP-shape closed-set fieldless
/// typed-enum peer, matching the trajectory the first-mover
/// [`crate::CaixaKind`] `From<{Self, &Self}> for Vec<u8>` lift
/// (b245fd6), the second-mover [`crate::dialeto::CaixaDialeto`] lift
/// (4cceaf5), and the third-mover [`crate::dep::DepList`] lift
/// (e974ca2) established across the caixa-core-internal tier. Every
/// future arm addition (an OTP-`rest_for_all` fifth arm the theory
/// [`ABSORPTION-ROADMAP`](https://github.com/pleme-io/theory/blob/main/ABSORPTION-ROADMAP.md)
/// might reach for once the four canonical OTP strategies stop
/// covering the substrate's discovered load-shape) grows the byte-
/// owned axis through one edit on the substrate-primitive
/// [`RestartStrategy::as_str`] accessor, mirroring the discipline the
/// paired [`AsRef<[u8]>`] borrowed byte-view axis campaign already
/// tracked across every closed-set fieldless typed enum peer on the
/// substrate.
///
/// Pinned load-bearing by
/// [`tests::restart_strategy_from_into_owned_vec_bytes_routes_through_as_str_accessor`]
/// (byte-parity pin against [`RestartStrategy::as_str`] across the
/// four-arm [`RestartStrategy::ALL`] emit-set binding the byte-owned
...
impl From<RestartStrategy> for Vec<u8> {
    fn from(strategy: RestartStrategy) -> Vec<u8> {
        strategy.as_str().as_bytes().to_vec()
    }
}

/// Trait-idiomatic *borrowed-input, owned-`Vec<u8>` output* byte-
/// owned reverse projection on the first M2 OTP-shape closed-set
/// fieldless typed enum peer on the caixa surface
/// ([`RestartStrategy`]) — the borrowed-input peer of
/// [`From<RestartStrategy> for Vec<u8>`], closing the
/// `{Self, &Self} → Vec<u8>` pair on the byte-owned reverse-projection
/// axis in one lift. Routes byte-for-byte through the substrate-
/// primitive [`RestartStrategy::as_str`] `pub const fn` accessor so
/// every consumer that holds a borrowed [`&RestartStrategy`] and
/// needs an owned [`Vec<u8>`] — a future
/// `.iter().map(Vec::<u8>::from).collect()` pipe over
/// `&[RestartStrategy]` (whose iterator yields `&RestartStrategy`,
/// not `RestartStrategy`, so the owned-input axis alone forces every
...
/// that walks [`RestartStrategy::ALL`] through an `Into<Vec<u8>>`-
/// bound per-arm byte-writer to surface the accepted `:estrategia`
/// set — reaches the substrate primitive through one trait dispatch
/// rather than a `Vec::<u8>::from(*strategy)` spurious-`Copy`-deref
/// restatement.
impl From<&RestartStrategy> for Vec<u8> {
    fn from(strategy: &RestartStrategy) -> Vec<u8> {
        strategy.as_str().as_bytes().to_vec()
...
/// M2-OTP-shape supervisor-slot closed-set fieldless typed enum peer on the
/// caixa surface ([`RestartStrategy`]) — the owned-input peer of
/// [`TryFrom<&[u8]> for RestartStrategy`], mirroring the closed
/// [`From<RestartStrategy> for Vec<u8>`] + [`From<&RestartStrategy> for
/// Vec<u8>`] byte-owned *forward*-projection pair on this same enum onto
/// the byte-owned *reverse*-projection axis. Routes byte-for-byte through
/// [`<Self as TryFrom<&[u8]>>::try_from`] on the [`Vec<u8>::as_slice`]
/// borrow, so the owned-input surface reaches the same
/// [`std::str::from_utf8`] + [`RestartStrategy::from_wire`] resolution
...
/// [`RestartStrategy`] otherwise picks between (a) an open-coded
/// `<RestartStrategy as TryFrom<&[u8]>>::try_from(bytes.as_slice())` at
/// every call site (whose type bounds have no compile-time link to the
/// byte-owned reverse-projection axis), (b) a two-hop
/// `String::from_utf8(bytes)` + [`RestartStrategy::from_wire`] composition
...
/// axis at the substrate-primitive [`RestartStrategy::from_wire`] accessor
/// so every future `<T: TryFrom<Vec<u8>>>`-bound owned-byte-vec consumer —
/// a future M4 `mesh.pleme.io/v1alpha1/Supervisor` CR admission-webhook
/// body reader that hands the `spec.estrategia` byte-tail off as a
/// [`Vec<u8>`] before UTF-8 validation commits allocation, a
/// `bytes::Bytes::to_vec()`-shape wire-body composer walking a prior
/// audit's per-supervisor rejection payload back to the typed enum, a
/// `std::io::Read::read_to_end`-shape audit-log source whose framing
/// yields an owned byte-vec per per-strategy scalar, an
...
/// ([`crate::dep::DepList`], commit 42091cb) — onto the first M2-OTP-shape
/// supervisor-slot closed-set fieldless typed enum peer, tracking the
...
/// fieldless typed enum peer on the substrate ([`RestartPolicy`],
/// [`crate::aplicacao::PlacementStrategy`],
/// [`crate::aplicacao::RateLimitUnit`], [`crate::aplicacao::WitShape`],
/// [`crate::render::PathShapeViolation`], and the outside-`caixa-core`
/// peers `InvariantKind`, `ArchVerdict`, `Severity`, `FixSafety`,
...
/// RestartStrategy`] unit-error shape, preserving the trait-family
...
/// in the error deliberately declined — a caller that needs the bytes
/// back holds a clone before the call, and the closed-set-enum use site
/// rarely wants the raw bytes back past a "did you mean" diagnostic that
/// operates on the wire vocabulary rather than the input).
///
/// Pinned load-bearing by
/// [`tests::restart_strategy_try_from_vec_bytes_routes_through_borrowed_byte_view_axis`]
...
/// substrate-primitive [`RestartStrategy::from_wire`] accessor, and a
/// four-corner {owned-input, borrowed-input} × {`From<Self>` → `Vec<u8>`,
/// `From<&Self>` → `Vec<u8>`} round-trip witness available on this enum
/// because [`RestartStrategy::as_str`] and [`RestartStrategy::from_wire`]
...
/// [`tests::restart_strategy_try_from_vec_bytes_rejects_unknown_and_non_utf8_bytes`]
/// (rejection witness against silent accept-set widening on both the
/// non-UTF-8 byte-sequence rejection path and the unknown-wire-vocabulary
/// rejection path — the latter includes the sibling kebab-case
/// dispatcher-catalog byte-strings on the two-axis wire/catalog split so
...
impl TryFrom<Vec<u8>> for RestartStrategy {
    type Error = ();
...
/// string-owned reverse projection on the first M2-OTP-shape supervisor-slot
/// closed-set fieldless typed enum peer on the caixa surface
/// ([`RestartStrategy`]) — the owned-input peer of the paired
/// [`TryFrom<&str> for RestartStrategy`] str-view reverse-projection axis, and
/// the string-owned reverse companion of the pre-existing string-owned
/// *forward* pair ([`From<RestartStrategy> for String`],
/// [`From<&RestartStrategy> for String`]) already lifted on this same enum.
/// Routes owned [`String`] input through the paired borrowed-input
/// [`TryFrom<&str>`] axis via [`String::as_str`] so every consumer that holds
/// an owned `String` — a future M4 `mesh.pleme.io/v1alpha1/Supervisor` CR
/// admission-webhook body reader that hands the `spec.estrategia`
/// `PascalCase` scalar off as an owned [`String`] after UTF-8 validation, a
/// `serde_yaml::from_str` / `serde_json::from_str` de-serialize round-trip
/// whose composer surfaces the `:estrategia` scalar as an owned [`String`]
/// typed field, a `feira supervisor --estrategia <OneForOne|OneForAll|
/// RestForOne|SimpleOneForOne>` `clap`-derived arg-parse whose owned-`String`
/// positional lands the canonical arm at the typed dispatch, a
/// per-`:supervisor`-slot overlay resolver reading an owned [`String`] out of
/// a `ConfigMap` `data.supervisor-estrategia` scalar, an
/// `<T: TryFrom<String>>`-bound generic loader over any of the substrate's
/// closed-set typed enums — reaches the same four-arm `PascalCase` accept-set
...
/// (a2e6f02), lifted onto the first closed-set fieldless typed-enum peer
/// [`crate::aplicacao::WitShape`] (e6aac29), extended onto the second closed-
/// set fieldless typed-enum peer [`crate::aplicacao::RateLimitUnit`]
/// (94a9c5e), extended onto the third closed-set fieldless typed-enum peer
/// [`crate::aplicacao::PlacementStrategy`] (d81a70a) — onto the first
/// M2-OTP-shape supervisor-slot closed-set fieldless typed-enum peer, the
/// per-`:supervisor` sibling-restart-strategy discriminator. The peers
/// [`RestartPolicy`], [`crate::CaixaKind`], [`crate::CaixaDialeto`], and
/// [`crate::dep::DepList`] remain the next targets of the campaign, mirroring
/// the trajectory the closed byte-view / byte-owned reverse-projection
/// families already walked across the same closed-set peers.
///
/// Rust's standard library carries no blanket
/// `impl<T: for<'a> TryFrom<&'a str>> TryFrom<String> for T`, so a consumer
/// that holds an owned [`String`] and needs a typed [`RestartStrategy`]
/// otherwise picks between (a) an open-coded
/// `<RestartStrategy as TryFrom<&str>>::try_from(s.as_str())` at every call
/// site whose type bounds have no compile-time link back to the string-owned
/// reverse-projection axis, (b) a `let s: &str = &s;
/// RestartStrategy::try_from(s)` two-step whose borrow arithmetic leaks a
/// per-call-site lifetime dance rather than a single trait dispatch, or (c) a
/// `String::into_bytes` + [`TryFrom<Vec<u8>>`] detour that reaches the
/// substrate-primitive `from_wire` accessor through a UTF-8 re-validation hop
/// the owned-`String` axis already knows to skip. This impl closes the
/// string-owned reverse-projection axis at the substrate-primitive
/// [`RestartStrategy::from_wire`] accessor so every future
/// `<T: TryFrom<String>>`-bound owned-string consumer reaches the same
/// four-arm `PascalCase` accept-set through one trait dispatch.
///
/// `type Error = ()` matches the sibling [`TryFrom<&str> for
/// RestartStrategy`], [`TryFrom<&[u8]> for RestartStrategy`], and
/// [`TryFrom<Vec<u8>> for RestartStrategy`] unit-error shapes, preserving the
/// trait-family consistency across the {str-view, byte-view, byte-owned,
/// string-owned} reverse-projection square. The owned [`String`] input is
/// dropped on the error path (the standard-library `String::from_utf8`
/// convention of returning the input in the error deliberately declined — a
/// caller that needs the string back holds a clone before the call, and the
/// closed-set-enum use site rarely wants the raw string back past a "did you
/// mean" diagnostic that operates on the wire vocabulary rather than the
/// input).
///
/// Pinned load-bearing by
/// [`tests::restart_strategy_try_from_owned_string_routes_through_borrowed_str_view_axis`]
/// (byte-parity pin against the paired borrowed [`TryFrom<&str>`] axis across
/// the four-arm [`RestartStrategy::ALL`] accept-set on the owned-`String`
/// surface, cross-axis witness that the string-owned reverse projection
/// agrees with the sibling byte-view / byte-owned reverse-projection axes on
/// every accepted arm through the shared substrate-primitive
/// [`RestartStrategy::from_wire`] accessor, and a closed-cycle witness
/// against the paired string-owned forward-projection pair — `Self → String
/// → TryFrom<String> → Self` round-trips to the originating arm on every
/// canonical `PascalCase` scalar) and
/// [`tests::restart_strategy_try_from_owned_string_rejects_unknown_wire_strings`]
...
/// including empty / whitespace-only inputs, the sibling kebab-case
/// dispatcher-catalog byte-strings on the two-axis wire/catalog split so a
/// caller that confuses the two axes trips here rather than at a downstream
/// K8s-CR round-trip miss, case-fold rebrand candidates, whitespace-padded /
/// trailing-newline / quote-wrapped forms, and English-rebrand candidates,
/// with per-input cross-axis parity against the borrowed [`TryFrom<&str>`]
/// reverse-projection axis).
impl TryFrom<String> for RestartStrategy {
    type Error = ();
...
/// Trait-idiomatic *owned-`String`* forward projection on the second
/// M2 OTP-shape closed-set typed enum ([`RestartPolicy`]) — the
/// owned-heap-string companion to the paired `&'static str`-returning
/// [`From<RestartPolicy> for &'static str`] / [`From<&RestartPolicy>
/// for &'static str`] impls immediately above. Routes byte-for-byte
/// through the substrate-primitive [`RestartPolicy::as_str`] `pub
/// const fn` accessor (via [`str::to_owned`]) so every consumer that
/// binds a [`RestartPolicy`] through the standard-library `.into()` /
/// [`From<Self> for String`] (equivalently [`Into<String>`]) axis — a
/// future `serde_json::Value::String(policy.into())` structured-payload
/// composer where the `Value::String` arm typing demands an owned
/// [`String`] and the sibling [`&'static str`]-returning axis forces
/// an explicit `.to_owned()` / `String::from` restatement at every
/// call site, a future `HashMap::<String, RestartPolicy>::from_iter(
/// RestartPolicy::ALL.iter().map(|p| (p.into(), *p)))` per-policy
/// lookup where the map's key type is owned [`String`] rather than
/// [`&'static str`], a future `Cow::<'static, str>::Owned(policy.into())`
/// composer on the future M4 admission-webhook rejection body's
/// owned-arm, the future wasm-operator's per-child post-exit
/// diagnostic emit `serde_json::json!({ "restart": policy })` where the
/// JSON serializer's `Serialize` impl on [`String`] owns the emit-path
/// — reaches the same three-arm lifted
/// [`crate::render::SUPERVISOR_CHILD_RESTART_PERMANENT`] /
/// [`crate::render::SUPERVISOR_CHILD_RESTART_TEMPORARY`] /
/// [`crate::render::SUPERVISOR_CHILD_RESTART_TRANSIENT`] const the
/// paired [`std::fmt::Display`], [`AsRef<str>`],
/// [`RestartPolicy::as_str`], and the two `&'static str`-returning
/// forward-projection impls already return.
///
/// Extends the trait-idiomatic *owned-`String`* forward-projection
/// axis onto the second-of-two M2 OTP-shape closed-set typed enums on
/// the caixa surface — mirror of the first-mover
/// [`From<RestartStrategy> for String`] (7baa18a) that opened this
/// axis on the sibling supervisor-level strategy enum. Rust's standard
/// library does not carry a blanket `impl<T: AsRef<str>> From<T> for
/// String` (nor an `impl<T: fmt::Display> From<T> for String`), so
/// every closed-set typed enum that carries the paired `AsRef<str>` /
/// `Display` / `From<Self> for &'static str` triple but not the
/// owned-[`String`] axis forces every owned-string call site through a
/// `.to_string()` / `.as_str().to_owned()` / `String::from(policy.as_str())`
/// detour whose type bounds have no compile-time link to the
/// substrate primitive.
///
/// Deliberately routes through the human-readable
/// [`RestartPolicy::as_str`] axis — for this enum the wire format
/// (`PascalCase`, tatara-lisp author surface `:restart Permanent`) and
/// the diagnostic byte-string share the same vocabulary by
/// construction (unlike the sibling [`crate::CaixaKind`] enum whose
/// two axes diverge), so the owned-[`String`] projection lands
/// byte-identically on both the wire vocabulary the paired
/// [`serde::Serialize`] derive emits and the diagnostic vocabulary the
/// [`RestartPolicy::as_str`] helper returns, and — because the paired
/// [`TryFrom<&str>`] / [`RestartPolicy::from_wire`] reverse-projection
/// axis parses the same `PascalCase` vocabulary — the direct two-way
/// `Self → String → Self` round-trip composes without the wire-vocab
/// intermediate hop the peer [`crate::CaixaKind`] owned-[`String`]
/// axis pair requires.
///
/// Pinned load-bearing by
/// [`tests::restart_policy_from_into_owned_string_routes_through_as_str_accessor`]
/// (byte-parity pin against [`RestartPolicy::as_str`] across the
/// three-arm emit-set, plus a blanket `.into::<String>()` shape
/// witness) and
/// [`tests::restart_policy_from_into_owned_string_and_static_str_agree_on_every_arm`]
/// (cross-axis partition pin against the paired owned-input
/// [`From<RestartPolicy> for &'static str`] impl and the sibling
/// [`ToString::to_string`] surface routed through [`std::fmt::Display`],
/// plus a `.iter().copied().map(String::from)` pipe witness over
/// [`RestartPolicy::ALL`], plus a direct round-trip witness through
/// [`TryFrom<&str>`] on the owned-[`String`]'s [`String::as_str`]
/// borrow that closes the two-way `Self → String → Self` round-trip
/// on the trait-idiomatic owned-[`String`] forward + reverse axis
/// pair).
impl From<RestartPolicy> for String {
    fn from(policy: RestartPolicy) -> String {
        policy.as_str().to_owned()
    }
}

/// Trait-idiomatic *borrowed-input, owned-`String` output* forward
/// projection on the second-of-two M2 OTP-shape closed-set typed enum
/// ([`RestartPolicy`]) — the fourth (and closing) corner of the
/// `{Self, &Self} × {&'static str, String}` 2×2 trait-idiomatic
/// projection family on this enum, mirror of the first-mover
/// [`From<&RestartStrategy> for String`] (579385f) that opened the
/// 2×2-completion corner on the sibling supervisor-level strategy
/// enum. Routes byte-for-byte through the substrate-primitive
/// [`RestartPolicy::as_str`] `pub const fn` accessor (via
/// [`str::to_owned`]) so every consumer that holds a borrowed
/// [`&RestartPolicy`] and needs an owned [`String`] — a future
/// `serde_json::Value::String(String::from(&policy))` structured-payload
/// composer over a borrowed field, a future `Iterator::map` over
/// `&[RestartPolicy]` that projects to owned keys through
/// `.iter().map(String::from)`, a future `HashMap::<String,
/// RestartPolicy>::from_iter` that keys off a borrowed-iteration axis
/// where dereferencing the policy would force an unnecessary `Copy` at
/// every step, the future wasm-operator's per-supervisor
/// `child_policies.iter().map(String::from).collect()` per-child post-
/// exit restart-decision diagnostic emit whose iteration axis is
/// borrowed by construction — reaches the same three-arm lifted
/// [`crate::render::SUPERVISOR_CHILD_RESTART_PERMANENT`] /
/// [`crate::render::SUPERVISOR_CHILD_RESTART_TEMPORARY`] /
/// [`crate::render::SUPERVISOR_CHILD_RESTART_TRANSIENT`] const the
/// paired [`std::fmt::Display`], [`AsRef<str>`],
/// [`RestartPolicy::as_str`], and the three other trait-idiomatic
/// forward-projection impls
/// ([`From<RestartPolicy> for &'static str`],
/// [`From<&RestartPolicy> for &'static str`],
/// [`From<RestartPolicy> for String`]) already return.
///
/// Second peer on the substrate-wide trait-idiomatic *borrowed-input,
/// owned-`String` output* forward-projection family opened on
/// [`crate::supervisor::RestartStrategy`] (579385f) — closes the
/// `{Self, &Self} × {&'static str, String}` 2×2 projection corner on
/// both M2 OTP-shape sibling peers (the paired supervisor-level
/// sibling-restart-strategy axis and the per-child restart-decision-
/// policy axis), so the whole M2 OTP-shape axis pair now carries the
/// full four-corner family by construction. Rust's standard library
/// does not carry a blanket `impl<T: AsRef<str>> From<&T> for String`
/// (nor an `impl<T: fmt::Display> From<&T> for String`), so every
/// closed-set typed enum that carries the paired `AsRef<str>` /
/// `Display` / `From<Self> for &'static str` / `From<&Self> for
/// &'static str` / `From<Self> for String` quintuple but not the
/// borrowed-input owned-[`String`] axis forces every borrowed-input
/// owned-string call site through a `policy.as_str().to_owned()` /
/// `String::from(*policy)` (with a spurious `Copy`) /
/// `policy.to_string()` (through `Display`) detour whose type bounds
/// have no compile-time link to the substrate primitive.
///
/// Deliberately routes through the human-readable
/// [`RestartPolicy::as_str`] axis — for this enum the wire format
...
/// the diagnostic byte-string share the same vocabulary by
/// construction (unlike the sibling [`crate::CaixaKind`] enum whose
/// two axes diverge), so the borrowed-input owned-[`String`]
/// projection lands byte-identically on both the wire vocabulary the
/// paired [`serde::Serialize`] derive emits and the diagnostic
/// vocabulary the [`RestartPolicy::as_str`] helper returns, and —
/// because the paired [`TryFrom<&str>`] / [`RestartPolicy::from_wire`]
/// reverse-projection axis parses the same `PascalCase` vocabulary —
/// the direct two-way `&Self → String → Self` round-trip composes
/// without the wire-vocab intermediate hop the peer
/// [`crate::CaixaKind`] axis pair requires.
///
/// The remaining thirteen closed-set typed enums on the caixa
/// substrate surface (`CaixaKind`, `CaixaDialeto`, `DepList`,
/// `PlacementStrategy`, `WitShape`, `RateLimitUnit`,
/// `PathShapeViolation`, `InvariantKind`, `ArchVerdict`, `Severity`,
/// `FixSafety`, `Semantic`, `FerriteRuntime`) are the future targets
/// of this 2×2-completion campaign — each carries the same paired
/// quintuple that this borrowed-input owned-[`String`] axis extends
/// onto.
///
/// Pinned load-bearing by
/// [`tests::restart_policy_from_into_borrowed_owned_string_routes_through_as_str_accessor`]
/// (byte-parity pin against [`RestartPolicy::as_str`] across the
/// three-arm emit-set through the borrowed-input surface) and
/// [`tests::restart_policy_from_into_borrowed_owned_string_agrees_with_paired_axes_on_every_arm`]
/// (cross-axis partition pin against the paired owned-input owned-
/// [`String`] [`From<RestartPolicy> for String`] impl, the paired
/// borrowed-input owned-[`&'static str`] [`From<&RestartPolicy> for
/// &'static str`] impl, and the sibling [`ToString::to_string`]
/// surface routed through [`std::fmt::Display`], plus a direct round-
/// trip witness through [`TryFrom<&str>`] on the owned-[`String`]'s
/// [`String::as_str`] borrow that closes the two-way
/// `&Self → String → Self` round-trip on the trait-idiomatic
/// borrowed-input owned-[`String`] forward + reverse axis pair).
impl From<&RestartPolicy> for String {
...
/// Trait-idiomatic *borrowed-input, [`std::borrow::Cow<'static, str>`]
/// output* forward projection on the M2 OTP-shape per-child-restart
/// [`RestartPolicy`] closed-set typed enum — the borrowed-input
/// companion to the paired owned-input
/// [`From<RestartPolicy> for std::borrow::Cow<'static, str>`] impl
/// immediately above (0612398). Routes byte-for-byte through the same
/// substrate-primitive [`RestartPolicy::as_str`] `pub const fn`
/// accessor (via [`std::borrow::Cow::Borrowed`]) so every consumer
/// that holds a `&RestartPolicy` and needs a
/// [`std::borrow::Cow<'static, str>`] — a
/// `RestartPolicy::ALL.iter().map(std::borrow::Cow::from).collect::<Vec<_>>()`
/// per-arm accept-set materializer (whose iterator over
/// `&'static [RestartPolicy]` yields `&RestartPolicy`, not
/// `RestartPolicy`, so the paired owned-input
/// [`From<RestartPolicy> for std::borrow::Cow<'static, str>`] axis
/// alone forces every call site through an explicit `.copied()` /
/// dereference / [`Copy`]-bound restatement rather than the direct
/// trait-idiomatic projection), a future generic
/// `<T: for<'a> Into<std::borrow::Cow<'static, str>>>`-bound emitter
/// on a per-child-policy diagnostic column that walks the
/// `iter().map(Into::into)` shape verbatim, the future M4 admission-
/// webhook rejection body that composes the accepted-policy
/// enumeration from an iterated
/// `RestartPolicy::ALL.iter().map(|p| p.into())` pipe rather than a
/// per-arm `match p { … }` cascade — reaches the same three-arm
/// lifted [`crate::render::SUPERVISOR_CHILD_RESTART_PERMANENT`] /
/// [`crate::render::SUPERVISOR_CHILD_RESTART_TEMPORARY`] /
/// [`crate::render::SUPERVISOR_CHILD_RESTART_TRANSIENT`] const the
/// paired [`std::fmt::Display`], [`AsRef<str>`],
/// [`RestartPolicy::as_str`], the four
/// `{Self, &Self} × {&'static str, String}` 2×2 trait-idiomatic
/// forward-projection corners, and the paired owned-input
/// [`From<RestartPolicy> for std::borrow::Cow<'static, str>`] impl
/// already return.
...
/// Deliberately returns [`std::borrow::Cow::Borrowed`] rather than
/// [`std::borrow::Cow::Owned`] — the substrate-primitive
/// [`RestartPolicy::as_str`] accessor's return carries the
/// `&'static str` lifetime by construction (each `match` arm resolves
/// to a [`crate::render::SUPERVISOR_CHILD_RESTART_*`] `pub const &str`
/// with static lifetime), so the zero-alloc borrowed arm is the
/// type-correct projection with no runtime allocation.
///
/// Closes the `{Self, &Self}` input-shape corner on the M2 OTP-shape
/// per-child-restart [`std::borrow::Cow<'static, str>`] axis opened
/// one commit prior (0612398) on the paired owned-input
/// [`From<RestartPolicy> for std::borrow::Cow<'static, str>`] impl —
/// second-of-two-in-M2 closed-set fieldless typed enum peer on the
/// caixa surface (paired with the sibling-restart [`RestartStrategy`]
/// which carries both {Self, &Self} × Cow<'static, str> corners since
/// 7dd28b3 owned-input, 9b3e4b3 borrowed-input), exactly as d45c409
/// closed it on the top-level [`crate::CaixaKind`] one commit after
/// the owning half (99c1735) landed. This lift closes the whole M2
/// OTP-shape tier of the substrate-wide [`Cow<'static, str>`]
/// forward-projection campaign on both input-shape corners
/// ({Self, &Self}) of both M2 OTP-shape sibling peers
/// ([`RestartStrategy`] and [`RestartPolicy`]), so the remaining
/// eleven substrate-wide peers (`PlacementStrategy`, `RateLimitUnit`,
/// `DepList`, `CaixaDialeto`, `WitShape`, `PathShapeViolation`,
/// `InvariantKind`, `ArchVerdict`, `Severity`, `FixSafety`, `Semantic`,
/// `FerriteRuntime`) become the future targets of the campaign. Rust's
...
/// [`Copy`] deref (`std::borrow::Cow::from(*policy)`) or a
/// `std::borrow::Cow::Borrowed(policy.as_str())` open-code whose type
/// bounds have no compile-time link to the substrate primitive.
///
/// Pinned load-bearing by
/// [`tests::restart_policy_from_borrowed_into_static_cow_str_routes_through_as_str_accessor`]
/// (byte-parity + zero-alloc [`std::borrow::Cow::Borrowed`]-arm pin
/// against [`RestartPolicy::as_str`] across the three-arm
/// [`RestartPolicy::ALL`] through the borrowed-input surface) and
/// [`tests::restart_policy_from_borrowed_into_static_cow_str_agrees_with_paired_axes_on_every_arm`]
/// (cross-axis partition pin against the paired owned-input
/// [`From<RestartPolicy> for std::borrow::Cow<'static, str>`], the
/// paired borrowed-input owned-`&'static str`
/// [`From<&RestartPolicy> for &'static str`], and the paired
/// borrowed-input owned-`String` [`From<&RestartPolicy> for String`]
/// impls, plus a `.iter().map(std::borrow::Cow::from)` pipe witness
/// over [`RestartPolicy::ALL`] — whose iterator yields
/// `&RestartPolicy` by construction, so the borrowed-input
/// [`Cow<'static, str>`] axis is what routes the pipe through the
/// substrate-primitive [`RestartPolicy::as_str`] accessor with the
/// zero-alloc [`Cow::Borrowed`] arm by construction and without a
/// spurious [`Copy`] deref).
impl From<&RestartPolicy> for std::borrow::Cow<'static, str> {
    fn from(policy: &RestartPolicy) -> std::borrow::Cow<'static, str> {
...
/// closed-set fieldless typed enum — the single-threaded reference-
/// counted peer of the paired owned-input
/// [`From<RestartPolicy> for std::sync::Arc<str>`] impl (b05724e) on the
...
/// blanket), and the [`std::sync::Arc<str>`] and [`std::rc::Rc<str>`]
/// trait tables are disjoint — so a single-threaded caixa-operator
/// reconciliation pass that shares the `:children :restart` wire
/// byte-string across intra-reconcile-loop tree nodes through the cheaper
/// non-atomic [`std::rc::Rc<str>`] refcount (the atomically-reference-
/// counted [`std::sync::Arc<str>`] return-shape cannot provide within a
/// single-threaded reconciliation pass without paying the atomic-fence
...
/// the substrate primitive through this impl and no other.
///
/// Closes the substrate-wide trait-idiomatic [`std::rc::Rc<str>`]
/// forward-projection axis on the M2-OTP-shape `:supervisor
/// :estrategia` and `:children :restart` slot pair the sibling-restart
/// [`RestartStrategy`] first-mover (71ad8f4) opened one projection tier
/// prior on the paired sibling enum — closes the paired axis on the
/// second (and third-and-final) M2 OTP-shape closed-set fieldless typed
/// enum peer on the caixa surface, matching the discipline the paired
/// [`std::sync::Arc<str>`] forward-projection axis campaign already
/// carried across the same slot pair (bca2ec8 → b3e72d7 on
/// [`RestartStrategy`]; b05724e → borrowed-close on this enum).
///
/// Pinned load-bearing by
/// [`tests::restart_policy_from_into_rc_str_routes_through_as_str_accessor`]
/// (byte-parity pin against [`RestartPolicy::as_str`] across the
/// three-arm [`RestartPolicy::ALL`] emit-set on the owned-input
/// surface, plus a blanket-derived [`Into`] shape witness and cross-
/// axis byte-parity pins against the sibling owned-input `{&'static
/// str, String, Cow<'static, str>, Box<str>, std::sync::Arc<str>}`
/// return-shape axes).
impl From<RestartPolicy> for std::rc::Rc<str> {
    fn from(policy: RestartPolicy) -> std::rc::Rc<str> {
        std::rc::Rc::<str>::from(policy.as_str())
    }
}
...
/// projection on the M2 OTP-shape per-child-restart [`RestartPolicy`]
/// closed-set fieldless typed enum — the borrowed-input companion to
/// the paired owned-input [`From<RestartPolicy> for std::rc::Rc<str>`]
/// impl immediately above, closing the `{Self, &Self}` input-shape
/// corner of the [`std::rc::Rc<str>`] axis on the second (and third-
/// and-final) M2 OTP-shape closed-set fieldless typed enum peer on the
/// caixa surface. Routes byte-for-byte through the substrate-primitive
/// [`RestartPolicy::as_str`] `pub const fn` accessor via
/// [`std::rc::Rc::<str>::from`] on the returned `&'static str`, so a
/// `RestartPolicy::ALL.iter().map(std::rc::Rc::<str>::from)`-shaped
/// pipe (whose iterator over `&'static [RestartPolicy]` yields
/// `&RestartPolicy` by construction) reaches the same three-arm lifted
/// [`crate::render::SUPERVISOR_CHILD_RESTART_PERMANENT`] /
/// [`crate::render::SUPERVISOR_CHILD_RESTART_TEMPORARY`] /
/// [`crate::render::SUPERVISOR_CHILD_RESTART_TRANSIENT`] const roster
/// the paired owned-input axis and the sibling `{Self, &Self} ×
/// {&'static str, String, Cow<'static, str>, Box<str>,
/// std::sync::Arc<str>}` forward-projection corner already return.
///
/// Rust's standard library carries no blanket
...
///
/// Closes the substrate-wide trait-idiomatic [`std::rc::Rc<str>`]
/// forward-projection family on the M2-OTP-shape `:supervisor
/// :estrategia` + `:children :restart` slot pair — the sibling
/// [`RestartStrategy`] first-mover (71ad8f4) opened + closed the pair
/// on the sibling `:supervisor :estrategia` half one projection tier
/// prior, and the paired owned-input [`From<RestartPolicy> for
/// std::rc::Rc<str>`] impl immediately above opens the same axis on
/// this half — this borrowed-input impl closes it.
///
/// Pinned load-bearing by
/// [`tests::restart_policy_from_borrowed_into_rc_str_routes_through_as_str_accessor`]
/// (byte-parity pin against [`RestartPolicy::as_str`] across the
/// three-arm [`RestartPolicy::ALL`] emit-set on the borrowed-input
/// surface, plus a blanket-derived [`Into`] shape witness, a cross-
/// axis partition pin against the paired owned-input
/// [`From<RestartPolicy> for std::rc::Rc<str>`] and the sibling
/// borrowed-input `{&'static str, String, Cow<'static, str>, Box<str>,
/// std::sync::Arc<str>}` return-shape axes, and a
/// `.iter().map(std::rc::Rc::<str>::from)` pipe witness over
/// [`RestartPolicy::ALL`] that resolves through the borrowed-input
/// axis without a spurious [`Copy`] deref).
impl From<&RestartPolicy> for std::rc::Rc<str> {
    fn from(policy: &RestartPolicy) -> std::rc::Rc<str> {
        std::rc::Rc::<str>::from(policy.as_str())
...
/// str-view reverse-projection axis carried by the paired
/// [`TryFrom<&str> for RestartPolicy`] impl (which routes through the
/// substrate-primitive [`RestartPolicy::from_wire`] `Option<Self>` accessor
/// on the three-arm `PascalCase` accept-set the sibling
/// [`RestartPolicy::as_str`] emitter returns). Routes byte-for-byte through
/// the standard-library [`std::str::from_utf8`] UTF-8 validator and then
/// through [`RestartPolicy::from_wire`] so every consumer that holds a
/// borrowed [`&[u8]`] and needs to project it back into a typed
/// [`RestartPolicy`] — a future `bytes::Bytes::as_ref()`-fed reader that
/// parses a per-child `:restart` `PascalCase` wire scalar from an
/// already-borrowed framing byte-tail (a
/// `tracing::field::valuable::Value::Bytes` recorder on the future
/// wasm-operator's per-child restart-decision diagnostic emission path, a
/// future audit-report re-loader binding a prior
/// [`RestartPolicy::as_str`] output from a mmap'd byte-slice back through
/// the typed enum for cross-run comparison), a future M4
/// `mesh.pleme.io/v1alpha1/Supervisor` CR admission-webhook rejection body
/// that reads a `spec.children[].restart` field off a raw HTTP body
/// byte-slice before UTF-8 validation commits allocation, a future generic
/// `<T: for<'a> TryFrom<&'a [u8]>>`-bound loader over any of the
/// substrate's closed-set typed enums — reaches the same three-arm
/// `PascalCase` wire accept-set the sibling method-named
/// [`RestartPolicy::from_wire`] resolver and the paired trait-idiomatic
/// [`TryFrom<&str>`] axis already resolve against, rather than an open-
/// coded per-call-site
/// `std::str::from_utf8(bytes).ok().and_then(RestartPolicy::from_wire)`
/// composition or a
/// `<RestartPolicy as TryFrom<&str>>::try_from(std::str::from_utf8(bytes)?)`
/// two-hop shape whose type bounds have no compile-time link to the
/// substrate primitive.
///
/// Closes the substrate-wide trait-idiomatic *byte-view reverse-projection*
/// family on the M2-OTP-shape `:supervisor :estrategia` + `:children
/// :restart` slot pair the sibling [`RestartStrategy`] first-mover
/// (c699a83) opened one commit prior — extends the family from
/// [`crate::CaixaKind`] (18d1940), [`crate::CaixaDialeto`] (d102cb8),
/// [`crate::dep::DepList`] (b8f25d5), and [`RestartStrategy`] (c699a83)
/// onto the second (and final) M2-OTP-shape closed-set fieldless typed
/// enum peer on the caixa surface, matching the trajectory the paired
/// byte-owned reverse-projection axis campaign already walked across the
/// same slot pair (63e5dd0 → 96a522a). Rust's standard library carries no
/// blanket `impl<T: for<'a> TryFrom<&'a str>> TryFrom<&[u8]> for T`, so a
...
/// remaining closed-set fieldless typed enum peer on the substrate
/// ([`crate::aplicacao::PlacementStrategy`],
/// [`crate::aplicacao::RateLimitUnit`], [`crate::aplicacao::WitShape`],
/// and the outside-`caixa-core` peers `PathShapeViolation`,
/// `InvariantKind`, `ArchVerdict`, `Severity`, `FixSafety`, `Semantic`,
/// `FerriteRuntime`) is a future target of the campaign.
///
/// `type Error = ()` matches the sibling [`RestartPolicy::from_wire`]'s
/// `Option<Self>` return-shape's deliberate deferral of error typing and
/// the paired trait-idiomatic [`TryFrom<&str>`] axis's unit-error shape —
/// the caller picks the diagnostic form appropriate for its use site (a
/// future `feira supervisor --restart …` arg-parse composes its own
/// per-verb "unknown restart policy: <arg> — accepted: {…}" message
/// enumerating [`RestartPolicy::WIRE_NAMES`]; a future admission-webhook
...
/// site lifts the unit-error to a per-verb error type). Two rejection
/// paths route through the single unit-error: an invalid UTF-8
/// byte-sequence ([`std::str::from_utf8`] returns `Err`) and a valid UTF-8
/// byte-string that falls outside the three-arm `PascalCase` accept-set
/// ([`RestartPolicy::from_wire`] returns `None`) — both collapse onto
/// `Err(())` so the trait signature stays consistent with the sibling
/// str-view reverse axis, and a caller that needs to distinguish the two
/// failure modes composes [`std::str::from_utf8`] +
/// [`RestartPolicy::from_wire`] explicitly.
///
/// Pinned load-bearing by
/// [`tests::restart_policy_try_from_bytes_routes_through_from_wire_accessor`]
/// (byte-parity pin against [`RestartPolicy::from_wire`] across the
/// three-arm [`RestartPolicy::ALL`] accept-set on the borrowed byte-slice
/// surface, plus a cross-axis witness that the byte-view reverse
/// projection agrees with the paired [`TryFrom<&str>`] str-view reverse
/// axis on every accepted arm, and a forward/reverse byte-view cross-axis
/// witness that feeding the paired [`AsRef<[u8]>`] byte-tail back through
/// the new impl round-trips to the originating arm) and
/// [`tests::restart_policy_try_from_bytes_rejects_unknown_and_non_utf8_bytes`]
/// (rejection witness against silent accept-set widening on both the
/// non-UTF-8 byte-sequence rejection path and the unknown-wire-vocabulary
/// rejection path — the latter includes the sibling kebab-case
/// dispatcher-catalog byte-strings on the two-axis wire/catalog split so
...
/// Trait-idiomatic *owned-`String` input, `Result<Self, ()>` output*
/// string-owned reverse projection on the second (and final) M2-OTP-shape
/// supervisor-slot closed-set fieldless typed enum peer on the caixa
/// surface ([`RestartPolicy`]) — the owned-input peer of the paired
/// [`TryFrom<&str> for RestartPolicy`] str-view reverse-projection axis,
/// and the string-owned reverse companion of the pre-existing string-owned
/// *forward* pair ([`From<RestartPolicy> for String`],
/// [`From<&RestartPolicy> for String`]) already lifted on this same enum.
/// Routes owned [`String`] input through the paired borrowed-input
/// [`TryFrom<&str>`] axis via [`String::as_str`] so every consumer that
/// holds an owned `String` — a future M4 `mesh.pleme.io/v1alpha1/Supervisor`
/// CR admission-webhook body reader that hands the
/// `spec.children[].restart` `PascalCase` scalar off as an owned [`String`]
/// after UTF-8 validation, a `serde_yaml::from_str` / `serde_json::from_str`
/// de-serialize round-trip whose composer surfaces the `:children :restart`
/// scalar as an owned [`String`] typed field, a
/// `feira supervisor --restart <Permanent|Temporary|Transient>`
/// `clap`-derived arg-parse whose owned-`String` positional lands the
/// canonical arm at the typed dispatch, a per-`:children`-slot overlay
/// resolver reading an owned [`String`] out of a `ConfigMap`
/// `data.children-restart` scalar, an `<T: TryFrom<String>>`-bound generic
/// loader over any of the substrate's closed-set typed enums — reaches the
/// same three-arm `PascalCase` accept-set through one trait dispatch.
///
/// Extends the substrate-wide trait-idiomatic *string-owned reverse-
/// projection* family — opened on the compound M3-mesh
/// `:politicas :rate-limit` primitive [`crate::aplicacao::RateLimit`]
/// (a2e6f02), lifted onto the first closed-set fieldless typed-enum peer
/// [`crate::aplicacao::WitShape`] (e6aac29), extended onto the second
/// closed-set fieldless typed-enum peer [`crate::aplicacao::RateLimitUnit`]
/// (94a9c5e), extended onto the third closed-set fieldless typed-enum peer
/// [`crate::aplicacao::PlacementStrategy`] (d81a70a), extended onto the
/// first M2-OTP-shape supervisor-slot closed-set fieldless typed-enum peer
/// [`RestartStrategy`] (78fe8c8) — onto the second (and final) M2-OTP-shape
/// supervisor-slot closed-set fieldless typed-enum peer, the per-`:children`
/// restart-decision-policy discriminator. This closes the string-owned
/// reverse-projection axis on the M2-OTP-shape `:supervisor :estrategia` +
/// `:children :restart` slot pair, mirroring the trajectory the byte-view
/// / byte-owned / str-view reverse-projection families already walked
/// across the same slot pair. The peers [`crate::CaixaKind`],
/// [`crate::CaixaDialeto`], and [`crate::dep::DepList`] remain the next
/// targets of the campaign.
///
/// Rust's standard library carries no blanket
/// `impl<T: for<'a> TryFrom<&'a str>> TryFrom<String> for T`, so a consumer
/// that holds an owned [`String`] and needs a typed [`RestartPolicy`]
/// otherwise picks between (a) an open-coded
/// `<RestartPolicy as TryFrom<&str>>::try_from(s.as_str())` at every call
/// site whose type bounds have no compile-time link back to the string-
/// owned reverse-projection axis, (b) a `let s: &str = &s;
/// RestartPolicy::try_from(s)` two-step whose borrow arithmetic leaks a
/// per-call-site lifetime dance rather than a single trait dispatch, or
/// (c) a `String::into_bytes` + [`TryFrom<Vec<u8>>`] detour that reaches
/// the substrate-primitive `from_wire` accessor through a UTF-8 re-
/// validation hop the owned-`String` axis already knows to skip. This
/// impl closes the string-owned reverse-projection axis at the substrate-
/// primitive [`RestartPolicy::from_wire`] accessor so every future
/// `<T: TryFrom<String>>`-bound owned-string consumer reaches the same
/// three-arm `PascalCase` accept-set through one trait dispatch.
///
/// `type Error = ()` matches the sibling [`TryFrom<&str> for
/// RestartPolicy`], [`TryFrom<&[u8]> for RestartPolicy`], and
/// [`TryFrom<Vec<u8>> for RestartPolicy`] unit-error shapes, preserving
/// the trait-family consistency across the {str-view, byte-view, byte-
/// owned, string-owned} reverse-projection square. The owned [`String`]
/// input is dropped on the error path (the standard-library
/// `String::from_utf8` convention of returning the input in the error
/// deliberately declined — a caller that needs the string back holds a
/// clone before the call, and the closed-set-enum use site rarely wants
/// the raw string back past a "did you mean" diagnostic that operates on
/// the wire vocabulary rather than the input).
///
/// Pinned load-bearing by
/// [`tests::restart_policy_try_from_owned_string_routes_through_borrowed_str_view_axis`]
...
/// primitive [`RestartPolicy::from_wire`] accessor, and a closed-cycle
/// witness against the paired string-owned forward-projection pair —
/// `Self → String → TryFrom<String> → Self` round-trips to the
/// originating arm on every canonical `PascalCase` scalar) and
/// [`tests::restart_policy_try_from_owned_string_rejects_unknown_wire_strings`]
/// (rejection witness against silent accept-set widening — mirrors the
/// corpus the paired [`TryFrom<&str>`] rejection witness already pins,
/// including empty / whitespace-only inputs, the sibling kebab-case
/// dispatcher-catalog byte-strings on the two-axis wire/catalog split so
/// a caller that confuses the two axes trips here rather than at a
/// downstream K8s-CR round-trip miss, case-fold rebrand candidates,
/// whitespace-padded / trailing-newline / quote-wrapped forms, and
/// English-rebrand candidates, with per-input cross-axis parity against
/// the borrowed [`TryFrom<&str>`] reverse-projection axis).
...
mod tests {
    ...
    #[test]
    fn restart_strategy_from_into_borrowed_owned_string_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<&RestartStrategy> for String` — asserts the
        // borrowed-input owned-`String`-returning standard-library trait
        // impl and the substrate-primitive [`RestartStrategy::as_str`]
        // `pub const fn` accessor resolve to the same four-arm emit-set
        // across every arm the exhaustive [`RestartStrategy::ALL`] slice
        ...
        // `let key: String = (&strategy).into();`-shaped call site
        // reaches through this impl and no other — the paired sibling
        // `From<RestartStrategy> for String` impl forces every
    ...
    #[test]
    fn restart_strategy_from_into_borrowed_owned_string_agrees_with_paired_axes_on_every_arm() {
        // Cross-axis partition pin: the newly lifted trait-idiomatic
        // borrowed-input owned-`String` `From<&RestartStrategy> for
        // String` (this lift), the paired owned-input owned-`String`
        // `From<RestartStrategy> for String` (7baa18a), the paired
        // borrowed-input owned-`&'static str` `From<&RestartStrategy>
        // for &'static str` (e941836), and the paired owned-input
        // owned-`&'static str` `From<RestartStrategy> for &'static str`
        // (523157d) — every corner of the `{Self, &Self} × {&'static
        // str, String}` 2×2 trait-idiomatic projection family — must
        // resolve identically on every arm, locking the four
        // return-shape × input-shape paths together so any future
        // detour trips at caixa-core test time. Also byte-parity
        // witness against the sibling [`ToString::to_string`] surface
        // routed through [`std::fmt::Display`] and a direct round-trip
        // witness through the paired trait-idiomatic reverse
        // [`TryFrom<&str>`] axis on the owned-`String`'s
        // [`String::as_str`] borrow that closes the two-way
        // `&Self → String → Self` round-trip on the trait-idiomatic
        ...
        for &variant in RestartStrategy::ALL {
            ...
            assert_eq!(
                ...
                "From<&RestartStrategy> for String and From<RestartStrategy> \
                 for &'static str must resolve identically on \
                 RestartStrategy::{variant:?} — divergence signals a break \
            ...
            assert_eq!(
                ...
                "From<&RestartStrategy> for String must byte-equal \
                 ...
                 ToString-through-Display axis have drifted onto different \
                 emit-sets"
            );
        }
        let via_iter: Vec<String> = RestartStrategy::ALL.iter().map(String::from).collect();
        let via_method: Vec<String> = RestartStrategy::ALL
            .iter()
            .map(|s| s.as_str().to_owned())
            .collect();
        assert_eq!(
            via_iter, via_method,
            "`.iter().map(String::from)` over RestartStrategy::ALL — a \
             call site whose iteration axis holds `&RestartStrategy` by \
             construction — must byte-equal `.iter().map(|s| \
             s.as_str().to_owned())` on every arm — the borrowed-input \
             owned-`String` `From<&RestartStrategy> for String` axis is \
             what makes the `String::from` composition route through the \
             substrate-primitive `RestartStrategy::as_str` accessor \
             without a spurious `Copy` deref (which would only be \
             reachable through the owned-input `From<RestartStrategy> for \
             String` axis by first calling `.copied()` on the iterator)"
        );
        for &variant in RestartStrategy::ALL {
            let emitted: String = (&variant).into();
            let re_parsed: Result<RestartStrategy, ()> =
            ...
            assert_eq!(
                ...
                "trait-idiomatic borrowed-input owned-`String` \
                 ...
                 round-trip &RestartStrategy::{variant:?} through \
    ...
    #[test]
    fn restart_strategy_from_into_static_cow_str_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<RestartStrategy> for std::borrow::Cow<'static, str>` —
        // asserts the standard-library trait impl and the substrate-
        // primitive [`super::RestartStrategy::as_str`] `pub const fn`
        // accessor resolve to the same four-arm emit-set across every
        // arm the exhaustive [`super::RestartStrategy::ALL`] slice
        // enumerates. Rust's standard library does not carry a blanket
        // `impl<T: AsRef<str>> From<T> for Cow<'static, str>` (nor an
        // `impl<T: fmt::Display> From<T> for Cow<'static, str>`), so
        // the `Cow<'static, str>` forward-projection axis is a
        // distinct trait-idiomatic surface that a
        // `let key: Cow<'static, str> = strategy.into();`-shaped call
        // site reaches through this impl and no other — the paired
        // sibling `From<RestartStrategy> for &'static str` and
        // `From<RestartStrategy> for String` impls force every
        ...
        // arm (an accidental `Cow::Owned(strategy.to_string())` rewrite
        // that would allocate on every call site where the
        // `&'static str` return of [`super::RestartStrategy::as_str`]
    ...
    #[test]
    fn restart_strategy_from_into_static_cow_str_agrees_with_paired_axes_on_every_arm() {
        // Cross-axis partition pin: the newly lifted trait-idiomatic
        // `From<RestartStrategy> for std::borrow::Cow<'static, str>`
        // (this lift), the paired owned-input `From<RestartStrategy>
        // for &'static str` (523157d), and the paired owned-input
        // `From<RestartStrategy> for String` (7baa18a) forward
        ...
        // `.into_owned()`, `From<RestartStrategy> for String`, and
        ...
        // witness over [`super::RestartStrategy::ALL`] that
        // materializes the four-arm accept-set through the
        // [`std::borrow::Cow<'static, str>`] axis alone — the exact
        // shape a future `axum::response::IntoResponse` per-strategy
        // rejection-body composer, a future M4 admission-webhook
        // per-strategy rejection-reason emitter whose typing rules out
        // the sibling [`AsRef<str>`] borrowed return, or a future
        // substrate-wide per-strategy diagnostic surface that binds
        ...
        for &variant in RestartStrategy::ALL {
            let via_cow: std::borrow::Cow<'static, str> =
                <std::borrow::Cow<'static, str> as From<RestartStrategy>>::from(variant);
            let via_static: &'static str = <&'static str as From<RestartStrategy>>::from(variant);
            let via_string: String = <String as From<RestartStrategy>>::from(variant);
            assert_eq!(
                via_cow.as_ref(),
                via_static,
        ...
        assert_eq!(
            ...
            "`.iter().copied().map(Cow::from)` over \
             RestartStrategy::ALL must byte-equal `.iter().map(|s| \
             Cow::Borrowed(s.as_str()))` on every arm — the \
             trait-idiomatic `From<RestartStrategy> for Cow<'static, \
             ...
             per-call-site `Cow::Owned(strategy.to_string())` \
             allocation"
        );
        for cow in &via_iter {
            assert!(
                ...
                "every element of the \
                 ...
                 RestartStrategy::ALL must land on the zero-alloc \
    ...
    #[test]
    fn restart_strategy_from_borrowed_into_box_str_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<&RestartStrategy> for Box<str>` — asserts the
        ...
        // `let key: Box<str> = (&strategy).into();`-shaped call site
        // or a `RestartStrategy::ALL.iter().map(Box::<str>::from)`-
        // shaped pipe reaches through this impl and no other — the
        // paired owned-input `From<RestartStrategy> for Box<str>`
        // impl (69ef45c) forces every borrowed-input call site
        // through an explicit `Copy` deref
        // (`Box::<str>::from((*strategy).as_str())`) or a
        // `Box::<str>::from(strategy.as_str())` open-code whose
        // type bounds have no compile-time link back to the
        // substrate primitive.
        //
        // Second peer on the substrate-wide trait-idiomatic
        // [`Box<str>`] forward-projection family on this enum —
        // closes the `{Self, &Self}` input-shape corner of the
        // [`Box<str>`] axis on the first M2 OTP-shape closed-set
        // fieldless typed enum peer on the caixa surface
        // (`:supervisor :estrategia`), exactly as ee577fd closed
        // the paired [`Cow<'static, str>`] axis one commit after
        // its owning half (7dd28b3) landed. Every future closed-
        // set fieldless typed enum peer on the substrate is a
        // future target of the campaign.
        //
        // Also byte-parity witness against the paired owned-input
        // [`From<RestartStrategy> for Box<str>`] and the sibling
        // borrowed-input [`From<&RestartStrategy> for &'static str`],
        // [`From<&RestartStrategy> for String`], and
        // [`From<&RestartStrategy> for Cow<'static, str>`]
        ...
        // [`super::RestartStrategy::ALL`] — whose iterator yields
        // `&RestartStrategy` by construction, so the borrowed-input
        ...
        for &variant in RestartStrategy::ALL {
            ...
            assert_eq!(
                via_trait.as_ref(),
                via_method,
                "From<&RestartStrategy> for Box<str> impl must \
                 round-trip &RestartStrategy::{variant:?} to the same \
                 lifted SUPERVISOR_ESTRATEGIA_* const \
            ...
            assert_eq!(
                via_trait, owned_box,
                "From<&RestartStrategy> for Box<str> and \
        ...
        let via_iter: Vec<Box<str>> = RestartStrategy::ALL.iter().map(Box::<str>::from).collect();
        let via_method: Vec<Box<str>> = RestartStrategy::ALL
            .iter()
        ...
        assert_eq!(
            ...
            "`.iter().map(Box::<str>::from)` over \
             ...
             equal `.iter().map(|s| Box::<str>::from(s.as_str()))` \
             on every arm — the borrowed-input Box<str> \
             `From<&RestartStrategy> for Box<str>` axis is what \
    ...
    #[test]
    fn restart_strategy_from_borrowed_into_arc_str_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<&RestartStrategy> for std::sync::Arc<str>` —
        // asserts the borrowed-input standard-library trait impl and
        // the substrate-primitive [`super::RestartStrategy::as_str`]
        ...
        // call site or a
        // `RestartStrategy::ALL.iter().map(std::sync::Arc::<str>::from)`-
        // shaped pipe reaches through this impl and no other — the
        // paired owned-input
        // `From<RestartStrategy> for std::sync::Arc<str>` impl
        // (bca2ec8) forces every borrowed-input call site through an
        // explicit `Copy` deref
        // (`std::sync::Arc::<str>::from((*strategy).as_str())`) or a
        // `std::sync::Arc::<str>::from(strategy.as_str())` open-code
        ...
        //
        // Second peer on the substrate-wide trait-idiomatic
        // [`std::sync::Arc<str>`] forward-projection family on this
        // enum — closes the `{Self, &Self}` input-shape corner of
        // the [`std::sync::Arc<str>`] axis on the first M2 OTP-shape
        // closed-set fieldless typed enum peer on the caixa surface
        // (`:supervisor :estrategia`), exactly as 59ae5dc closed the
        // paired [`Box<str>`] axis one commit after its owning half
        // (69ef45c) landed. Every future closed-set fieldless typed
        // enum peer on the substrate is a future target of the
        // campaign.
        //
        // Also byte-parity witness against the paired owned-input
        // [`From<RestartStrategy> for std::sync::Arc<str>`] and the
        // sibling borrowed-input
        // [`From<&RestartStrategy> for &'static str`],
        // [`From<&RestartStrategy> for String`],
        // [`From<&RestartStrategy> for Cow<'static, str>`], and
        // [`From<&RestartStrategy> for Box<str>`] return-shape axes —
        ...
        // [`super::RestartStrategy::as_str`] accessor without a
        // spurious [`Copy`] deref (which would only be reachable
        // through the owned-input
        // [`From<RestartStrategy> for std::sync::Arc<str>`] axis by
        ...
        let via_iter: Vec<std::sync::Arc<str>> = RestartStrategy::ALL
            .iter()
            .map(std::sync::Arc::<str>::from)
            .collect();
        let via_method: Vec<std::sync::Arc<str>> = RestartStrategy::ALL
            .iter()
        ...
        assert_eq!(
            ...
            "`.iter().map(std::sync::Arc::<str>::from)` over \
             RestartStrategy::ALL — a call site whose iteration axis \
             holds `&RestartStrategy` by construction — must byte-\
             equal `.iter().map(|s| std::sync::Arc::<str>::from(s.as_str()))` \
             on every arm — the borrowed-input std::sync::Arc<str> \
             `From<&RestartStrategy> for std::sync::Arc<str>` axis is \
    ...
    #[expect(
        ...
        reason = "the byte-owned reverse-projection axis is extended \
                  here onto the first M2-OTP-shape closed-set fieldless \
                  typed-enum peer, so the pin binds the new impl against \
                  every paired byte-view and str-owned axis on the same \
                  enum plus a generic <T: Into<Vec<u8>>>-bound consumer \
                  witness and a std::io::Write::write_all-shape owned-\
                  byte-sink surface witness on both owned and borrowed \
                  input shapes to lock the whole family against a future \
                  silent regression"
    )]
    fn restart_strategy_from_into_owned_vec_bytes_routes_through_as_str_accessor() {
        // `<T: Into<Vec<u8>>>`-bound-consumer witness helper: a generic
        // owned-byte-input function accepts a [`super::RestartStrategy`]
        // directly through the trait bound, without the caller open-
        // coding the three-hop `strategy.as_str().as_bytes().to_vec()`
        ...
        // `Into<Vec<u8>>`, so a future per-supervisor per-`:estrategia`
        ...
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<RestartStrategy> for Vec<u8>` and
        // `impl From<&RestartStrategy> for Vec<u8>` — asserts the trait-
        ...
        // `.as_bytes().to_vec()` byte-tail resolve to the same four-arm
        // PascalCase wire byte-string emit-set across every arm the
        // exhaustive [`super::RestartStrategy::ALL`] slice enumerates.
        // Extends the substrate-wide trait-idiomatic byte-owned
        // reverse-projection axis onto the first M2-OTP-shape closed-
        // set fieldless typed-enum peer on the caixa surface
        // (`:supervisor :estrategia`), matching the trajectory the
        // first-mover [`super::crate::CaixaKind`] lift (b245fd6), the
        // second-mover [`super::crate::dialeto::CaixaDialeto`] lift
        // (4cceaf5), and the third-mover
        // [`super::crate::dep::DepList`] lift (e974ca2) established
        // across the caixa-core-internal tier.
        for &variant in RestartStrategy::ALL {
            let via_owned_from: Vec<u8> = <Vec<u8> as From<RestartStrategy>>::from(variant);
            let via_borrowed_from: Vec<u8> = <Vec<u8> as From<&RestartStrategy>>::from(&variant);
            let via_method_bytes: Vec<u8> = variant.as_str().as_bytes().to_vec();
            assert_eq!(
                via_owned_from, via_method_bytes,
                "From<RestartStrategy> for Vec<u8> impl must byte-equal \
                 RestartStrategy::as_str().as_bytes().to_vec() on \
                 RestartStrategy::{variant:?} — divergence signals a \
                 silent detour off the substrate-primitive accessor"
            );
            assert_eq!(
                via_borrowed_from, via_method_bytes,
                "From<&RestartStrategy> for Vec<u8> impl must byte-\
                 equal RestartStrategy::as_str().as_bytes().to_vec() \
                 on RestartStrategy::{variant:?} — divergence signals \
                 a silent detour off the substrate-primitive accessor"
            );
            assert_eq!(
                ...
                "From<RestartStrategy> for Vec<u8> and \
                 From<&RestartStrategy> for Vec<u8> must byte-equal \
                 each other on RestartStrategy::{variant:?} — \
            ...
            // borrowed byte-view axis (cd4c4e0): the byte-owned
            ...
            // primitive accessor.
            let borrowed_bytes: &[u8] = <RestartStrategy as AsRef<[u8]>>::as_ref(&variant);
            assert_eq!(
                via_owned_from,
                borrowed_bytes.to_vec(),
                "From<RestartStrategy> for Vec<u8> and AsRef<[u8]> \
                 for RestartStrategy must resolve to byte-equal byte-\
                 tails on RestartStrategy::{variant:?} — divergence \
                 signals the byte-owned and byte-view axes have \
                 drifted off the same substrate-primitive as_str \
                 accessor"
            );
            ...
            // allocates (or borrows) the same PascalCase wire byte-
            // string the substrate-primitive accessor emits, so the
            // byte-owned axis must byte-equal each of their owned
            // byte-tails by construction.
            let owned_string: String = <String as From<RestartStrategy>>::from(variant);
            assert_eq!(
                via_owned_from,
                owned_string.into_bytes(),
                "From<RestartStrategy> for Vec<u8> and \
                 String::from(strategy).into_bytes() must resolve to \
                 byte-equal byte-tails on RestartStrategy::{variant:?}"
            );
            let owned_cow: std::borrow::Cow<'static, str> =
                <std::borrow::Cow<'static, str> as From<RestartStrategy>>::from(variant);
            assert_eq!(
            ...
            assert_eq!(
                ...
                "From<RestartStrategy> for Vec<u8> and \
                 From<RestartStrategy> for Box<str> must resolve to \
                 byte-equal byte-tails on RestartStrategy::{variant:?}"
            );
            let owned_arc: std::sync::Arc<str> =
                <std::sync::Arc<str> as From<RestartStrategy>>::from(variant);
        ...
        // `clippy::items_after_statements`) accepts a
        // [`super::RestartStrategy`] and a `&RestartStrategy`
        // directly through the trait bound, without the caller open-
        // coding the three-hop `strategy.as_str().as_bytes().to_vec()`
        // composition.
        for &variant in RestartStrategy::ALL {
            ...
            // `&RestartStrategy` local so the generic-consumer witness
            // routes through `From<&RestartStrategy> for Vec<u8>` (T
            // binds to `&RestartStrategy`) rather than clippy-collapsing
        ...
        // [`super::RestartStrategy`] directly on both owned and
        // borrowed input shapes, routing its byte-tail through the
        // substrate-primitive `as_str` accessor — the shape a future
        // per-supervisor per-`:estrategia` audit-log emit composes to
        // fold an `:estrategia` discriminator byte-tag into a
        ...
        for &variant in RestartStrategy::ALL {
            ...
            assert_eq!(
                owned_folded,
                variant.as_str().as_bytes(),
                "`sink.write_all(strategy)`-shape composition on \
                 RestartStrategy::{variant:?} must fold the same byte-\
                 tail RestartStrategy::as_str().as_bytes() returns"
            );
    ...
    #[test]
    fn restart_policy_from_into_owned_string_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<RestartPolicy> for String` — asserts the
        // owned-`String`-returning standard-library trait impl and the
        // substrate-primitive [`RestartPolicy::as_str`] `pub const fn`
        // accessor resolve to the same three-arm emit-set across every
        // arm the exhaustive [`RestartPolicy::ALL`] slice enumerates.
        // Rust's standard library does not carry a blanket
        // `impl<T: AsRef<str>> From<T> for String` (nor an
        // `impl<T: fmt::Display> From<T> for String`), so the
        // owned-`String` forward-projection axis is a distinct
        // trait-idiomatic surface that a `let key: String =
        // policy.into();`-shaped call site reaches through this impl
        // and no other — the paired sibling `From<RestartPolicy> for
        // &'static str` impl forces every owned-`String` call site
        // through an explicit `.to_owned()` / `String::from`
        // restatement. Peer of the first-mover
        // [`restart_strategy_from_into_owned_string_routes_through_as_str_accessor`]
        // (7baa18a) — extends the trait-idiomatic owned-`String`
        // forward-projection axis onto the second-of-two M2 OTP-shape
        // closed-set typed enums on the caixa surface (per-child
        // restart-decision-policy sibling on the same M2 `:supervisor`
        // slot).
        for &variant in RestartPolicy::ALL {
            let via_trait: String = <String as From<RestartPolicy>>::from(variant);
            let via_method: &'static str = variant.as_str();
            assert_eq!(
                via_trait.as_str(),
                via_method,
                "From<RestartPolicy> for String impl must round-trip \
                 RestartPolicy::{variant:?} to the same lifted \
                 SUPERVISOR_CHILD_RESTART_* const RestartPolicy::as_str \
                 returns — divergence signals a silent detour off the \
                 substrate-primitive accessor"
            ...
            assert_eq!(
                via_into.as_str(),
                via_method,
                "Into<String>::into on RestartPolicy::{variant:?} must \
                 byte-equal RestartPolicy::as_str on the same input — the \
                 blanket-derived Into shape must resolve to the same as_str \
                 dispatch as the explicit From impl"
            );
    ...
    #[test]
    fn restart_policy_from_into_owned_string_and_static_str_agree_on_every_arm() {
        // Cross-axis partition pin: the paired trait-idiomatic
        // owned-`String` `From<RestartPolicy> for String` (this lift)
        // and owned-`&'static str` `From<RestartPolicy> for &'static
        // str` (9fb37d0) forward projections must resolve identically
        // on every arm, locking the two return-type-shape paths
        // together so any future detour trips at caixa-core test time.
        // Also byte-parity witness against the sibling
        // [`ToString::to_string`] surface routed through
        ...
        // resolve identically on every arm so a future consumer that
        // picks any of the three lands on the same lifted
        // SUPERVISOR_CHILD_RESTART_* const. Then a `.iter().copied()
        // .map(String::from)` pipe witness over [`RestartPolicy::ALL`]
        // that materializes the three-arm accept-set through the
        // owned-`String` axis alone — the exact shape a future
        // wasm-operator per-child post-exit restart-decision
        // diagnostic line composer or a
        // `HashMap::<String, RestartPolicy>::from_iter(
        //     RestartPolicy::ALL.iter().copied().map(|p| (p.into(), p)))`-style
        // owned-key per-policy lookup reaches through — closing the
        // owned-`String` forward-projection axis's iterator-pipe
        // shape. Then a direct round-trip witness through the paired
        ...
        for &variant in RestartPolicy::ALL {
            ...
            assert_eq!(
                owned_string.as_str(),
                owned_static,
                "From<RestartPolicy> for String and From<RestartPolicy> \
                 for &'static str must resolve identically on \
                 RestartPolicy::{variant:?} — divergence signals the \
                 owned-`String` and owned-`&'static str` forward-projection \
                 return-type-shape paths have drifted onto different \
            ...
            assert_eq!(
                owned_string, via_to_string,
                "From<RestartPolicy> for String must byte-equal \
                 RestartPolicy::to_string on RestartPolicy::{variant:?} — \
                 divergence signals the trait-idiomatic owned-`String` \
                 forward-projection axis and the ToString-through-Display \
                 axis have drifted onto different emit-sets"
            );
        }
        let via_iter: Vec<String> = RestartPolicy::ALL
            .iter()
            .copied()
            .map(String::from)
            .collect();
        let via_method: Vec<String> = RestartPolicy::ALL
            .iter()
            .map(|p| p.as_str().to_owned())
            .collect();
        assert_eq!(
            via_iter, via_method,
            "`.iter().copied().map(String::from)` over RestartPolicy::ALL \
             must byte-equal `.iter().map(|p| p.as_str().to_owned())` on \
             every arm — the owned-`String` `From<RestartPolicy> for \
             String` axis is what makes the `String::from` composition \
             route through the substrate-primitive `RestartPolicy::as_str` \
             accessor rather than through a per-call-site `.to_owned()` / \
             `String::from(policy.as_str())` detour"
        );
        for &variant in RestartPolicy::ALL {
            let emitted: String = variant.into();
            let re_parsed: Result<RestartPolicy, ()> =
                <RestartPolicy as TryFrom<&str>>::try_from(emitted.as_str());
            assert_eq!(
                re_parsed,
                Ok(variant),
                "trait-idiomatic owned-`String` forward-projection + \
                 reverse-projection axis pair must round-trip \
                 RestartPolicy::{variant:?} through `.into::<String>()` \
                 and back through `TryFrom<&str>` on the owned-`String`'s \
                 String::as_str borrow — a break signals the owned-`String` \
                 forward-emit and reverse-parse axes have drifted onto \
                 different vocabularies"
    ...
    #[test]
    fn restart_policy_from_into_borrowed_owned_string_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<&RestartPolicy> for String` — asserts the
        ...
        // [`RestartPolicy::as_str`] `pub const fn` accessor resolve to
        // the same three-arm emit-set across every arm the exhaustive
        // [`RestartPolicy::ALL`] slice enumerates. Rust's standard
        // library does not carry a blanket `impl<T: AsRef<str>>
        // From<&T> for String` (nor an `impl<T: fmt::Display> From<&T>
        // for String`), so the borrowed-input owned-`String` forward-
        // projection axis is a distinct trait-idiomatic surface that a
        // `let key: String = (&policy).into();`-shaped call site
        // reaches through this impl and no other — the paired sibling
        // `From<RestartPolicy> for String` impl forces every borrowed-
        // input call site through an explicit `Copy` deref
        // (`String::from(*policy)`) or an `.as_str().to_owned()` /
        // `.to_string()` detour. Peer of the first-mover
        // [`restart_strategy_from_into_borrowed_owned_string_routes_through_as_str_accessor`]
        // (579385f) — extends the trait-idiomatic borrowed-input
        // owned-`String` forward-projection axis onto the second-of-
        // two M2 OTP-shape closed-set typed enums on the caixa surface
        // (per-child restart-decision-policy sibling on the same M2
        // `:supervisor` slot).
        for &variant in RestartPolicy::ALL {
            let via_trait: String = <String as From<&RestartPolicy>>::from(&variant);
            let via_method: &'static str = variant.as_str();
            ...
            assert_eq!(
                ...
                via_method,
                "Into<String>::into on &RestartPolicy::{variant:?} must \
                 byte-equal RestartPolicy::as_str on the same input — \
    ...
    #[test]
    fn restart_policy_from_into_borrowed_owned_string_agrees_with_paired_axes_on_every_arm() {
        // Cross-axis partition pin: the newly lifted trait-idiomatic
        // borrowed-input owned-`String` `From<&RestartPolicy> for
        // String` (this lift), the paired owned-input owned-`String`
        // `From<RestartPolicy> for String` (7851725), the paired
        // borrowed-input owned-`&'static str` `From<&RestartPolicy>
        // for &'static str` (842c7f3), and the paired owned-input
        // owned-`&'static str` `From<RestartPolicy> for &'static str`
        // (9fb37d0) — every corner of the `{Self, &Self} × {&'static
        // str, String}` 2×2 trait-idiomatic projection family — must
        // resolve identically on every arm, locking the four
        // return-shape × input-shape paths together so any future
        // detour trips at caixa-core test time. Also byte-parity
        ...
        // [`restart_strategy_from_into_borrowed_owned_string_agrees_with_paired_axes_on_every_arm`]
        // (579385f) — closes the whole `{Self, &Self} × {&'static str,
        // String}` 2×2 projection corner on both M2 OTP-shape sibling
        // peers.
        for &variant in RestartPolicy::ALL {
            ...
            assert_eq!(
                ...
                "From<&RestartPolicy> for String and From<RestartPolicy> \
                 ...
                 break in the diagonal corner of the {{Self, &Self}} × \
                 {{&'static str, String}} 2×2 trait-idiomatic \
                 projection family"
            );
            let via_to_string: String = variant.to_string();
        ...
        let via_iter: Vec<String> = RestartPolicy::ALL.iter().map(String::from).collect();
        let via_method: Vec<String> = RestartPolicy::ALL
            .iter()
            .map(|p| p.as_str().to_owned())
            .collect();
        assert_eq!(
            via_iter, via_method,
            "`.iter().map(String::from)` over RestartPolicy::ALL — a \
             call site whose iteration axis holds `&RestartPolicy` by \
             construction — must byte-equal `.iter().map(|p| \
             p.as_str().to_owned())` on every arm — the borrowed-input \
             owned-`String` `From<&RestartPolicy> for String` axis is \
             what makes the `String::from` composition route through \
             the substrate-primitive `RestartPolicy::as_str` accessor \
             without a spurious `Copy` deref (which would only be \
             reachable through the owned-input `From<RestartPolicy> \
             for String` axis by first calling `.copied()` on the \
             iterator)"
        );
        for &variant in RestartPolicy::ALL {
            let emitted: String = (&variant).into();
            ...
            assert_eq!(
                ...
                "trait-idiomatic borrowed-input owned-`String` \
                 forward-projection + reverse-projection axis pair must \
                 round-trip &RestartPolicy::{variant:?} through \
                 `.into::<String>()` on the borrowed-input surface and \
                 back through `TryFrom<&str>` on the owned-`String`'s \
                 String::as_str borrow — a break signals the \
                 borrowed-input owned-`String` forward-emit and \
    ...
    #[test]
    fn restart_policy_from_borrowed_into_static_cow_str_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<&RestartPolicy> for std::borrow::Cow<'static, str>` —
        // asserts the borrowed-input standard-library trait impl and
        ...
        // the borrowed-input `Cow<'static, str>` forward-projection
        // axis is a distinct trait-idiomatic surface that a
        // `let key: Cow<'static, str> = (&policy).into();`-shaped
        // call site or a
        // `RestartPolicy::ALL.iter().map(Cow::from)`-shaped pipe
        // reaches through this impl and no other — the paired owned-
        // input `From<RestartPolicy> for Cow<'static, str>` impl
        // (0612398) forces every borrowed-input call site through an
        // explicit `Copy` deref (`Cow::from(*policy)`) or a
        // `Cow::Borrowed(policy.as_str())` open-code whose type
        // bounds have no compile-time link back to the substrate
        // primitive.
        //
        // Also asserts the projection lands on the zero-alloc
        ...
        // surface.
        //
        // Closes the `{Self, &Self}` input-shape corner on the M2
        // OTP-shape per-child-restart [`Cow<'static, str>`] axis on
        // the second-of-two-in-M2 closed-set fieldless typed enum peer
        // on the caixa surface (`:supervisor :children :restart`),
        // exactly as d45c409 closed it on the top-level
        // [`super::CaixaKind`] one commit after the owning half
        // (99c1735) landed and as 9b3e4b3 closed it on the sibling
        // M2 OTP-shape [`super::RestartStrategy`] one commit after
        // (7dd28b3) landed. This lift closes the whole M2 OTP-shape
        // tier of the substrate-wide Cow<'static, str> forward-
        // projection campaign on both input-shape corners
        // ({Self, &Self}) of both M2 OTP-shape sibling peers.
        for &variant in RestartPolicy::ALL {
            let via_trait: std::borrow::Cow<'static, str> =
                <std::borrow::Cow<'static, str> as From<&RestartPolicy>>::from(&variant);
    ...
    #[test]
    fn restart_policy_from_borrowed_into_static_cow_str_agrees_with_paired_axes_on_every_arm() {
        // Cross-axis partition pin: the newly lifted trait-idiomatic
        // borrowed-input `From<&RestartPolicy> for
        // std::borrow::Cow<'static, str>` (this lift), the paired
        // owned-input `From<RestartPolicy> for
        // std::borrow::Cow<'static, str>` (0612398), the paired
        // borrowed-input owned-`&'static str` `From<&RestartPolicy>
        // for &'static str`, and the paired borrowed-input owned-
        // `String` `From<&RestartPolicy> for String` must resolve
        // identically on every arm, locking the four
        // return-shape × input-shape paths together by construction so
        // any future detour trips at caixa-core test time. Also byte-
        // parity witness against the sibling [`ToString::to_string`]
        // surface routed through [`std::fmt::Display`] — every owned-
        // heap-string path (this axis's `.into_owned()` promotion, the
        // paired [`From<&RestartPolicy> for String`], and
        // `.to_string()`) resolves to the same lifted
        // [`crate::render::SUPERVISOR_CHILD_RESTART_*`] const per arm.
        //
        // Then a `.iter().map(std::borrow::Cow::from)` pipe witness
        ...
        // accessor without a spurious [`Copy`] deref (which would only
        // be reachable through the owned-input
        // [`From<RestartPolicy> for Cow<'static, str>`] axis by first
        ...
        // [`restart_strategy_from_borrowed_into_static_cow_str_agrees_with_paired_axes_on_every_arm`]
        // (9b3e4b3) on the M2 OTP-shape sibling-restart axis — closes
        // the whole borrowed-input `Cow<'static, str>` +
        // paired `{&'static str, String}` cross-axis-parity corner on
        // both M2 OTP-shape sibling peers.
        for &policy in RestartPolicy::ALL {
            let borrowed_cow: std::borrow::Cow<'static, str> =
                <std::borrow::Cow<'static, str> as From<&RestartPolicy>>::from(&policy);
            let owned_cow: std::borrow::Cow<'static, str> =
                <std::borrow::Cow<'static, str> as From<RestartPolicy>>::from(policy);
            let borrowed_static: &'static str =
                <&'static str as From<&RestartPolicy>>::from(&policy);
            let borrowed_string: String = <String as From<&RestartPolicy>>::from(&policy);
            assert_eq!(
                borrowed_cow, owned_cow,
                "From<&RestartPolicy> for Cow<'static, str> and \
                 From<RestartPolicy> for Cow<'static, str> must \
                 resolve identically on RestartPolicy::{policy:?} — \
                 divergence signals the borrowed-input and owned-input \
                 Cow<'static, str> forward-projection input-shape \
                 paths have drifted onto different emit-sets"
            );
            assert_eq!(
                ...
                "From<&RestartPolicy> for Cow<'static, str> and \
                 From<&RestartPolicy> for &'static str must resolve \
                 identically on RestartPolicy::{policy:?} — \
                 divergence signals the borrowed-input Cow<'static, \
                 str> and &'static str return-shape paths have drifted \
                 onto different emit-sets"
            );
            ...
            let via_to_string: String = policy.to_string();
            assert_eq!(
                borrowed_cow.as_ref(),
                via_to_string.as_str(),
                "From<&RestartPolicy> for Cow<'static, str> must \
                 byte-equal RestartPolicy::to_string on \
                 RestartPolicy::{policy:?} — divergence signals \
                 the trait-idiomatic borrowed-input Cow<'static, str> \
                 forward-projection axis and the ToString-through-\
                 Display axis have drifted onto different emit-sets"
            );
        ...
        assert_eq!(
            via_iter, via_method,
            "`.iter().map(Cow::from)` over RestartPolicy::ALL — a \
             call site whose iteration axis holds `&RestartPolicy` \
             by construction — must byte-equal `.iter().map(|p| \
             Cow::Borrowed(p.as_str()))` on every arm — the borrowed-\
             input Cow<'static, str> `From<&RestartPolicy> for \
             Cow<'static, str>` axis is what makes the `Cow::from` \
             composition route through the substrate-primitive \
             `RestartPolicy::as_str` accessor with the zero-alloc \
             Cow::Borrowed arm by construction and without a spurious \
             `Copy` deref (which would only be reachable through the \
             owned-input `From<RestartPolicy> for Cow<'static, str>` \
             axis by first calling `.copied()` on the iterator)"
        );
        for cow in &via_iter {
            assert!(
                matches!(cow, std::borrow::Cow::Borrowed(_)),
                "every element of the .iter().map(Cow::from) pipe \
                 over RestartPolicy::ALL must land on the zero-\
                 alloc Cow::Borrowed arm — a Cow::Owned outcome on \
                 any arm signals the pipe's iteration axis has \
                 silently allocated where the substrate-primitive \
                 RestartPolicy::as_str `&'static str` return makes \
    ...
    #[test]
    fn restart_policy_from_into_rc_str_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<RestartPolicy> for std::rc::Rc<str>` — asserts
        // the owned-input standard-library trait impl and the
        // substrate-primitive [`super::RestartPolicy::as_str`]
        // `pub const fn` accessor resolve to the same three-arm emit-
        // set across every arm the exhaustive
        // [`super::RestartPolicy::ALL`] slice enumerates, and cross-
        // witnesses against every sibling owned-input `{&'static str,
        // String, Cow<'static, str>, Box<str>, std::sync::Arc<str>}`
        // return-shape axis so the six return-shape paths on the
        // owned-input surface lock together by construction. Closes
        // the substrate-wide [`std::rc::Rc<str>`] forward-projection
        // campaign on the M2-OTP-shape `:supervisor :estrategia` +
        // `:children :restart` slot pair the sibling-restart
        // [`RestartStrategy`] first-mover (71ad8f4) opened one
        // projection tier prior on the paired sibling enum.
        for &variant in RestartPolicy::ALL {
            let via_trait: std::rc::Rc<str> =
                <std::rc::Rc<str> as From<RestartPolicy>>::from(variant);
            let via_method: &'static str = variant.as_str();
            assert_eq!(
                via_trait.as_ref(),
                via_method,
                "From<RestartPolicy> for std::rc::Rc<str> impl must \
                 round-trip RestartPolicy::{variant:?} to the same \
                 lifted SUPERVISOR_CHILD_RESTART_* const \
                 RestartPolicy::as_str returns — divergence signals \
                 a silent detour off the substrate-primitive accessor"
            ...
            assert_eq!(
                via_trait.as_ref(),
                owned_static,
                "From<RestartPolicy> for std::rc::Rc<str> and \
                 From<RestartPolicy> for &'static str must resolve \
                 identically on RestartPolicy::{variant:?}"
            );
            let owned_string: String = <String as From<RestartPolicy>>::from(variant);
    ...
    #[test]
    fn restart_policy_from_borrowed_into_rc_str_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<&RestartPolicy> for std::rc::Rc<str>` — asserts
        // the borrowed-input standard-library trait impl and the
        // substrate-primitive [`super::RestartPolicy::as_str`]
        // `pub const fn` accessor resolve to the same three-arm emit-
        // set across every arm the exhaustive
        // [`super::RestartPolicy::ALL`] slice enumerates. Closes the
        // `{Self, &Self}` input-shape corner of the
        // [`std::rc::Rc<str>`] axis on this enum, cross-witnesses
        // against the paired owned-input axis and every sibling
        // borrowed-input return-shape axis, and locks the
        // `.iter().map(std::rc::Rc::<str>::from)` pipe over
        // [`super::RestartPolicy::ALL`] to the substrate-primitive
        // accessor without a spurious [`Copy`] deref (which would only
        // be reachable through the owned-input axis by first calling
        // `.copied()` on the iterator).
        for &variant in RestartPolicy::ALL {
            ...
            assert_eq!(
                ...
                "Into<std::rc::Rc<str>>::into on \
                 &RestartPolicy::{variant:?} must byte-equal \
                 RestartPolicy::as_str on the same input"
            );
            let owned_rc: std::rc::Rc<str> =
                <std::rc::Rc<str> as From<RestartPolicy>>::from(variant);
            assert_eq!(
                via_trait, owned_rc,
                "From<&RestartPolicy> for std::rc::Rc<str> and \
                 From<RestartPolicy> for std::rc::Rc<str> must \
                 resolve identically on RestartPolicy::{variant:?}"
            );
            let borrowed_static: &'static str =
                <&'static str as From<&RestartPolicy>>::from(&variant);
        ...
        let via_iter: Vec<std::rc::Rc<str>> = RestartPolicy::ALL
            .iter()
            .map(std::rc::Rc::<str>::from)
            .collect();
        let via_method: Vec<std::rc::Rc<str>> = RestartPolicy::ALL
            .iter()
        ...
        assert_eq!(
            ...
            "`.iter().map(std::rc::Rc::<str>::from)` over \
             ...
             on every arm — the borrowed-input std::rc::Rc<str> \
             `From<&RestartPolicy> for std::rc::Rc<str>` axis is \
             what makes the `std::rc::Rc::<str>::from` composition \
             route through the substrate-primitive \
             `RestartPolicy::as_str` accessor without a spurious \
             `Copy` deref (which would only be reachable through the \
             owned-input `From<RestartPolicy> for std::rc::Rc<str>` \
             axis by first calling `.copied()` on the iterator)"
        );
    }
    ...
    #[test]
    fn restart_strategy_try_from_vec_bytes_routes_through_borrowed_byte_view_axis() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl TryFrom<Vec<u8>> for RestartStrategy` — asserts the trait-
        ...
        // owned reverse-projection axis onto the first M2-OTP-shape
        // supervisor-slot closed-set fieldless typed enum peer — owned-
        // input mirror of the paired [`TryFrom<&[u8]>`] byte-view
        // reverse-projection axis (c699a83), and byte-owned reverse
        // companion of the pre-existing byte-owned *forward*-projection
        // pair ([`From<RestartStrategy> for Vec<u8>`],
        // [`From<&RestartStrategy> for Vec<u8>`]) on this same enum.
        ...
        // (83a1526) on the [`crate::CaixaDialeto`] peer, and the sibling
        // third-mover
        // [`crate::dep::tests::dep_list_try_from_vec_bytes_routes_through_borrowed_byte_view_axis`]
        // (42091cb) on the [`crate::dep::DepList`] peer — tracks the
        ...
        // substrate-primitive [`RestartStrategy::from_wire`] accessor so
        // every future `<T: TryFrom<Vec<u8>>>`-bound owned-byte-vec
        // consumer reaches the same four-arm `PascalCase` wire accept-
        // set through one trait dispatch.
        for &variant in RestartStrategy::ALL {
            let wire_bytes: Vec<u8> = variant.as_str().as_bytes().to_vec();
            assert_eq!(
                <RestartStrategy as TryFrom<Vec<u8>>>::try_from(wire_bytes.clone()),
                Ok(variant),
                "TryFrom<Vec<u8>> impl on RestartStrategy must round-trip \
                 RestartStrategy::{variant:?}.as_str().as_bytes().to_vec() \
                 back to Ok(RestartStrategy::{variant:?}) — divergence \
            ...
            let via_owned: Result<RestartStrategy, ()> =
                <RestartStrategy as TryFrom<Vec<u8>>>::try_from(wire_bytes.clone());
            let via_borrowed: Result<RestartStrategy, ()> =
                <RestartStrategy as TryFrom<&[u8]>>::try_from(wire_bytes.as_slice());
            assert_eq!(
            ...
            // Four-corner witness: because [`RestartStrategy`] carries
            ...
            let owned_forward: Vec<u8> = <Vec<u8> as From<RestartStrategy>>::from(variant);
            let borrowed_forward: Vec<u8> = <Vec<u8> as From<&RestartStrategy>>::from(&variant);
            assert_eq!(
                owned_forward, wire_bytes,
                "From<RestartStrategy> for Vec<u8> forward projection on \
                 RestartStrategy::{variant:?} must byte-equal \
            ...
            assert_eq!(
                ...
                "From<&RestartStrategy> for Vec<u8> forward projection \
                 on &RestartStrategy::{variant:?} must byte-equal \
                 variant.as_str().as_bytes().to_vec() — divergence \
                 signals the paired forward pair drifted off the \
                 substrate-primitive as_str accessor"
            );
            assert_eq!(
                <RestartStrategy as TryFrom<Vec<u8>>>::try_from(owned_forward.clone()),
                Ok(variant),
                "Four-corner round-trip on RestartStrategy::{variant:?} \
                 through From<RestartStrategy> for Vec<u8> then \
                 TryFrom<Vec<u8>> for RestartStrategy must return \
                 Ok(variant) — divergence signals the byte-owned \
                 forward pair and the byte-owned reverse axis have \
                 drifted apart"
            );
            assert_eq!(
                <RestartStrategy as TryFrom<Vec<u8>>>::try_from(borrowed_forward),
                Ok(variant),
                "Four-corner round-trip on RestartStrategy::{variant:?} \
                 through From<&RestartStrategy> for Vec<u8> then \
                 TryFrom<Vec<u8>> for RestartStrategy must return \
    ...
    #[test]
    fn restart_strategy_try_from_vec_bytes_rejects_unknown_and_non_utf8_bytes() {
        // Rejection witness on the `impl TryFrom<Vec<u8>> for
        // RestartStrategy` — sweeps the same two rejection paths the
        ...
        // `Err` before [`RestartStrategy::from_wire`] runs) and the
        // valid-UTF-8-but-unknown-wire rejection path
        // ([`RestartStrategy::from_wire`] returns `None` on a byte-
        ...
        // sibling [`RestartStrategy::from_wire`] doc block makes load-
        ...
        // (83a1526) on the [`crate::CaixaDialeto`] peer, and the
        // sibling third-mover
        // [`crate::dep::tests::dep_list_try_from_vec_bytes_rejects_unknown_and_non_utf8_bytes`]
        // (42091cb) on the [`crate::dep::DepList`] peer rejection
        // witnesses.
        let non_utf8_rejected: &[&[u8]] = &[
            &[0xFF],
            &[0x80],
        ...
        for &input in non_utf8_rejected {
            ...
            assert_eq!(
                <RestartStrategy as TryFrom<Vec<u8>>>::try_from(owned),
                Err(()),
                "TryFrom<Vec<u8>> impl on RestartStrategy must reject \
            ...
            assert_eq!(
                <RestartStrategy as TryFrom<Vec<u8>>>::try_from(input.to_vec()),
                <RestartStrategy as TryFrom<&[u8]>>::try_from(input),
                "TryFrom<Vec<u8>> and TryFrom<&[u8]> reverse-projection \
                 axes on RestartStrategy must agree on the non-UTF-8 \
        ...
        // (c699a83) already pins on the paired byte-view axis: the
        ...
        // wrong English rebrand candidates.
        let unknown_wire_rejected: &[&[u8]] = &[
            b"",
            b" ",
            b"\n",
        ...
        for &input in unknown_wire_rejected {
            ...
            assert_eq!(
                ...
                <RestartStrategy as TryFrom<&[u8]>>::try_from(input),
                "TryFrom<Vec<u8>> and TryFrom<&[u8]> reverse-projection \
                 axes on RestartStrategy must agree on the valid-UTF-8-\
    ...
    #[test]
    fn restart_policy_try_from_bytes_routes_through_from_wire_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl TryFrom<&[u8]> for RestartPolicy` — asserts the trait-
        // idiomatic byte-view reverse-projection standard-library impl
        // and the substrate-primitive [`RestartPolicy::from_wire`]
        // `Option<Self>` accessor resolve to the same three-arm
        // `PascalCase` wire accept-set across every arm the exhaustive
        // [`RestartPolicy::ALL`] slice enumerates. Closes the substrate-
        // wide trait-idiomatic byte-view reverse-projection axis on the
        // M2-OTP-shape `:supervisor :estrategia` + `:children :restart`
        // slot pair the sibling [`RestartStrategy`] first-mover
        // (c699a83) opened one commit prior — mirror of the paired
        // [`TryFrom<&str> for RestartPolicy`] str-view reverse-
        // projection axis on the same enum, and the byte-view companion
        // of the pre-existing byte-owned reverse-projection family
        // ([`AsRef<[u8]>`], [`From<RestartPolicy> for Vec<u8>`],
        ...
        // axis whose parse arm-set carries no compile-time byte-view
        // surface. This impl closes the byte-view reverse-projection
        // axis at the substrate-primitive [`RestartPolicy::from_wire`]
        // accessor so every future `<T: for<'a> TryFrom<&'a [u8]>>`-
        // bound consumer reaches the same three-arm `PascalCase` wire
        // accept-set through one trait dispatch.
        for &variant in RestartPolicy::ALL {
            let wire_bytes: &[u8] = variant.as_str().as_bytes();
            assert_eq!(
                <RestartPolicy as TryFrom<&[u8]>>::try_from(wire_bytes),
                Ok(variant),
                "TryFrom<&[u8]> impl on RestartPolicy must round-trip \
                 RestartPolicy::{variant:?}.as_str().as_bytes() back to \
                 Ok(RestartPolicy::{variant:?}) — divergence from \
                 RestartPolicy::from_wire signals a silent detour off \
                 the substrate-primitive accessor"
            );
            assert_eq!(
                <RestartPolicy as TryFrom<&[u8]>>::try_from(wire_bytes).ok(),
                RestartPolicy::from_wire(variant.as_str()),
                "TryFrom<&[u8]> ok()-projection on \
                 RestartPolicy::{variant:?}.as_str().as_bytes() must \
                 byte-equal RestartPolicy::from_wire on the paired \
                 &str input"
            );
            // Cross-axis witness: the byte-view reverse-projection axis
            // must agree with the paired str-view reverse-projection
            ...
            let via_bytes: Result<RestartPolicy, ()> =
                <RestartPolicy as TryFrom<&[u8]>>::try_from(wire_bytes);
            assert_eq!(
                via_bytes, via_str,
                "TryFrom<&[u8]> and TryFrom<&str> reverse-projection \
            ...
            // paired [`AsRef<[u8]>`] byte-tail back through the new
            // impl and assert it round-trips to the originating arm.
            let via_asref: &[u8] = <RestartPolicy as AsRef<[u8]>>::as_ref(&variant);
            assert_eq!(
                <RestartPolicy as TryFrom<&[u8]>>::try_from(via_asref),
                Ok(variant),
                "TryFrom<&[u8]> ∘ AsRef<[u8]> must round-trip \
                 RestartPolicy::{variant:?} — divergence signals the \
                 forward and reverse byte-view axes have drifted off \
    ...
    #[test]
    fn restart_policy_try_from_bytes_rejects_unknown_and_non_utf8_bytes() {
        // Rejection witness on the `impl TryFrom<&[u8]> for RestartPolicy`
        // — sweeps two rejection paths the byte-view reverse-projection
        // axis collapses onto the single unit-error `Err(())` return: the
        // invalid-UTF-8 rejection path ([`std::str::from_utf8`] returns
        // `Err` before [`RestartPolicy::from_wire`] runs) and the
        // valid-UTF-8-but-unknown-wire rejection path
        // ([`RestartPolicy::from_wire`] returns `None` on a byte-string
        // outside the three-arm `PascalCase` accept-set). Both must
        // reject, so a future accidental widening of the trait impl's
        // accept-set (a case-fold path, a silent acceptance of the
        // kebab-case dispatcher-catalog byte-strings on this axis — which
        // would collide the two-axis wire/catalog split the sibling
        // [`RestartPolicy::from_wire`] doc block makes load-bearing — a
        ...
        for &input in non_utf8_rejected {
            assert_eq!(
                ...
                Err(()),
                "TryFrom<&[u8]> impl on RestartPolicy must reject the \
                 non-UTF-8 byte-sequence {input:?} with Err(()) — \
                 silent acceptance signals the UTF-8 validation path \
                 collapsed onto a default arm rather than the trait-\
                 idiomatic unit-error"
        ...
        // Valid-UTF-8-but-unknown-wire candidates mirror the corpus the
        // sibling `restart_policy_try_from_str_rejects_unknown_byte_strings`
        // str-view rejection witness already pins on the paired
        // [`TryFrom<&str>`] axis: the empty byte-string, whitespace-only
        // padding, the kebab-case dispatcher-catalog byte-strings on the
        // sibling axis the pre-existing [`std::str::FromStr`] impl the
        // [`gen_platform::FromStrKind`] derive installs parses onto (a
        // caller who confuses the two axes trips here rather than at a
        // downstream K8s-CR round-trip miss), lowercase / uppercase /
        // mixed-case folds of each `PascalCase` arm, whitespace-padded /
        // trailing-newline / quote-wrapped forms, and plausible-but-wrong
        // English rebrand candidates (`Ephemeral`, `Always`, `Never`,
        // `OnAbnormalExit`, `intrinsic`).
        let unknown_wire_rejected: &[&[u8]] = &[
            b"",
            b" ",
            b"\n",
        ...
        for &input in unknown_wire_rejected {
            ...
            // paired str-view reverse-projection axis
            // ([`TryFrom<&str>`]) — both route through the same
            // [`RestartPolicy::from_wire`] resolver, so the two
            // rejection paths align by construction.
            if let Ok(s) = std::str::from_utf8(input) {
                assert_eq!(
                    <RestartPolicy as TryFrom<&[u8]>>::try_from(input),
                    <RestartPolicy as TryFrom<&str>>::try_from(s),
                    "TryFrom<&[u8]> and TryFrom<&str> reverse-\
                     projection axes on RestartPolicy must agree on \
                     the valid-UTF-8 input {input:?} — divergence \
                     signals the two reverse paths have drifted off \
                     the same substrate-primitive from_wire accessor"
                );
    ...
    #[test]
    fn restart_strategy_try_from_owned_string_routes_through_borrowed_str_view_axis() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl TryFrom<String> for RestartStrategy` — asserts the
        // trait-idiomatic string-owned reverse-projection standard-
        // library impl and the sibling borrowed-input [`TryFrom<&str>`]
        // axis resolve to the same four-arm `PascalCase` wire accept-set
        // across every arm the exhaustive [`RestartStrategy::ALL`] slice
        // enumerates. Extends the substrate-wide trait-idiomatic string-
        // owned reverse-projection axis onto the first M2-OTP-shape
        // supervisor-slot closed-set fieldless typed-enum peer — owned-
        // input mirror of the paired [`TryFrom<&str>`] str-view reverse-
        // projection axis, and string-owned reverse companion of the
        // pre-existing string-owned *forward*-projection pair
        // ([`From<RestartStrategy> for String`],
        // [`From<&RestartStrategy> for String`]) on this same enum. Peer
        // of the sibling opener
        ...
        // primitive, the sibling first-mover
        // [`crate::aplicacao::tests::wit_shape_try_from_owned_string_routes_through_borrowed_str_view_axis`]
        // (e6aac29) on the [`crate::aplicacao::WitShape`] closed-set peer,
        // the sibling second-mover
        // [`crate::aplicacao::tests::rate_limit_unit_try_from_owned_string_routes_through_borrowed_str_view_axis`]
        // (94a9c5e) on the [`crate::aplicacao::RateLimitUnit`] peer, and
        // the sibling third-mover
        // [`crate::aplicacao::tests::placement_strategy_try_from_owned_string_routes_through_borrowed_str_view_axis`]
        // (d81a70a) on the [`crate::aplicacao::PlacementStrategy`] peer —
        // tracks the "delegate through `TryFrom<&str>` on the
        // `String::as_str` borrow" discipline the compound-primitive
        // opener and closed-set-peer first-mover established.
        //
        // Rust's standard library carries no blanket
        // `impl<T: for<'a> TryFrom<&'a str>> TryFrom<String> for T`, so
        // an owned-`String` caller otherwise picks between an open-coded
        // `<T as TryFrom<&str>>::try_from(s.as_str())` at every call
        // site (whose type bounds have no compile-time link back to the
        // string-owned reverse-projection axis) or a `let s: &str = &s;
        // T::try_from(s)` two-step whose borrow arithmetic leaks a per-
        // call-site lifetime dance. This impl closes the string-owned
        // reverse-projection axis at the substrate-primitive
        // [`RestartStrategy::from_wire`] accessor so every future
        // `<T: TryFrom<String>>`-bound owned-string consumer reaches the
        // same four-arm `PascalCase` accept-set through one trait
        // dispatch.
        for &variant in RestartStrategy::ALL {
            let wire_string: String = variant.as_str().to_string();
            assert_eq!(
                <RestartStrategy as TryFrom<String>>::try_from(wire_string.clone()),
                Ok(variant),
                "TryFrom<String> impl on RestartStrategy must round-trip \
                 RestartStrategy::{variant:?}.as_str().to_string() back \
                 to Ok(RestartStrategy::{variant:?}) — divergence from \
            ...
            // `PascalCase` wire vocabulary through the substrate-primitive
            // `from_wire` accessor, and the owned-input axis delegates to
            // the borrowed peer by design.
            let via_owned: Result<RestartStrategy, ()> =
                <RestartStrategy as TryFrom<String>>::try_from(wire_string.clone());
            let via_borrowed: Result<RestartStrategy, ()> =
                <RestartStrategy as TryFrom<&str>>::try_from(wire_string.as_str());
            assert_eq!(
                via_owned, via_borrowed,
            ...
            // Cross-axis witness against the paired byte-view and byte-
            // owned reverse axes — the four reverse paths (str-view
            // borrowed, string-owned, byte-view borrowed, byte-owned)
            // share one substrate primitive.
            let via_bytes_borrowed: Result<RestartStrategy, ()> =
                <RestartStrategy as TryFrom<&[u8]>>::try_from(wire_string.as_bytes());
            let via_bytes_owned: Result<RestartStrategy, ()> =
                <RestartStrategy as TryFrom<Vec<u8>>>::try_from(wire_string.as_bytes().to_vec());
            assert_eq!(
                via_owned, via_bytes_borrowed,
                "TryFrom<String> and TryFrom<&[u8]> reverse-projection \
                 axes on RestartStrategy must agree on \
                 RestartStrategy::{variant:?} — divergence signals the \
                 string-owned and byte-view reverse paths have drifted \
                 off the same substrate-primitive from_wire accessor"
            );
            assert_eq!(
                ...
                "TryFrom<String> and TryFrom<Vec<u8>> reverse-projection \
                 axes on RestartStrategy must agree on \
                 RestartStrategy::{variant:?} — divergence signals the \
                 string-owned and byte-owned reverse paths have drifted \
                 off the same substrate-primitive from_wire accessor"
            );
            ...
            // canonical `PascalCase` scalar. Both the owned-input
            // `From<RestartStrategy> for String` and the borrowed-input
            // `From<&RestartStrategy> for String` corners must feed back
            // through the new impl to `Ok(variant)`.
            let owned_forward: String = <String as From<RestartStrategy>>::from(variant);
            let borrowed_forward: String = <String as From<&RestartStrategy>>::from(&variant);
            assert_eq!(
                owned_forward, wire_string,
                "From<RestartStrategy> for String forward projection on \
                 RestartStrategy::{variant:?} must byte-equal \
                 variant.as_str().to_string() — divergence signals the \
                 paired forward pair drifted off the substrate-primitive \
                 as_str accessor"
            );
            assert_eq!(
            ...
            assert_eq!(
                <RestartStrategy as TryFrom<String>>::try_from(owned_forward.clone()),
                Ok(variant),
                "Closed-cycle round-trip on RestartStrategy::{variant:?} \
                 through From<RestartStrategy> for String then \
                 TryFrom<String> for RestartStrategy must return \
                 Ok(variant) — divergence signals the string-owned \
                 forward pair and the string-owned reverse axis have \
                 drifted apart"
            );
            assert_eq!(
                <RestartStrategy as TryFrom<String>>::try_from(borrowed_forward),
                Ok(variant),
                "Closed-cycle round-trip on RestartStrategy::{variant:?} \
                 through From<&RestartStrategy> for String then \
                 TryFrom<String> for RestartStrategy must return \
                 Ok(variant) — divergence signals the borrowed-input \
                 forward corner and the owned-input string reverse \
                 corner have drifted apart"
            );
        }
    }

    #[test]
    fn restart_strategy_try_from_owned_string_rejects_unknown_wire_strings() {
        // Rejection witness on the `impl TryFrom<String> for
        // RestartStrategy` — sweeps the corpus of valid-UTF-8-but-
        // unknown-wire byte-strings the sibling borrowed [`TryFrom<&str>`]
        // axis already rejects and asserts every one lands on `Err(())`,
        // so a future accidental widening of the trait impl's accept-set
        // (a case-fold path, a silent inclusion of the kebab-case
        // dispatcher-catalog byte-strings on the sibling axis that would
        // collide the two-axis wire/catalog split the sibling
        // [`RestartStrategy::from_wire`] doc block makes load-bearing, a
        // stray fallback that maps whitespace-padded canonical scalars
        // onto their unpadded arm rather than the trait-idiomatic
        // `Err(())`) trips at caixa-core test time. Peer of the sibling
        // borrowed-input rejection witness
        // [`restart_strategy_try_from_str_rejects_unknown_byte_strings`]
        // on the same enum, and the sibling byte-view / byte-owned
        // rejection witnesses
        // [`restart_strategy_try_from_bytes_rejects_unknown_and_non_utf8_bytes`]
        // (c699a83) /
        // [`restart_strategy_try_from_vec_bytes_rejects_unknown_and_non_utf8_bytes`]
        // (34951fe) on the same enum.
        ...
        for &input in unknown_wire_rejected {
            ...
            assert_eq!(
                <RestartStrategy as TryFrom<String>>::try_from(owned),
                Err(()),
                "TryFrom<String> impl on RestartStrategy must reject the \
                 valid-UTF-8-but-unknown-wire byte-string {input:?} with \
                 Err(()) — silent acceptance signals an accept-set \
                 widening off the paired RestartStrategy::from_wire \
                 resolver"
            );
            ...
            // axis delegates to the borrowed peer.
            assert_eq!(
                <RestartStrategy as TryFrom<String>>::try_from(input.to_string()),
                <RestartStrategy as TryFrom<&str>>::try_from(input),
                "TryFrom<String> and TryFrom<&str> reverse-projection \
                 axes on RestartStrategy must agree on the valid-UTF-8-\
                 but-unknown-wire input {input:?} — divergence signals \
    ...
    #[test]
    fn restart_policy_try_from_owned_string_routes_through_borrowed_str_view_axis() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl TryFrom<String> for RestartPolicy` — asserts the trait-
        // idiomatic string-owned reverse-projection standard-library impl
        // and the sibling borrowed-input [`TryFrom<&str>`] axis resolve
        // to the same three-arm `PascalCase` wire accept-set across every
        // arm the exhaustive [`RestartPolicy::ALL`] slice enumerates.
        // Extends the substrate-wide trait-idiomatic string-owned reverse-
        // projection axis onto the second (and final) M2-OTP-shape
        // supervisor-slot closed-set fieldless typed-enum peer — owned-
        // input mirror of the paired [`TryFrom<&str>`] str-view reverse-
        // projection axis, and string-owned reverse companion of the
        // pre-existing string-owned *forward*-projection pair
        // ([`From<RestartPolicy> for String`],
        // [`From<&RestartPolicy> for String`]) on this same enum. Peer
        // of the sibling first-mover
        // [`crate::aplicacao::tests::rate_limit_try_from_owned_string_routes_through_borrowed_str_view_axis`]
        // (a2e6f02) on the compound [`crate::aplicacao::RateLimit`]
        // primitive, and the sibling
        // [`restart_strategy_try_from_owned_string_routes_through_borrowed_str_view_axis`]
        // (78fe8c8) on the sibling first M2-OTP-shape supervisor-slot
        // [`RestartStrategy`] peer — tracks the "delegate through
        // `TryFrom<&str>` on the `String::as_str` borrow" discipline the
        // compound-primitive opener and closed-set-peer first-mover
        // established. This closes the string-owned reverse-projection
        // axis on the M2-OTP-shape `:supervisor :estrategia` +
        // `:children :restart` slot pair.
        for &variant in RestartPolicy::ALL {
            let wire_string: String = variant.as_str().to_string();
            assert_eq!(
                <RestartPolicy as TryFrom<String>>::try_from(wire_string.clone()),
                Ok(variant),
                "TryFrom<String> impl on RestartPolicy must round-trip \
                 RestartPolicy::{variant:?}.as_str().to_string() back \
                 to Ok(RestartPolicy::{variant:?}) — divergence from \
                 the sibling TryFrom<&str> axis signals a silent detour \
                 off the substrate-primitive from_wire accessor"
            );
            // Cross-axis witness: the string-owned reverse-projection
            ...
            let via_owned: Result<RestartPolicy, ()> =
                <RestartPolicy as TryFrom<String>>::try_from(wire_string.clone());
            let via_borrowed: Result<RestartPolicy, ()> =
                <RestartPolicy as TryFrom<&str>>::try_from(wire_string.as_str());
            assert_eq!(
                via_owned, via_borrowed,
                "TryFrom<String> and TryFrom<&str> reverse-projection \
                 axes on RestartPolicy must agree on \
            ...
            // borrowed, string-owned, byte-view borrowed, byte-owned)
            // share one substrate primitive.
            let via_bytes_borrowed: Result<RestartPolicy, ()> =
                <RestartPolicy as TryFrom<&[u8]>>::try_from(wire_string.as_bytes());
            let via_bytes_owned: Result<RestartPolicy, ()> =
                <RestartPolicy as TryFrom<Vec<u8>>>::try_from(wire_string.as_bytes().to_vec());
            assert_eq!(
                via_owned, via_bytes_borrowed,
                "TryFrom<String> and TryFrom<&[u8]> reverse-projection \
                 axes on RestartPolicy must agree on \
            ...
            // → Self` round-trips to the originating arm on every
            // canonical `PascalCase` scalar. Both the owned-input
            // `From<RestartPolicy> for String` and the borrowed-input
            // `From<&RestartPolicy> for String` corners must feed back
            // through the new impl to `Ok(variant)`.
            let owned_forward: String = <String as From<RestartPolicy>>::from(variant);
            let borrowed_forward: String = <String as From<&RestartPolicy>>::from(&variant);
            assert_eq!(
                owned_forward, wire_string,
                "From<RestartPolicy> for String forward projection on \
                 RestartPolicy::{variant:?} must byte-equal \
                 variant.as_str().to_string() — divergence signals the \
                 paired forward pair drifted off the substrate-primitive \
                 as_str accessor"
            ...
            assert_eq!(
                <RestartPolicy as TryFrom<String>>::try_from(owned_forward.clone()),
                Ok(variant),
                "Closed-cycle round-trip on RestartPolicy::{variant:?} \
                 through From<RestartPolicy> for String then \
                 TryFrom<String> for RestartPolicy must return \
                 Ok(variant) — divergence signals the string-owned \
                 forward pair and the string-owned reverse axis have \
                 drifted apart"
            );
            assert_eq!(
                <RestartPolicy as TryFrom<String>>::try_from(borrowed_forward),
                Ok(variant),
                "Closed-cycle round-trip on RestartPolicy::{variant:?} \
                 through From<&RestartPolicy> for String then \
                 TryFrom<String> for RestartPolicy must return \
    ...
    #[test]
    fn restart_policy_try_from_owned_string_rejects_unknown_wire_strings() {
        // Rejection witness on the `impl TryFrom<String> for
        // RestartPolicy` — sweeps the corpus of valid-UTF-8-but-
        // unknown-wire byte-strings the sibling borrowed [`TryFrom<&str>`]
        // axis already rejects and asserts every one lands on `Err(())`,
        // so a future accidental widening of the trait impl's accept-set
        // (a case-fold path, a silent inclusion of the kebab-case
        ...
        // `Err(())`) trips at caixa-core test time. Peer of the sibling
        // borrowed-input rejection witness
        // [`restart_policy_try_from_str_rejects_unknown_byte_strings`]
        // on the same enum, and the sibling byte-view / byte-owned
        // rejection witnesses
        // [`restart_policy_try_from_bytes_rejects_unknown_and_non_utf8_bytes`]
        // (d9ef5f0) /
        // [`restart_policy_try_from_vec_bytes_rejects_unknown_and_non_utf8_bytes`]
        // (7592085) on the same enum.
        let unknown_wire_rejected: &[&str] = &[
            "",
            " ",
            "\n",
        ...
        for &input in unknown_wire_rejected {
            let owned: String = input.to_string();
            assert_eq!(
                <RestartPolicy as TryFrom<String>>::try_from(owned),
                Err(()),
                "TryFrom<String> impl on RestartPolicy must reject the \
                 valid-UTF-8-but-unknown-wire byte-string {input:?} with \
                 Err(()) — silent acceptance signals an accept-set \
                 widening off the paired RestartPolicy::from_wire \
                 resolver"
            );
            // Cross-axis witness against the borrowed str-view axis:
            // the two paths must agree by construction, since the owned
            // axis delegates to the borrowed peer.
            assert_eq!(
                <RestartPolicy as TryFrom<String>>::try_from(input.to_string()),
                <RestartPolicy as TryFrom<&str>>::try_from(input),
                "TryFrom<String> and TryFrom<&str> reverse-projection \
                 axes on RestartPolicy must agree on the valid-UTF-8-\
                 but-unknown-wire input {input:?} — divergence signals \
