...
/// projection on the two-list dep-graph [`DepList`] closed-set typed
/// enum — the fourth (and closing) corner of the
/// `{Self, &Self} × {&'static str, String}` 2×2 trait-idiomatic
/// projection family on this enum, mirror of the peer M2 OTP-shape
/// [`From<&RestartStrategy> for String`] (579385f) and
/// [`From<&RestartPolicy> for String`] (8465740) that opened and
/// closed the corner on the sibling supervisor-level restart-strategy
/// and per-child restart-decision enums. Routes byte-for-byte through
/// the substrate-primitive [`DepList::as_str`] `pub const fn` accessor
/// (via [`str::to_owned`]) so every consumer that holds a borrowed
/// [`&DepList`] and needs an owned [`String`] — a future
/// `serde_json::Value::String(String::from(&list))` structured-payload
/// composer over a borrowed field, a future `Iterator::map` over
/// `&[DepList]` that projects to owned keys through
/// `.iter().map(String::from)` (whose iterator yields `&DepList`, not
/// `DepList`, so the owned-input [`From<DepList> for String`] axis
/// alone forces every call site through an explicit `.copied()` /
/// spurious [`Copy`] deref restatement rather than the direct trait-
/// idiomatic projection), a future `HashMap::<String, DepList>::from_iter`
/// that keys off a borrowed-iteration axis where dereferencing the list
/// would force an unnecessary `Copy` at every step, the future
/// wasm-operator's per-manifest `list_axes.iter().map(String::from).collect()`
/// per-list author-surface-tag diagnostic emit whose iteration axis is
/// borrowed by construction — reaches the same two-arm lifted
/// [`crate::render::DEP_AUTHOR_KEY_DEPS`] /
/// [`crate::render::DEP_AUTHOR_KEY_DEPS_DEV`] const the paired
/// [`std::fmt::Display`], [`AsRef<str>`], [`DepList::as_str`], and the
/// three other trait-idiomatic forward-projection impls
/// ([`From<DepList> for &'static str`],
/// [`From<&DepList> for &'static str`],
/// [`From<DepList> for String`]) already return.
///
/// Third peer on the substrate-wide trait-idiomatic *borrowed-input,
/// owned-`String` output* forward-projection family opened on
/// [`crate::supervisor::RestartStrategy`] (579385f) and closed on
/// [`crate::supervisor::RestartPolicy`] (8465740) — extends the
/// `{Self, &Self} × {&'static str, String}` 2×2 projection corner off
/// the M2 OTP-shape axis pair onto the first non-M2 closed-set
/// fieldless typed enum peer (the two-list dep-graph axis). Rust's
/// standard library does not carry a blanket
/// `impl<T: AsRef<str>> From<&T> for String` (nor an
/// `impl<T: fmt::Display> From<&T> for String`), so every closed-set
/// typed enum that carries the paired `AsRef<str>` / `Display` /
/// `From<Self> for &'static str` / `From<&Self> for &'static str` /
/// `From<Self> for String` quintuple but not the borrowed-input owned-
/// [`String`] axis forces every borrowed-input owned-string call site
/// through a `list.as_str().to_owned()` / `String::from(*list)` (with a
/// spurious `Copy`) / `list.to_string()` (through `Display`) detour
/// whose type bounds have no compile-time link to the substrate
/// primitive.
///
/// Same as the peer [`crate::supervisor::RestartStrategy`] /
/// [`crate::supervisor::RestartPolicy`] borrowed-input owned-[`String`]
/// axis pairs (whose forward emit and reverse parse share one
/// vocabulary by construction — `PascalCase` on the M2 OTP-shape
/// peers), [`DepList`]'s [`DepList::as_str`] emit and
/// [`DepList::from_wire`] parse resolve through the same lifted
/// [`crate::render::DEP_AUTHOR_KEY_DEPS`] /
/// [`crate::render::DEP_AUTHOR_KEY_DEPS_DEV`] consts by construction
/// (the `":deps"` / `":deps-dev"` leading-colon lispy author-surface
/// tags — there is no wire/diagnostic axis split on this enum), so the
/// borrowed-input owned-[`String`] projection this impl exposes
...
///
/// The remaining ten closed-set typed enums on the caixa substrate
/// surface (`CaixaKind`, `CaixaDialeto`, `PlacementStrategy`,
...
/// carries the same paired quintuple that this borrowed-input owned-
/// [`String`] axis extends onto.
///
/// Pinned load-bearing by
/// [`tests::dep_list_from_into_borrowed_owned_string_routes_through_as_str_accessor`]
/// (byte-parity pin against [`DepList::as_str`] across the two-arm
/// emit-set through the borrowed-input surface) and
/// [`tests::dep_list_from_into_borrowed_owned_string_agrees_with_paired_axes_on_every_arm`]
/// (cross-axis partition pin against the paired owned-input owned-
/// [`String`] [`From<DepList> for String`] impl, the paired borrowed-
/// input owned-[`&'static str`] [`From<&DepList> for &'static str`]
/// impl, and the sibling [`ToString::to_string`] surface routed through
/// [`std::fmt::Display`], plus a direct round-trip witness through
...
/// Trait-idiomatic *forward* projection on the two-list dep-graph
/// [`DepList`] closed-set typed enum from an *owned* input onto the
/// borrowed-heap-string [`std::borrow::Cow<'static, str>`] axis —
...
/// [`DepList`] through the standard-library `.into()` /
/// [`From<Self> for std::borrow::Cow<'static, str>`] (equivalently
/// [`Into<std::borrow::Cow<'static, str>>`]) axis reaches the same
/// two-arm lifted [`crate::render::DEP_AUTHOR_KEY_DEPS`] /
/// [`crate::render::DEP_AUTHOR_KEY_DEPS_DEV`] canonical `pub const
/// &str` values the paired [`From<DepList> for &'static str`],
/// [`From<&DepList> for &'static str`], [`From<DepList> for String`],
/// and [`From<&DepList> for String`] 2×2 trait-idiomatic forward-
/// projection corners, the sibling [`std::fmt::Display`],
/// [`AsRef<str>`], and [`DepList::as_str`] surfaces already return,
/// rather than an open-coded per-call-site
/// `std::borrow::Cow::Borrowed(list.as_str())` /
/// `std::borrow::Cow::Owned(list.to_string())` composition whose
...
/// values with static lifetime), so the zero-alloc borrowed arm is
/// the type-correct projection with no runtime allocation. The
/// paired [`std::borrow::Cow::Owned`] arm stays reachable at the
/// call site through the existing [`From<DepList> for String`] axis
/// composed with [`std::borrow::Cow::from`] on the resulting owned
/// [`String`] — a caller who chose to mutate the projection lands
/// on the owned arm by their own composition, not by the substrate-
...
/// [`From<DepList> for &'static str`], [`From<DepList> for String`],
/// [`AsRef<str>`], and [`std::fmt::Display`] surfaces do not
/// implicitly extend to a [`std::borrow::Cow<'static, str>`]-bound
/// call site — every such site is forced through a
/// `Cow::Borrowed(list.as_str())` / `Cow::Owned(list.to_string())`
/// open-code whose type bounds have no compile-time link back to
/// the substrate primitive until this lift.
///
/// First-mover on the outside-M3 substrate-wide tier of the
/// substrate-wide trait-idiomatic [`std::borrow::Cow<'static, str>`]
/// forward-projection campaign, opening the tier on the first
/// caixa-core-internal closed-set fieldless typed enum peer outside
/// the M2 OTP-shape and M3 mesh-shape tiers. The
/// [`crate::CaixaKind`] top-level first-mover
/// (99c1735 owned-input, d45c409 borrowed-input) opened the axis on
/// the structurally most fundamental closed-set fieldless typed
...
/// the M2 OTP-shape tier; the paired M3-mesh-shape
/// [`crate::aplicacao::WitShape`] `:contratos :wit` census-label
/// (8634dec, 25690ef), [`crate::aplicacao::PlacementStrategy`]
/// `:placement :estrategia` distribution-strategy (eee504d,
/// afdf0f4), and [`crate::aplicacao::RateLimitUnit`] `:politicas
/// :rate-limit` canonical-suffix (1d59925, `From<&RateLimitUnit>`
/// Cow closer) closed the M3-mesh-shape tier. The remaining
/// outside-M3 caixa-core peers ([`crate::CaixaDialeto`],
...
/// future targets of this campaign; the paired borrowed-input
/// [`From<&DepList> for std::borrow::Cow<'static, str>`]
/// `{Self, &Self}`-closer on this outside-M3-tier-opening peer is
/// the next commit's target.
///
/// Same three-path convergence discipline as the paired sibling
/// [`From<DepList> for &'static str`] / [`From<DepList> for String`]
/// / [`std::fmt::Display`] / [`AsRef<str>`] surfaces (this
/// [`std::borrow::Cow<'static, str>`] axis, the paired sibling
/// surfaces, and [`DepList::as_str`] all route through the same two
/// lifted [`crate::render::DEP_AUTHOR_KEY_DEPS`] /
/// [`crate::render::DEP_AUTHOR_KEY_DEPS_DEV`] `pub const &str`
/// values by construction), so a future variant addition, rename,
/// or per-arm wire-tag drift reaches every forward-projection path
/// through exactly one caixa-core edit at the [`DepList::as_str`]
/// `match` head.
///
/// Pinned load-bearing by
/// [`tests::dep_list_from_into_static_cow_str_routes_through_as_str_accessor`]
/// (byte-parity + zero-alloc [`std::borrow::Cow::Borrowed`]-arm pin
/// against [`DepList::as_str`] across the two-arm [`DepList::ALL`])
/// and
/// [`tests::dep_list_from_into_static_cow_str_agrees_with_paired_axes_on_every_arm`]
/// (cross-axis partition pin against the paired
/// [`From<DepList> for &'static str`], [`From<DepList> for String`],
/// and [`ToString`]-through-[`std::fmt::Display`] axes, plus a
/// `.iter().copied().map(Cow::from)` pipe witness over
/// [`DepList::ALL`] that materializes the two-arm accept-set through
/// the [`std::borrow::Cow<'static, str>`] axis alone and pins the
...
// Fold the eleven `DepError::FonteCaminho<Variant> { nome: nome.to_string(),
// caminho: caminho.to_string() }` two-slot struct-variant wire-up sites at
// [`DepSource::validate_caminho`] onto one substrate primitive per typed
// variant — the paired `{ nome: String, caminho: String }` two-slot family
// on [`DepError`], sibling of the peer
// [`crate::supervisor::supervisor_caixa_only_ctors!`] (db09650, 3 variants
// on `{ caixa: String }`) on the `SupervisorError` envelope, the peer
// [`crate::aplicacao::contrato_empty_pair_ctors!`] (8580068, 4 variants on
// `{ de, para }`), [`crate::aplicacao::aplicacao_field_reason_ctors!`]
// (981060b, 7 variants on `{ <field>: String, reason: String }`),
// [`crate::aplicacao::contrato_target_ctors!`] (14b81d5, 2 variants on
// `{ de, para, wit, expected }`), and
// [`crate::aplicacao::contrato_pair_value_reason_ctors!`] (14e13f1, 3
// variants on `{ de, para, <field>: String, reason: String }`) on the
// `AplicacaoError` envelopes, the peer
...
// (3fe3dd7, 6 variants on `<Variant>(String)`) on the `LayoutError`
// envelopes, the peer three [`crate::limits::limits_codec_value_*_ctors!`]
// codec-family macros (81c856c, 12 wire-ups) on the `LimitsError`
// envelope, and the peer [`crate::upgrade::upgrade_from_script_ctors!`]
// (8e67041, 3 variants on `{ from: String, script: PathBuf }`) on the
// `UpgradeError` envelope. First fold family on this `DepError` envelope.
//
// Each of the eleven wire-up sites on this shape (the leading-byte cascade
// closing the seven `FonteCaminho{Absolute, TildeExpansion, VarExpansion,
// LeadingWhitespace, LeadingHyphen}` arms; the orthogonal single-metachar
// cascade closing `FonteCaminhoBackslash` on `\`; the shell-lexer-metachar
// cascade closing `FonteCaminhoShell{Pipe, Semicolon, Background,
// CommandSubstitution}` on the four single-byte shell operators; and the
// trailing-`/` reproducibility arm closing `FonteCaminhoTrailingSlash`)
// opened the identical `DepError::FonteCaminho<Variant> { nome:
// nome.to_string(), caminho: caminho.to_string() }` four-line
// struct-literal against the same `(nome: &str, caminho: &str)` local pair
...
// `AplicacaoError` / `SupervisorError` / `LimitsError` / `UpgradeError`
// families each closed on their sibling envelopes. The eleven variants
// share one `{ nome: String, caminho: String }` shape, so the fold routes
// each wire-up site through one dispatch per typed variant.
//
// The macro below generates one `#[must_use]` inherent constructor per
// variant of shape `fn <ctor>(nome: &str, caminho: &str) -> Self`, so every
// wire-up site collapses onto one dispatch:
// `DepError::<ctor>(nome, caminho)`, byte-equal to the pre-lift
// struct-literal on the same `(&str, &str)` fixture. The uniform two-field
...
// re-checking the `:fonte :caminho` axis, a per-lacre overlay resolver
...
// four-line struct-literal in lockstep with the eleven in-crate wire-up
// sites.
...
mod tests {
    ...
    // Fail-before-pass-after pins on the eleven
    // [`fonte_caminho_ctors!`]-generated `DepError::fonte_caminho_*`
    // constructors folded from the [`DepSource::validate_caminho`]
    // wire-up sites. Each pins the generated ctor's output to the
    // pre-lift struct-literal on the same `(&str, &str)` fixture, so
    // any wrapper-side lowercase / trim / re-order / silent-field-swap
    // regression on the two-field `{ nome: nome.to_string(), caminho:
    // caminho.to_string() }` construction surfaces here rather than at
    // a downstream diagnostic-shape mismatch. Peer of the sibling
    // `empty_child_version_ctor_matches_struct_literal_wrap` /
    // `contrato_endpoint_empty_ctor_matches_struct_literal_wrap` /
    // `entrada_host_invalid_ctor_matches_struct_literal_wrap` /
    // `nome_only_ctor_routes_caixa_through_nome_accessor` equivalence
    // pins on the peer `SupervisorError` / `AplicacaoError` /
    // `LayoutError` / `LimitsError` / `UpgradeError` envelopes.
    ...
    fn fonte_caminho_ctors_route_nome_and_caminho_through_to_string() {
        // Cross-axis pin: sweep the two constructor input axes
        // (`nome: &str`, `caminho: &str`) through a non-default fixture
        // pair against every generated arm in the
        // [`fonte_caminho_ctors!`] macro, so any wrapper-side lowercase
        // / trim / truncate / re-order on the two-field
        // `{ nome, caminho }` construction — or a silent field swap
        // between the two axes at codegen time — surfaces here rather
        // than at a downstream diagnostic-shape mismatch. Peer of the
        // sibling `supervisor_caixa_only_ctors_route_caixa_through_
        // to_string` cross-axis routing pin on the peer
        // `SupervisorError` envelope, extended here onto the
        // `DepError` `{ nome: String, caminho: String }` envelope so
        // every substrate-primitive ctor family in caixa-core
        // guarantees each `&str`-field construction routes the
        // caller's `&str` verbatim through `.to_string()`.
    ...
    #[test]
    fn dep_list_from_into_borrowed_owned_string_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<&DepList> for String` — asserts the borrowed-input
        // owned-`String`-returning standard-library trait impl and the
        // substrate-primitive [`super::DepList::as_str`] `pub const fn`
        // accessor resolve to the same two-arm emit-set across every
        ...
        // `let key: String = (&list).into();`-shaped call site reaches
        // through this impl and no other — the paired sibling
        // `From<DepList> for String` impl forces every borrowed-input
        // call site through an explicit `Copy` deref
        // (`String::from(*list)`) or an `.as_str().to_owned()` /
        ...
        // (579385f) and the second-peer
        // [`crate::supervisor::tests::restart_policy_from_into_borrowed_owned_string_routes_through_as_str_accessor`]
        // (8465740) — extends the trait-idiomatic borrowed-input owned-
        // `String` forward-projection axis off the M2 OTP-shape sibling
        // axis pair onto the first non-M2 closed-set fieldless typed
        // enum peer (the two-list dep-graph axis).
        for &variant in super::DepList::ALL {
            let via_trait: String = <String as From<&super::DepList>>::from(&variant);
            let via_method: &'static str = variant.as_str();
            assert_eq!(
    ...
    #[test]
    fn dep_list_from_into_borrowed_owned_string_agrees_with_paired_axes_on_every_arm() {
        // Cross-axis partition pin: the newly lifted trait-idiomatic
        // borrowed-input owned-`String` `From<&DepList> for String`
        // (this lift), the paired owned-input owned-`String`
        // `From<DepList> for String` (32b0ee8), the paired borrowed-
        // input owned-`&'static str` `From<&DepList> for &'static str`
        // (64aa742), and the paired owned-input owned-`&'static str`
        // `From<DepList> for &'static str` (3455cbf) — every corner of
        ...
        // (8465740) — closes the whole `{Self, &Self} × {&'static str,
        // String}` 2×2 projection corner on the third substrate-wide
        // closed-set fieldless typed enum peer (the two-list dep-graph
        // axis, first outside the M2 OTP-shape sibling pair).
        //
        // Unlike the peer [`crate::CaixaKind`] axis pair (whose forward
        // `From` emit lands on the lowercase Portuguese `as_str`
        // diagnostic vocabulary while the reverse `TryFrom<&str>`
        ...
        // forcing the round-trip through an intermediate
        // [`crate::CaixaKind::wire_name`] hop), [`super::DepList`]'s
        // [`super::DepList::as_str`] emit and
        // [`super::DepList::from_wire`] parse resolve through the same
        // lifted [`crate::render::DEP_AUTHOR_KEY_DEPS`] /
        // [`crate::render::DEP_AUTHOR_KEY_DEPS_DEV`] consts by
        // construction (there is no wire/diagnostic axis split on this
        // enum), so the borrowed-input owned-`String` forward axis and
        // the reverse axis compose directly — matching the peer
        // [`crate::supervisor::RestartStrategy`] /
        // [`crate::supervisor::RestartPolicy`] borrowed-input owned-
        // `String` axis pairs.
        for &list in super::DepList::ALL {
            let borrowed_string: String = <String as From<&super::DepList>>::from(&list);
            let owned_string: String = <String as From<super::DepList>>::from(list);
            let borrowed_static: &'static str =
                <&'static str as From<&super::DepList>>::from(&list);
            let owned_static: &'static str = <&'static str as From<super::DepList>>::from(list);
            assert_eq!(
                borrowed_string, owned_string,
                "From<&DepList> for String and From<DepList> for String \
                 must resolve identically on DepList::{list:?} — \
            ...
            assert_eq!(
                ...
                "From<&DepList> for String and From<&DepList> for \
                 &'static str must resolve identically on \
                 DepList::{list:?} — divergence signals the borrowed-\
                 input `&'static str` and owned-`String` return-shape \
            ...
            assert_eq!(
                ...
                "From<&DepList> for String and From<DepList> for \
                 &'static str must resolve identically on \
                 DepList::{list:?} — divergence signals a break in the \
        ...
        let via_iter: Vec<String> = super::DepList::ALL.iter().map(String::from).collect();
        let via_method: Vec<String> = super::DepList::ALL
            .iter()
            .map(|l| l.as_str().to_owned())
            .collect();
        assert_eq!(
            via_iter, via_method,
            "`.iter().map(String::from)` over DepList::ALL — a call \
             site whose iteration axis holds `&DepList` by construction \
             — must byte-equal `.iter().map(|l| l.as_str().to_owned())` \
             on every arm — the borrowed-input owned-`String` \
             `From<&DepList> for String` axis is what makes the \
             `String::from` composition route through the substrate-\
             primitive `DepList::as_str` accessor without a spurious \
             `Copy` deref (which would only be reachable through the \
             owned-input `From<DepList> for String` axis by first \
        ...
        for &variant in super::DepList::ALL {
            ...
            assert_eq!(
                ...
                "trait-idiomatic borrowed-input owned-`String` \
                 ...
                 round-trip &DepList::{variant:?} through \
                 ...
                 DepList's forward emit and reverse parse share the \
                 same lifted DEP_AUTHOR_KEY_DEPS* consts by \
    ...
    #[test]
    fn dep_list_from_into_static_cow_str_routes_through_as_str_accessor() {
        // Fail-before-pass-after byte-parity pin on the newly lifted
        // `impl From<DepList> for std::borrow::Cow<'static, str>` —
        // asserts the standard-library trait impl and the substrate-
        // primitive [`super::DepList::as_str`] `pub const fn`
        // accessor resolve to the same two-arm emit-set across every
        // arm the exhaustive [`super::DepList::ALL`] slice
        ...
        // a `let key: Cow<'static, str> = list.into();`-shaped call
        // site reaches through this impl and no other — the paired
        // sibling `From<DepList> for &'static str` and
        // `From<DepList> for String` impls force every
        // `Cow<'static, str>`-parameterized call site through a
        // `Cow::Borrowed(list.as_str())` /
        // `Cow::Owned(list.to_string())` composition whose type
        ...
        // [`super::DepList::as_str`] accessor's `&'static str`
        // return lifetime by construction (each match arm resolves
        // to one of the two lifted
        // [`crate::render::DEP_AUTHOR_KEY_DEPS`] /
        // [`crate::render::DEP_AUTHOR_KEY_DEPS_DEV`] `pub const
        // &str` values) makes the borrowed arm the type-correct
        // projection with no runtime allocation. Any future silent
        // detour that routes the impl through the owned arm trips
        ...
        // [`crate::CaixaKind`] top-level opener (99c1735 + d45c409),
        // the paired M2 OTP-shape
        // [`crate::supervisor::RestartStrategy`] (7dd28b3 + 9b3e4b3)
        // and [`crate::supervisor::RestartPolicy`] (0612398 +
        // ee577fd), and the paired M3-mesh-shape
        // [`crate::aplicacao::WitShape`] (8634dec + 25690ef),
        // [`crate::aplicacao::PlacementStrategy`] (eee504d +
        // afdf0f4), and [`crate::aplicacao::RateLimitUnit`] (1d59925)
        // peers onto the first outside-M3 caixa-core peer (the two-
        // list dep-graph axis), opening the outside-M3 caixa-core
        // tier of the substrate-wide Cow<'static, str> forward-
        // projection campaign's owned-input corner.
        for &variant in super::DepList::ALL {
            ...
            assert_eq!(
                ...
                "From<DepList> for Cow<'static, str> impl must \
                 ...
                 crate::render::DEP_AUTHOR_KEY_DEPS* const \
                 DepList::as_str returns — divergence signals a \
    ...
    #[test]
    fn dep_list_from_into_static_cow_str_agrees_with_paired_axes_on_every_arm() {
        // Cross-axis partition pin: the newly lifted trait-idiomatic
        // `From<DepList> for std::borrow::Cow<'static, str>` (this
        // lift), the paired owned-input `From<DepList> for
        // &'static str` (3455cbf), and the paired owned-input
        // `From<DepList> for String` (32b0ee8) forward projections
        ...
        // witness against the sibling [`ToString::to_string`]
        // surface routed through [`std::fmt::Display`] — every
        // owned-heap-string path (the `Cow::Owned` promotion of
        // this axis's `.into_owned()`, `From<DepList> for String`,
        // and `.to_string()`) resolves to the same two-arm lifted
        // [`crate::render::DEP_AUTHOR_KEY_DEPS`] /
        // [`crate::render::DEP_AUTHOR_KEY_DEPS_DEV`] byte-string per
        // arm.
        //
        // Then a `.iter().copied().map(std::borrow::Cow::from)` pipe
        // witness over [`super::DepList::ALL`] that materializes the
        // two-arm accept-set through the
        // [`std::borrow::Cow<'static, str>`] axis alone — the exact
        // shape a future M4 admission-webhook rejection body's
        // accepted-`:deps` / `:deps-dev` list-key enumeration, a
        // future substrate-wide per-arm diagnostic surface whose
        // typing rules out the sibling [`AsRef<str>`] borrowed
        // return, or a future per-arm dep-list emitter that binds
        // through a [`std::borrow::Cow<'static, str>`] boundary
        // reaches through — opening the composable-projection axis
        // on the first outside-M3 caixa-core closed-set fieldless
        ...
        for &variant in super::DepList::ALL {
            let via_cow: std::borrow::Cow<'static, str> =
                <std::borrow::Cow<'static, str> as From<super::DepList>>::from(variant);
            let via_static: &'static str = <&'static str as From<super::DepList>>::from(variant);
            let via_string: String = <String as From<super::DepList>>::from(variant);
            assert_eq!(
                via_cow.as_ref(),
                via_static,
        ...
        assert_eq!(
            via_iter, via_method,
            "`.iter().copied().map(Cow::from)` over DepList::ALL \
             must byte-equal `.iter().map(|l| \
             Cow::Borrowed(l.as_str()))` on every arm — the trait-\
             idiomatic `From<DepList> for Cow<'static, str>` axis is \
             what makes the `Cow::from` composition route through \
             the substrate-primitive `DepList::as_str` accessor with \
             the zero-alloc Cow::Borrowed arm by construction, \
             rather than a per-call-site `Cow::Owned(list.to_string())` \
             allocation"
        );
        for cow in &via_iter {
            assert!(
